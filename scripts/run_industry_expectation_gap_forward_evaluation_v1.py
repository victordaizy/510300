...
CONFIG_PATH = WORKSPACE_ROOT / "config" / "industry_expectation_gap_forward_evaluation_v1.yaml"
...
def _read_calendars(config: dict[str, Any]) -> tuple[list[Path], pd.DatetimeIndex]:
    ...
    return paths, load_trading_calendar(frames)
...
def main() -> int:
    ...
    evaluation_manifest = config["outputs"]["manifest"]
    verify_hash_manifest(WORKSPACE_ROOT, evaluation_manifest)
    ...
    calendar_paths, calendar = _read_calendars(config)
    metadata_paths = _verify_calendar_metadata(config, calendar_paths)
    ...
    inputs = config["mutable_outcome_inputs"]
    ...
    output_directory = _path(config["outputs"]["directory"])
    stem = f"industry_expectation_gap_forward_evaluation_v1_{report['observation_date']}"
