...
CONFIG = ROOT / "config" / "510300_frozen_microstructure_sharpe_audit_v1_0_2_correction.json"
MANIFEST = ROOT / "config" / "510300_frozen_microstructure_sharpe_audit_v1_0_2_manifest.json"
...
def main() -> int:
    if MANIFEST.exists():
        raise FileExistsError("V1.0.2冻结清单已存在，禁止覆盖")
    ...
    if sha256_file(parent_manifest) != correction["parent_manifest_sha256"]:
        raise ValueError("V1.0.1父清单哈希漂移")
    if sha256_file(parent_implementation) != correction["parent_implementation_sha256"]:
        raise ValueError("V1.0.1父实现哈希漂移")
    result_path = ROOT / "reports" / "audit" / "510300_frozen_microstructure_sharpe_audit_v1.json"
    if result_path.exists():
        raise FileExistsError("结果已经存在，禁止覆盖")
    tracked = [
        "config/510300_frozen_microstructure_sharpe_audit_v1_0_2_correction.json",
        "config/510300_frozen_microstructure_sharpe_audit_v1_0_1_manifest.json",
        "research/frozen_microstructure_sharpe_audit_v1_0_1.py",
        "research/frozen_microstructure_sharpe_audit_v1_0_2.py",
        "scripts/freeze_510300_frozen_microstructure_sharpe_audit_v1_0_2.py",
        "scripts/run_510300_frozen_microstructure_sharpe_audit_v1_0_2.py",
        "reports/audit/510300_frozen_microstructure_sharpe_audit_v1_0_1_run_failure.json"
    ]
    for relative in tracked:
        if not (ROOT / relative).is_file():
            raise FileNotFoundError(f"V1.0.2冻结文件不存在：{relative}")
    now = datetime.now(ZoneInfo("Asia/Shanghai")).isoformat()
    payload = {
        "project_id": correction["project_id"],
        "state": "FROZEN_MECHANICAL_LANGUAGE_CORRECTION_AFTER_UNREAD_IN_MEMORY_METRICS",
