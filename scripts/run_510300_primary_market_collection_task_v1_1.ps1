param(
    [string]$TaskName = "Codex-510300-Primary-Market-Collector-V1.1",
    [string]$RunnerPath = "",
    [string]$RuntimeRoot = ""
)
...
$runner = if ([string]::IsNullOrWhiteSpace($RunnerPath)) {
    Join-Path $sourceProjectRoot "scripts\run_510300_primary_market_forward_v1_1.ps1"
}
else {
...
try {
    ...
    try {
        & $powershellExecutable -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $runner -Watch -SkipPaperSignal *>> $logFile
