...
from research.a_share_hs_cash_tender_offer_evaluation_v1_0_3 import (  # noqa: E402
    PROTOCOL_ID,
    build_event_clock,
...
PROTOCOL_DOC = (
    ...
    "EVALUATION_V1_0_3_PROTOCOL.md"
)
MODULE_PATH = ROOT / "research/a_share_hs_cash_tender_offer_evaluation_v1_0_3.py"
RUNNER_PATH = Path(__file__).resolve()
TEST_PATH = ROOT / "tests/test_a_share_hs_cash_tender_offer_evaluation_v1_0_3.py"
PROTOCOL_STATUS = (
...
def verify_upstream(config: dict[str, Any]) -> dict[str, Any]:
    ...
    prior_evaluation = verify_manifest_tree(
        ROOT / inputs["prior_evaluation_protocol_manifest"],
        PROTOCOL_STATUS,
    )
    prior_failure_path = ROOT / inputs["prior_evaluation_failure_receipt"]
    prior_failure = json.loads(prior_failure_path.read_text(encoding="utf-8"))
    expected_prior_failure_status = (
        "FAILED_GLOBAL_INPUT_CONTRACT_H00300_REFERENCE_DATE_NOT_BRIDGEABLE"
    )
    if prior_failure.get("status") != expected_prior_failure_status:
        raise RuntimeError("V1.0.2失败回执状态漂移")
    if prior_failure.get("protocol_manifest_sha256") != prior_evaluation[
        "manifest_sha256"
    ]:
        raise RuntimeError("V1.0.2失败回执与冻结清单不匹配")
    if prior_failure.get("tender_event_market_price_read") is not False:
        raise RuntimeError("V1.0.2失败回执错误声称读取了事件市场价格")
    if prior_failure.get("tender_event_future_return_read") is not False:
        raise RuntimeError("V1.0.2失败回执错误声称读取了事件未来收益")
    ...
    return {
        ...
        "prior_evaluation_protocol": prior_evaluation,
        "prior_evaluation_failure_receipt": {
            "status": "PASS_PRIOR_EVALUATION_FAILURE_RECEIPT_VERIFIED",
            "path": relative(prior_failure_path),
            "sha256": sha256_file(prior_failure_path),
            "failure_status": prior_failure["status"],
...
def validate_plan(config: dict[str, Any]) -> dict[str, Any]:
    ...
    _, h00300_completion_validation = validate_h00300_completion(
        config, event_clock
    )
    return {
        ...
        "event_clock_without_market_validation": {
            "status": "PASS_EVENT_CLOCK_WITHOUT_MARKET_VALIDATED",
            "event_count": len(event_clock),
            "tender_event_market_price_read": False,
            "tender_event_future_return_read": False,
        },
        "h00300_completion_validation": h00300_completion_validation,
...
def freeze_protocol(config: dict[str, Any]) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "h00300_input_completion": {
            "reason": "V1.0.2因H00300参考含非开市日2018-06-18而在事件市场提取前停止；V1.0.3仅对冻结交易日历覆盖范围内的开市日桥接，并强制覆盖全部事件入场日和现金可用日",
            "open_bridge_formula": "H00300_CLOSE_T_TIMES_CSI300_PRICE_OPEN_T_DIV_CSI300_PRICE_CLOSE_T",
            "reference_date_scope": "FROZEN_EXCHANGE_OPEN_DATES_WITHIN_TRADING_CALENDAR_COVERAGE",
            "non_open_date_imputation": False,
...
def verify_protocol(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "benchmark_input_values_read_for_v1_0_1_and_v1_0_2_failure_diagnosis": True,
        "event_clock_dates_read_for_v1_0_2_date_scope_diagnosis": True,
    }
...
def validate_h00300_completion(
    config: dict[str, Any],
    event_clock: pd.DataFrame | None = None,
    ...
    trading_calendar["date"] = pd.to_datetime(
        trading_calendar["date"], errors="coerce"
    ).dt.normalize()
    ...
    if trading_calendar["date"].isna().any() or trading_calendar.empty:
        raise RuntimeError("冻结交易日历日期无效或为空")
    ...
    calendar_start = trading_calendar["date"].min()
    calendar_end = trading_calendar["date"].max()
    open_dates = set(
        trading_calendar.loc[
            trading_calendar["is_open"].fillna(False).astype(bool), "date"
        ].tolist()
    )
    within_calendar_coverage = reference["date"].between(
        calendar_start, calendar_end, inclusive="both"
    )
    exchange_open_reference = reference.loc[
        within_calendar_coverage & reference["date"].isin(open_dates)
    ].copy()
    excluded_non_open = reference.loc[
        within_calendar_coverage & ~reference["date"].isin(open_dates), "date"
    ].sort_values()
    overlap = exchange_open_reference.merge(price_index, on="date", how="inner")
    if len(overlap) != len(exchange_open_reference):
        missing_dates = sorted(
            set(exchange_open_reference["date"]) - set(price_index["date"])
        )
        raise RuntimeError(
            "H00300开盘桥接未覆盖冻结日历范围内的全部开市参考日期："
            f"{len(overlap)}/{len(exchange_open_reference)}；"
            f"缺失日期={[str(item.date()) for item in missing_dates]}"
        )
    output = pd.DataFrame(
        {
            "date": overlap["date"],
    ...
    close_check = output[["date", "close"]].merge(
        exchange_open_reference[["date", "h00300_close"]],
        on="date",
        how="inner",
        validate="one_to_one",
    )
    close_error = (
        close_check["close"] - close_check["h00300_close"]
    ).abs().max()
    tolerance = float(
    ...
    required_event_dates: set[pd.Timestamp] = set()
    if event_clock is not None:
        required_columns = {"entry_date", "cash_availability_date"}
        missing_columns = sorted(required_columns - set(event_clock.columns))
        if missing_columns:
            raise RuntimeError(f"事件时钟缺少H00300必需日期字段：{missing_columns}")
        required_series = pd.concat(
            [
                pd.to_datetime(event_clock["entry_date"], errors="coerce"),
                pd.to_datetime(
                    event_clock["cash_availability_date"], errors="coerce"
                ),
            ],
            ignore_index=True,
        ).dt.normalize()
        if required_series.isna().any():
            raise RuntimeError("事件时钟存在无效的H00300必需日期")
        required_event_dates = set(required_series.tolist())
        missing_required_dates = sorted(
            required_event_dates - set(output["date"].tolist())
        )
        if missing_required_dates:
            raise RuntimeError(
                "H00300桥接未覆盖全部事件入场日和现金可用日："
                f"{[str(item.date()) for item in missing_required_dates]}"
            )
    return output, {
        "status": "PASS_H00300_EXCHANGE_OPEN_DATE_PRICE_INDEX_BRIDGE_VALIDATED",
        "reference_rows_total": len(reference),
        "reference_rows_before_calendar_coverage": int(
            (reference["date"] < calendar_start).sum()
        ),
        "reference_rows_after_calendar_coverage": int(
            (reference["date"] > calendar_end).sum()
        ),
        "reference_rows_within_calendar_coverage": int(
            within_calendar_coverage.sum()
        ),
        "exchange_open_reference_rows": len(exchange_open_reference),
        "excluded_non_open_reference_rows": len(excluded_non_open),
        "excluded_non_open_reference_dates": [
            str(item.date()) for item in excluded_non_open.tolist()
        ],
        "price_index_rows": len(price_index),
        "overlap_rows": len(overlap),
        "required_event_date_count": len(required_event_dates),
        "missing_required_event_dates": [],
        "calendar_coverage_start": str(calendar_start.date()),
        "calendar_coverage_end": str(calendar_end.date()),
...
def run_evaluation(config: dict[str, Any]) -> dict[str, Any]:
    ...
    h00300, h00300_validation = validate_h00300_completion(config, event_clock)
