...
$orchestratorModule = "scripts.run_priority_forward_codex_automation_v1_10"
$orchestratorFile = Join-Path $projectRoot "scripts\run_priority_forward_codex_automation_v1_10.py"
$supervisorConfig = Join-Path $projectRoot "config\priority_forward_supervisor_v1_6.yaml"
$manifest = Join-Path $projectRoot "config\priority_forward_research_operations_v1_10_manifest.json"
...
foreach ($requiredFile in @($python, $orchestratorFile, $supervisorConfig, $manifest)) {
    if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) {
        throw "计划任务依赖文件缺失：$requiredFile"
    }
}
$manifestSha256 = Get-FileSha256 -LiteralPath $manifest
$arguments = "-m $orchestratorModule --phase morning --config `"$supervisorConfig`" --manifest `"$manifest`" --expected-manifest-sha256 $manifestSha256"
...
function Get-TaskAudit {
    ...
    return [ordered]@{
        ...
        orchestrator_module = $orchestratorModule
        ...
        deployment_patch_id = "WINDOWS_SCHEDULER_ORCHESTRATOR_MODULE_ENTRYPOINT_V1_10_1"
        research_runtime_patch_id = "PCF_IOPV_STRICT_TLS_ROUTE_V1_3_AND_STATUS_MODULE_ENTRYPOINT_V1_8_1"
        pcf_transport_contract = "SSE_STRICT_TRANSPORT_CONTRACT_V1"
...
$task = New-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description "工作日以Python模块入口运行零付费 V1.10 冻结清单；PCF严格TLS，错过窗口不补跑。"

Register-ScheduledTask -TaskName $TaskName -InputObject $task -Force | Out-Null
$registered = Get-ScheduledTask -TaskName $TaskName -ErrorAction Stop
$audit = Get-TaskAudit -Task $registered
if (-not $audit.exact_match) {
    throw "计划任务注册后与 V1.10.1 模块入口不一致"
}
$audit | ConvertTo-Json -Depth 5
