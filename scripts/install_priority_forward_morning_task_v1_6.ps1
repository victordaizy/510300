...
$orchestrator = Join-Path $projectRoot "scripts\run_priority_forward_codex_automation_v1_6.py"
$supervisorConfig = Join-Path $projectRoot "config\priority_forward_supervisor_v1_2.yaml"
$manifest = Join-Path $projectRoot "config\priority_forward_research_operations_v1_6_manifest.json"
...
$task = New-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description "工作日运行零付费 V1.6 冻结清单入口；同日原子占位去重，错过窗口不补跑。"

Register-ScheduledTask -TaskName $TaskName -InputObject $task -Force | Out-Null
$registered = Get-ScheduledTask -TaskName $TaskName -ErrorAction Stop
$audit = Get-TaskAudit -Task $registered
if (-not $audit.exact_match) {
    throw "计划任务注册后与 V1.6 入口不一致"
}
$audit | ConvertTo-Json -Depth 5
