...
DEFAULT_MANIFEST = ROOT / "config/priority_forward_research_operations_v1_8_manifest.json"
DEFAULT_CONFIG = ROOT / "config/priority_forward_supervisor_v1_4.yaml"
DELEGATE = ROOT / "scripts/run_priority_forward_codex_automation_v1_5.py"
PATCHED_TASK_LAUNCHER = (
    ROOT / "scripts/run_510300_primary_market_collection_task_v1_2_1.ps1"
)
PATCHED_FORWARD_LAUNCHER = (
    ROOT / "scripts/run_510300_primary_market_forward_v1_2_1.ps1"
)
ACCEPTED_MANIFEST_STATUSES = {
    "FROZEN_ZERO_PAID_FOUNDATION_V1_8_ACTIVE_NEXT_ELIGIBLE_WINDOW",
}
EXPECTED_RUNTIME_PATCH_ID = "PCF_IOPV_PYTHON_MODULE_ENTRYPOINT_V1_2_1"
...
def verify_manifest(manifest_path: Path, expected_manifest_sha256: str) -> dict[str, Any]:
    ...
    if manifest_path != DEFAULT_MANIFEST.resolve():
        raise RuntimeError(f"V1.8 入口只接受固定清单：{DEFAULT_MANIFEST}")
    if not manifest_path.is_file():
        raise RuntimeError(f"V1.8 冻结清单不存在：{manifest_path}")
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("V1.8 清单顶层必须是对象")
    if manifest.get("status") not in ACCEPTED_MANIFEST_STATUSES:
        raise RuntimeError(f"V1.8 清单状态不可执行：{manifest.get('status')}")
    if manifest.get("runtime_patch_id") != EXPECTED_RUNTIME_PATCH_ID:
        raise RuntimeError("V1.8 清单运行补丁标识不匹配")
    if manifest.get("data_purchase_budget_cny") != 0:
        raise RuntimeError("V1.8 清单的数据采购预算不是 0 CNY")
    if manifest.get("maximum_active_research_streams") != 2:
        raise RuntimeError("V1.8 清单的活动研究流上限不是 2")
    files = manifest.get("files")
    if not isinstance(files, list) or not files:
        raise RuntimeError("V1.8 清单没有受控文件")
    ...
    for item in files:
        if not isinstance(item, dict):
            raise RuntimeError("V1.8 清单文件记录必须是对象")
        relative_path = str(item.get("path") or "")
        if not relative_path or relative_path in seen_paths:
            raise RuntimeError(f"V1.8 清单文件路径为空或重复：{relative_path!r}")
    ...
    result = {
        "status": (
            "PASS_V1_8_FROZEN_ENTRYPOINT_VERIFIED"
            if not failures
            else "FAILED_V1_8_FROZEN_ENTRYPOINT_VERIFICATION"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="运行 V1.8 冻结入口")
    ...
    if config_path != DEFAULT_CONFIG.resolve():
        raise RuntimeError(f"V1.8 入口只接受固定编排配置：{DEFAULT_CONFIG}")
