...
$collector = if ([string]::IsNullOrWhiteSpace($CollectorScript)) {
    Join-Path $projectRoot "scripts\collect_510300_primary_market.py"
}
else {
    $CollectorScript
}
...
if ($collectorExitCode -eq 3) {
    [Console]::Error.WriteLine("PCF/IOPV免费外部来源失败，退出码：3")
    exit 3
}
if ($collectorExitCode -ne 0) {
    [Console]::Error.WriteLine("PCF/IOPV采集程序失败，退出码：$collectorExitCode")
    exit 1
}
