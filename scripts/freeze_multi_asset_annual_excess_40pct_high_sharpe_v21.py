...
from scripts.freeze_digital_asset_spot_low_volatility_stop_loss_v10 import (
    verify as verify_v10_manifest,
)
...
CONFIG = ROOT / "config" / "multi_asset_annual_excess_40pct_high_sharpe_v21.yaml"
MANIFEST = ROOT / "config" / "multi_asset_annual_excess_40pct_high_sharpe_v21_manifest.json"
TRACKED_FILES = [
    "config/multi_asset_annual_excess_40pct_high_sharpe_v21.yaml",
    "docs/MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V21_SPEC.md",
    "scripts/freeze_multi_asset_annual_excess_40pct_high_sharpe_v21.py",
]
...
def load_contract() -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V21扩展协议必须是YAML对象")
    objective = payload.get("objective", {})
    lane = payload.get("new_lane", {})
    failures: list[str] = []
    if payload.get("protocol", {}).get("protocol_id") != "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V21":
        failures.append("protocol_id")
    if payload.get("protocol", {}).get("parent_protocol_id") != "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V20":
    ...
    for key, value in {
        ...
        if float(objective.get(key, float("nan"))) != value:
            failures.append(key)
    if lane.get("fixed_spot_symbols") != ["BTCUSDT", "ETHUSDT"]:
        failures.append("fixed_spot_symbols")
    if lane.get("fixed_current_quarter_pairs") != ["BTCUSDT", "ETHUSDT"]:
        failures.append("fixed_current_quarter_pairs")
    if int(lane.get("basis_lookback_calendar_days", 0)) != 5:
        failures.append("basis_lookback")
    if float(lane.get("target_long_gross", float("nan"))) != 0.45:
        failures.append("target_long_gross")
    if float(lane.get("target_short_gross", float("nan"))) != 0.45:
        failures.append("target_short_gross")
    if float(lane.get("target_total_gross", float("nan"))) != 0.90:
        failures.append("target_total_gross")
    if float(lane.get("maximum_order_fraction", float("nan"))) != 0.001:
        failures.append("maximum_order_fraction")
    if float(lane.get("base_total_futures_cost_bps_per_leg", float("nan"))) != 8.0:
        failures.append("base_cost")
    if float(lane.get("stress_total_futures_cost_bps_per_leg", float("nan"))) != 30.0:
        failures.append("stress_cost")
    if any(bool(value) for value in payload.get("safety", {}).values()):
        failures.append("safety")
    if failures:
        raise ValueError(f"V21扩展协议被弱化或损坏：{sorted(set(failures))}")
    return payload
...
def tracked_hashes() -> dict[str, str]:
    ...
    if missing:
        raise FileNotFoundError(f"缺少V21冻结文件：{missing}")
...
def verify_v10_failure(contract: dict[str, Any]) -> dict[str, Any]:
    manifest_path = ROOT / contract["inputs"]["v10_manifest"]
    report_path = ROOT / contract["inputs"]["v10_visible_report"]
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    report = json.loads(report_path.read_text(encoding="utf-8"))
    required = contract["registered_v10_failure"]
    v10_verification = verify_v10_manifest()
    checks = {
        ...
        "manifest_verified": v10_verification.get("failure_count") == 0,
    ...
    if not all(checks.values()):
        raise RuntimeError(f"V10失败不符合V21登记条件：{checks}")
    return {
        "checks": checks,
...
def freeze() -> dict[str, Any]:
    ...
    if parent["failure_count"]:
        raise RuntimeError("V21父协议冻结清单验证失败")
    failure = verify_v10_failure(contract)
    files = tracked_hashes()
    manifest = {
        "schema_version": "1.0.0",
        "manifest_id": "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V21_MANIFEST",
    ...
    if MANIFEST.exists():
        ...
        for field in (
            ...
            if existing.get(field) != manifest.get(field):
                raise RuntimeError("现有V21清单与当前内容不同；不得覆盖")
        return existing
...
def verify() -> dict[str, Any]:
    if not MANIFEST.is_file():
        return {
            "status": "FAIL_MISSING_MULTI_ASSET_40PCT_HIGH_SHARPE_V21_MANIFEST",
            "failure_count": 1,
    ...
    try:
        if existing.get("v10_failure") != verify_v10_failure(contract):
            failures.append("v10_failure")
    except Exception:
        failures.append("v10_failure")
    parent = verify_parent_manifest()
    if parent["failure_count"]:
    ...
    return {
        "status": (
            "PASS_MULTI_ASSET_40PCT_HIGH_SHARPE_V21_MANIFEST_VERIFIED"
            if not failures
            else "FAIL_MULTI_ASSET_40PCT_HIGH_SHARPE_V21_MANIFEST"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="冻结或验证40个百分点高夏普V21扩展协议")
