...
def main():
    dest = ROOT / "scripts/finalize_round84_20260908.py"
    require(not dest.exists(), "84核对来源已建立")
    s = (ROOT / "scripts/finalize_round82_20260908.py").read_text(encoding="utf-8").split('    status = "COMPLETED_MIN_VARIANCE_MAIN_IMPROVEMENT_EARLY_WEAKER_TARGET_NOT_MET"')[0]
    s = s.replace("research.two_policy_min_variance_v1", "research.two_policy_wealth_budget_v1").replace("PRIMARY, P76", "PRIMARY, P46")
    s = s.replace("510300最小方差预算_第82轮_20260908", "510300累计净值预算_第84轮_20260908").replace("最小方差预算_结果及中文规则.md", "累计净值预算_结果及中文规则.md")
    s = s.replace("510300_AFTER_MIN_VARIANCE_TAIL_LOSS_20260908.md", "510300_AFTER_WEALTH_BUDGET_SYNTHETIC_FEAR_20260908.md")
    s = s.replace('== 81 and not OUT.exists()', '== 83 and not OUT.exists()').replace("第82轮前序", "第84轮前序")
    left = s.index("        weights = np.array([.5, .5])")
    right = s.index('        for cost in cfg["costs"]:', left)
    s = s[:left]+'''        for model, column in [("PANIC_ONLY", "panic_reference_equity"), ("REARM_RIDGE", "learned_reference_equity")]:
...
''' + s[right:]
    s = s.replace('P76 / period / cost / "TWO_POLICY_RISK_BUDGET_ledger.parquet"', 'P46 / period / cost / "PANIC_LEARNED_HALF_ledger.parquet"')
    s = s.replace('"saved_risk_budget_differences.csv"', '"saved_equal_budget_differences.csv"')
    s = s.replace('"KEY_MONTHLY_OPTIMUM_AND_COMPLETE_ACCOUNTS_CHECKED"', '"KEY_REFERENCE_EQUITY_DAILY_BUDGET_AND_COMPLETE_ACCOUNTS_CHECKED"').replace('"risk_update_checks":', '"daily_budget_checks":')
    s += '''    status = "COMPLETED_CUMULATIVE_WEALTH_BUDGET_TARGET_NOT_MET"
...
'''
    ...
    print("第84轮关键核对脚本已建立。", flush=True)
