...
from scripts.freeze_priority_forward_research_operations_v1_9 import (
    verify_manifest as verify_v1_9_manifest,
)


BASE_MANIFEST_PATH = ROOT / "config/priority_forward_research_operations_v1_9_manifest.json"
MANIFEST_PATH = ROOT / "config/priority_forward_research_operations_v1_10_manifest.json"
...
V1_10_PATHS = {
    "config/priority_forward_authoritative_status_v1_8_1.yaml",
    "config/priority_forward_supervisor_v1_6.yaml",
    "reports/audit/PRIORITY_FORWARD_V1_10_STATUS_MODULE_ENTRYPOINT_REGISTRATION.json",
    "scripts/install_priority_forward_morning_task_v1_10.ps1",
    "scripts/render_priority_forward_authoritative_status_v1_8_1.py",
    "scripts/run_priority_forward_authoritative_status_task_v1_8_1.ps1",
    "scripts/run_priority_forward_codex_automation_v1_10.py",
}


def tracked_paths() -> list[Path]:
    verify_v1_9_manifest()
...
def build_manifest() -> dict[str, Any]:
    ...
    return {
        ...
        "manifest_id": "PRIORITY_FORWARD_RESEARCH_OPERATIONS_V1_10",
        "status": (
            "FROZEN_ZERO_PAID_FOUNDATION_V1_10_STRICT_TLS_ROUTE_AND_"
            "STATUS_MODULE_ACTIVE_NEXT_ELIGIBLE_WINDOW"
        ),
        "created_at": datetime.now(ZoneInfo("Asia/Shanghai")).isoformat(),
        "effective_from": EFFECTIVE_FROM,
        "change_class": "OPERATIONAL_AUTHORITY_STATUS_PYTHON_MODULE_ENTRYPOINT",
        "runtime_patch_id": RUNTIME_PATCH_ID,
        "supersedes_runtime_manifest": "PRIORITY_FORWARD_RESEARCH_OPERATIONS_V1_9",
        "predecessor_predeployment_failure": {
            "status": "PROGRAM_FAILED_STATUS_RENDERER_FILE_ENTRYPOINT",
            "research_collection_triggered": False,
            "atomic_claim_created": False,
            "immutable_task_receipt_created": False,
            "quality_day_created": False,
            "evidence": (
                "reports/audit/"
                "PRIORITY_FORWARD_V1_10_STATUS_MODULE_ENTRYPOINT_REGISTRATION.json"
            ),
        },
        ...
        "pcf_transport_changed_from_predecessor": False,
        "authority_status_entrypoint_changed_from_predecessor": True,
        ...
        "supervisor_config": "config/priority_forward_supervisor_v1_6.yaml",
        "pcf_runtime": {
            "task_launcher": (
        ...
        "authority_status_runtime": {
            ...
            "config": "config/priority_forward_authoritative_status_v1_8_1.yaml",
            "task_receipt_schema_version": "1.8.1",
        },
        ...
        "mutable_outputs_not_hashed": [
            ...
            "reports/audit/priority_forward_authoritative_status_v1_8_1.json",
            "reports/audit/priority_forward_*_runs_v1_8_1/*.json",
            "reports/audit/priority_forward_*_runs_v1_10/*.json",
            "reports/audit/priority_forward_task_claims_v1_10/*.json",
            "output/**",
        ],
        "terminal_branches": {
...
def verify_manifest() -> dict[str, Any]:
    if not MANIFEST_PATH.is_file():
        raise RuntimeError("V1.10 运行清单不存在")
    ...
    if manifest.get("content_sha256") != content_sha256:
        failures.append({"failure": "CONTENT_SHA256"})
    exact_fields = {
        "status": (
            "FROZEN_ZERO_PAID_FOUNDATION_V1_10_STRICT_TLS_ROUTE_AND_"
            "STATUS_MODULE_ACTIVE_NEXT_ELIGIBLE_WINDOW"
    ...
    authority_runtime = manifest.get("authority_status_runtime", {})
    if authority_runtime.get("renderer_invocation") != (
        "python -m scripts.render_priority_forward_authoritative_status_v1_8_1"
    ):
        failures.append({"failure": "AUTHORITY_STATUS_MODULE_ENTRYPOINT"})
    result = {
        "status": (
            "PASS_PRIORITY_FORWARD_V1_10_MANIFEST_VERIFIED"
            if not failures
            else "FAILED_PRIORITY_FORWARD_V1_10_MANIFEST_VERIFICATION"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="冻结 V1.10 状态模块入口修复清单")
