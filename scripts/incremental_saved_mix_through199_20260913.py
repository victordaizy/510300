...
OUT = ROOT/'reports/research/510300_incremental_saved_mix_through199'
OLD = ROOT/'reports/research/510300_saved_combination_fast_screen_20260913'
...
def main():
    ...
    OUT.mkdir()
    source_folders = ['510300_risk_window_clock_batch_v1', '510300_target_band_partial_rebalance_v1',
    ...
    write_json(OUT/'STARTED.json', {'started_at': now(), 'reuse': str(OLD.relative_to(ROOT)),
        'incremental_source_folders': source_folders, 'starts': ['全部可用来源等权', '原198单独'],
        'max_iterations_per_start': 200, 'source_read_limit_new_candidates': 34,
    ...
    with np.load(OLD/'saved_net_return_matrices.npz') as saved:
        matrices = [saved[f'scenario_{j}'].copy() for j in range(4)]
    require(len(included) == 224 and all(m.shape[1] == 224 for m in matrices), '旧保存矩阵范围不同')
    identities = {hashlib.sha256(np.concatenate([m[:, j] for m in matrices]).astype('<f8').tobytes()).hexdigest(): j for j in range(224)}
    ...
    source_files = [OLD/'included_sources.json', OLD/'saved_net_return_matrices.npz']
    ...
    for name in source_folders:
        ...
        for model in cfg['candidate_models']:
            ...
            source_files += paths
            added += 1
    require(inspected == 34, '本次增量候选数量不同')
    ...
    n = len(included)
    baseline = next(item['column'] for item in included if item['model'] == 'TWO_CLOSE_ZERO_EXIT')
    starts = [('全部可用来源等权', np.full(n, 1/n)), ('原198单独', np.eye(1, n, baseline).ravel())]
    print(f'复用224条矩阵，增量核对{inspected*4}份账本，新增{added}条不同路径；开始两个固定起点。', flush=True)
    ...
    summary = {'completed_at': now(), 'status': 'COMPLETED_INCREMENTAL_VIRTUAL_MIX_NOT_ACCOUNT_BACKTEST',
        'reused_paths': 224, 'new_candidates_read': inspected, 'new_ledger_files_read': inspected*4,
    ...
    lines = ['# 截至199轮的增量虚拟组合筛选', '',
        f"复用224条既有矩阵，仅新增读取{inspected*4}份保存账本；当前{n}条不同完整四场景路径。两次固定起点共{summary['optimizer_objective_evaluations']}次目标函数评价，总耗时{summary['run_seconds']:.2f}秒。没有生成新的交易账户。", '',
        summary['limitation'], '',
