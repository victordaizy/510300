...
$orchestrator = Join-Path $projectRoot "scripts\run_priority_forward_codex_automation_v1_7.py"
$supervisorConfig = Join-Path $projectRoot "config\priority_forward_supervisor_v1_3.yaml"
$manifest = Join-Path $projectRoot "config\priority_forward_research_operations_v1_7_manifest.json"
...
$task = New-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description "工作日运行零付费 V1.7 冻结清单入口；仅以不可变任务回执按日原子去重，错过窗口不补跑。"

Register-ScheduledTask -TaskName $TaskName -InputObject $task -Force | Out-Null
$registered = Get-ScheduledTask -TaskName $TaskName -ErrorAction Stop
$audit = Get-TaskAudit -Task $registered
if (-not $audit.exact_match) {
    throw "计划任务注册后与 V1.7 入口不一致"
}
$audit | ConvertTo-Json -Depth 5
