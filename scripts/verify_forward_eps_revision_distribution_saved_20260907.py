"""核对保存的盈利分布、当时模型、进入退出和完整账户，不重训重跑。"""
...
from research.forward_eps_revision_distribution_features_v1 import build_features
from research.forward_eps_revision_distribution_policy_v1 import MODELS,CONTROLS,COHERENT
...
config=read(ROOT/'config/510300_forward_eps_revision_distribution_policy_v1.json')
result=read(OUT/'result.json')
monthly=pd.read_parquet(OUT/'monthly_features_and_mature_labels.parquet')
source_features=build_features(pd.read_parquet(SOURCE/'company_month_feature_evidence.parquet'),
...
verification={'checked_at':now(),'status':'PASS_SAVED_DISTRIBUTION_MODELS_CLOCKS_AND_COMPLETE_ACCOUNTS',
              'monthly_feature_rows_checked':len(monthly),'saved_trained_models_checked':trained,
              'complete_accounts_checked':len(result['all_metrics']),'reused_control_accounts_checked':8,
...
print('盈利分布、72个保存模型、14条账户、8次退出及保存区间核对通过。',flush=True)
