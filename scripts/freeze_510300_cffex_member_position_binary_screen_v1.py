"""冻结510300中金所IF会员持仓二元筛选V1的协议、程序、测试与输入。"""
...
SOURCE_PATHS = [
    ROOT / "research" / "cffex_member_position_binary_screen_v1.py",
    ...
    ROOT / "scripts" / "download_cffex_if_member_positions_v1.py",
    ROOT / "scripts" / "download_cffex_if_true_term_structure_inputs_v1_0_1.py",
    ROOT / "scripts" / "freeze_510300_cffex_member_position_binary_screen_v1.py",
    ROOT / "tests" / "test_cffex_member_position_binary_screen_v1.py",
]
...
def main() -> int:
    ...
    if protocol["study_id"] != "510300_CFFEX_MEMBER_POSITION_BINARY_SCREEN_V1":
    ...
    if candidate_hash != EXPECTED_CANDIDATE_SHA256:
        raise ValueError("正式会员排名获取前固定的候选合同发生漂移")
    ...
    if int(selection["minimum_eligible_contracts_per_day"]) != 3:
        raise ValueError("最低合资格合约数必须保持为3")
    if int(selection["maximum_rank_inclusive"]) != 20:
        raise ValueError("排名范围必须保持为前20名")
    if int(selection["minimum_calendar_days_to_expiry_inclusive"]) != 5:
        raise ValueError("最低到期天数必须保持为5日")

    manifest_path = ROOT / config["artifacts"]["freeze_manifest"]
    result_paths = [
        ROOT / config["artifacts"]["contract_summaries"],
        ...
        ROOT / config["artifacts"]["report_markdown"],
    ]
    ...
    manifest = {
        ...
        "fixed_contract_and_rank_selection": {
            "minimum_calendar_days_to_expiry_inclusive": int(
                selection["minimum_calendar_days_to_expiry_inclusive"]
            ),
            "minimum_unique_rank_rows_per_contract": int(
                selection["minimum_unique_rank_rows_per_contract"]
            ),
            "maximum_rank_inclusive": int(selection["maximum_rank_inclusive"]),
            "minimum_eligible_contracts_per_day": int(
                selection["minimum_eligible_contracts_per_day"]
            ),
        },
        ...
        "post_run_direction_percentile_min_contract_dte_or_rank_rescue": "FORBIDDEN",
        ...
        json.dumps(
            {
                ...
                "minimum_eligible_contracts_per_day": manifest[
                    "fixed_contract_and_rank_selection"
                ]["minimum_eligible_contracts_per_day"],
            },
            ensure_ascii=False,
            indent=2,
        ),
