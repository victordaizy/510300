"""在读取开发期结果前冻结GammaOIPressure迁移协议。"""
...
MANIFEST = ROOT / "config" / "510300_option_gamma_oi_pressure_v1_manifest.json"
FROZEN_FILES = (
    "config/510300_option_gamma_oi_pressure_v1.yaml",
    "research/option_gamma_oi_pressure_v1.py",
    "scripts/freeze_510300_option_gamma_oi_pressure_v1.py",
    "tests/test_510300_option_gamma_oi_pressure_v1.py",
)
...
def _load_and_assert() -> dict:
    ...
    if config["protocol"]["project_id"] != "510300_OPTION_GAMMA_OI_PRESSURE_V1":
        raise ValueError("GammaOIPressure协议ID漂移")
    if int(config["protocol"]["factor_count"]) > 10:
        raise ValueError("因子数超过用户限定的10个")
    if int(config["protocol"]["factor_count"]) != 1:
        raise ValueError("V1只能包含冻结的单一外部因子")
    ...
    if config["governance"]["parameter_search"] != "FORBIDDEN":
        raise ValueError("该外部公式迁移禁止参数搜索")
    if config["governance"]["validation_or_holdout_access"] != "FORBIDDEN":
        raise ValueError("验证与最终留出必须禁止访问")
...
def verify_protocol() -> tuple[dict, dict]:
    ...
    if not MANIFEST.is_file():
        raise RuntimeError("GammaOIPressure协议尚未冻结")
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    if set(manifest.get("frozen_files", {})) != set(FROZEN_FILES):
        raise RuntimeError("GammaOIPressure冻结文件集合漂移")
    drift = [
        relative
    ...
    if drift:
        raise RuntimeError(f"GammaOIPressure冻结文件哈希漂移：{drift}")
...
def main() -> int:
    ...
    if missing:
        raise FileNotFoundError(f"GammaOIPressure冻结文件缺失：{missing}")
    if MANIFEST.exists():
        verify_protocol()
        print("GammaOIPressure V1已冻结，全部哈希一致。")
        return 0
    for output in (config["paths"]["result_json"], config["paths"]["result_markdown"]):
        if (ROOT / output).exists():
            raise RuntimeError("开发结果已存在，禁止事后冻结因子规则")
    completed = subprocess.run(
        [sys.executable, "-m", "pytest", "tests/test_510300_option_gamma_oi_pressure_v1.py", "-q"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    if completed.returncode != 0:
        ...
        raise RuntimeError(f"GammaOIPressure冻结前测试失败：\n{detail}")
    payload = {
