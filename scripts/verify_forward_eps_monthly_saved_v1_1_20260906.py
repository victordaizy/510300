...
def compact(s):return re.sub(r'\s+','',unicodedata.normalize('NFKC',s).replace('−','-').replace('–','-').replace('—','-'))


def decimal_cell(value):
    token=unicodedata.normalize('NFKC',str(value)).replace('−','-').replace('–','-').replace('—','-').strip()
...
def verify(root,scope='financial',source_mode='original_pdf',fact_version='v1_1',policy_version='v1'):
    b=root/'reports/research';fact=b/f'510300_forward_eps_{scope}_facts_{fact_version}';out=b/f'510300_forward_eps_monthly_policy_{policy_version}_{scope}'
    config=read(root/f'config/510300_forward_eps_monthly_policy_{policy_version}.json');result=read(out/'result.json')
    ...
    for record in records:
        ...
        source_record=root/record['source_record']
        text_file=source_record.parent.parent/'page_texts'/(record['report_id']+'.json')
        saved_pages=read(text_file)
        assert saved_pages['source']['pdf_sha256']==source['pdf_sha256']
        if source_mode=='original_pdf':
            ...
            for actual,saved in zip(pages,saved_pages['pages']):assert actual==saved
        elif source_mode=='saved_page_text':
            pages=saved_pages['pages'][:3]
        else:raise ValueError('来源模式只允许原始PDF或保存原文页文本')
        ...
        for f in record['facts']:
            ...
            assert decimal_cell(f['source_eps_cell'])==Decimal(f['eps_value_exact'])
    ...
    return {'status':'PASS_READ_ONLY_SAVED_SOURCE_FEATURE_LABEL_ACCOUNT_RECOMPUTATION','scope':scope,
            'source_mode':source_mode,'pdf_reports_reextracted':source_checks if source_mode=='original_pdf' else 0,
            'saved_page_text_reports_checked':source_checks,'full_original_pdf_bytes_verified':source_mode=='original_pdf','eps_facts_checked':fact_checks,'company_month_rows_checked':len(company),
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--root',type=Path,default=Path(__file__).resolve().parents[1]);ap.add_argument('--scope',default='financial',choices=['financial','csi']);ap.add_argument('--output',type=Path);ap.add_argument('--source-mode',choices=['original_pdf','saved_page_text'],default='original_pdf');ap.add_argument('--fact-version',default='v1_1');ap.add_argument('--policy-version',default='v1');a=ap.parse_args()
    result=verify(a.root,a.scope,a.source_mode,a.fact_version,a.policy_version)
    if a.output:
