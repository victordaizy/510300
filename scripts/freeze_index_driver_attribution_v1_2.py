...
PARENT_MANIFEST_FILE = ROOT / "config" / "index_driver_attribution_v1_1_manifest.json"
FROZEN_FILES = (
    "docs/INDEX_DRIVER_ATTRIBUTION_V1_2_ADDENDUM.md",
    "config/index_driver_attribution_v1_2.yaml",
    "research/index_driver_attribution_v1_2.py",
    "scripts/build_index_driver_attribution_v1_2.py",
    "scripts/freeze_index_driver_attribution_v1_2.py",
    "tests/test_index_driver_attribution_v1_2.py",
)
...
def main() -> int:
    if not PARENT_MANIFEST_FILE.exists():
        raise FileNotFoundError("父版本 V1.1 冻结清单缺失")
    missing = [relative for relative in FROZEN_FILES if not (ROOT / relative).exists()]
    if missing:
        raise FileNotFoundError(f"V1.2 冻结文件缺失：{missing}")
    ...
    if any(governance[key] for key in forbidden_true):
        raise ValueError("V1.2 治理状态允许了禁止能力")
    industry_settings = contract["inputs"]["industry_intervals"]
    if industry_settings["fallback_taxonomy"] is not None:
        raise ValueError("V1.2 禁止行业分类回退补洞")
    if industry_settings["same_latest_in_date_conflict_rule"] != "硬失败":
        raise ValueError("V1.2 同一最新生效日多行业必须硬失败")
    ...
    if missing_inputs:
        raise FileNotFoundError(f"V1.2 冻结输入缺失：{missing_inputs}")
    ...
    if MANIFEST_FILE.exists():
        ...
        if _stable_payload(existing) != _stable_payload(payload):
            raise RuntimeError("归因 V1.2 冻结指纹已变化；禁止覆盖")
        print("归因 V1.2 冻结清单已存在，指纹一致。")
        return 0
