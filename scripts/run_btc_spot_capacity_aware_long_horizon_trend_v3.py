...
from research.btc_spot_capacity_aware_long_horizon_trend_v3 import (
    CONFIG, atomic_json, atomic_text, load_contract, run_visible,
)
from scripts.freeze_btc_spot_capacity_aware_long_horizon_trend_v3 import verify
...
def write_failure_report(
    ...
    report = {
        ...
        "decision": {
            ...
            "next_step": "保留V3失败并关闭复验；不得修改30日窗口、容量、产品、信号、波动目标或成本救回",
        },
    ...
    atomic_text(
        ...
        "\n".join(
            [
                "# BTC现货容量感知长周期趋势V3可见期失败", "",
                "状态：`FAILED_DATA_OR_EXECUTION_CONTRACT`", "",
                f"错误类型：`{type(exc).__name__}`", "", f"错误：{exc}", "",
