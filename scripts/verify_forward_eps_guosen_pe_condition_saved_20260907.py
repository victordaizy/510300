...
from research.forward_eps_guosen_pe_condition_policy_v1 import MODELS, CONTROLS, OUT, SOURCE, DIAGNOSTIC, PARENT, CONFIG, QUALIFIED_PE
...
def main():
    ...
    feature_months = pd.read_parquet(SOURCE / "monthly_forward_eps_features.parquet")
    company = pd.read_parquet(DIAGNOSTIC / "saved_company_pe_usage.parquet")
    old_company = pd.read_parquet(SOURCE / "company_month_feature_evidence.parquet")
    pd.testing.assert_frame_equal(company[old_company.columns], old_company)
    pd.testing.assert_frame_equal(monthly[feature_months.columns], feature_months)
    assert not company.duplicated(["origin", "ts_code"]).any()
    ...
    for origin, group in company.groupby("origin"):
        ...
        values = group.loc[group.original_pe_finite & group.report_relation_eligible, "reported_earnings_yield"]
        assert current.qualified_pe_company_count == len(values)
        equal_value(current[QUALIFIED_PE], values.median())
    pd.testing.assert_series_equal(monthly[QUALIFIED_PE], condition.qualified_pe_inverse_median)
    old_label_frame = pd.read_parquet(SOURCE / "monthly_features_and_mature_labels.parquet")
    for column in ["origin", "origin_index", "all_features_valid", "Y60", "label_exit_date"]:
        pd.testing.assert_series_equal(monthly[column], old_label_frame[column])
    ...
    for receipt in read(OUT / "training_receipts.json")["rows"]:
        before = old_training[receipt["origin"]]
        assert receipt["training_months"] == before["training_months"]
        assert receipt["status"] == before["status"]
    ...
    save(OUT / "saved_numerical_verification.json", {
        "checked_at": now(), "status": "PASS_SAVED_GUOSEN_PE_CONDITION_SAME_TRAINING_AND_COMPLETE_ACCOUNTS",
        "original_company_month_rows_checked": len(company), "monthly_feature_rows_checked": len(monthly),
        "saved_labels_checked": labels, "saved_trained_models_checked": trained,
        "matched_training_calendars_identical": True, "original_E3_all_training_calendars_and_labels_identical": True, "valid_month_end_orders_recomputed_from_saved_account": order_checks,
    ...
    print("原国信PE条件、原EPS相同训练日历、保存模型和八条完整进出场账户核对通过。", flush=True)
