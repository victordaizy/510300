"""零付费来源 V1.3：沿用原保存与去重语义，支持已批准的数据 Junction。"""
...
def _relative(path: Path, root: Path) -> str:
    if root.absolute() != PATHS.root.absolute():
        raise ValueError("保存路径的项目身份与目录合同不一致")
...
def write_content_addressed_raw(
    ...
    *,
    ...
    target_directory = PATHS.checked(raw_root / kind / observed_at.strftime("%Y%m%d"))
    target_directory.mkdir(parents=True, exist_ok=True)
    target = PATHS.checked(target_directory / f"{digest}.json")
...
def persist_record_atomic(
    ...
    output_file = PATHS.checked(output_file)
