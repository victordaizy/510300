"""复算已保存三项持仓模型、实际持仓输入和账户经济，不重新拟合或模拟。"""
...
from research.position_state_exit_v1 import ROOT, OUT, CONFIG, PRIMARY, P32
from research.position_state_exit_inputs_v1 import FEATURES
...
def source_and_training(cfg, data, models, originals):
    ...
    for record, original in zip(models, originals, strict=True):
        ...
        if usable:
            ...
            if record["status"] == "FIT_COMPLETE":
                ...
                mean = (weight[:, None] * x).sum(axis=0) / weight.sum()
                scale = np.sqrt((weight[:, None] * (x - mean) ** 2).sum(axis=0) / weight.sum())
                scale = np.where(scale > 1e-12, scale, 1.)
                np.testing.assert_allclose(mean, record["model"]["mean"], atol=1e-13, rtol=0)
                np.testing.assert_allclose(scale, record["model"]["scale"], atol=1e-13, rtol=0)
                require(record["model"]["features"] == FEATURES and len(record["model"]["coefficients"]) == 3, "实际拟合不是固定三项模型")
                design = np.clip((x - mean) / scale, -cfg["feature_clip"], cfg["feature_clip"])
                coefficients = np.asarray(record["model"]["coefficients"])
                residual = design @ coefficients + record["model"]["intercept"] - rows.target.to_numpy(float)
                gradient = design.T @ (weight * residual) + cfg["ridge_alpha"] * coefficients
                error = max(float(np.max(np.abs(gradient))), abs(float(weight @ residual)))
                require(error < 1e-9, "保存三项系数不满足原加权岭目标的一阶条件")
    ...
    return inputs, {"state_rows": len(data), "model_updates": len(checks), "fit_memberships": len(memberships), "completed_models": sum(m["status"] == "FIT_COMPLETE" for m in models)}
...
def main():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "三项持仓冻结内容改变")
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            for row in decisions.itertuples():
                ...
                if shares:
                    ...
                    np.testing.assert_allclose(values[:3], [getattr(row, name) for name in FEATURES], atol=2e-12, rtol=0)
                    ...
                    if stored and stored["status"] == "FIT_COMPLETE":
                        ...
                        estimate = linear_value(stored["model"], values[:3])
                        old_estimate = linear_value(originals[pos]["model"], values)
                        require(abs(estimate - row.continuation_prediction) < 1e-10, "持仓条件三项持仓无法复算")
                        ...
                        predictions.append({"period": period, "cost": cost_id, "origin": row.origin, "cycle_id": int(row.learning_cycle_id),
                            "position_state_prediction": estimate, "original_mean_prediction_on_same_actual_state": old_estimate,
                            "difference": estimate - old_estimate, "three_state_negative_original_nonnegative": estimate < 0 <= old_estimate,
                            "three_state_nonnegative_original_negative": old_estimate < 0 <= estimate,
    ...
    receipt = {"verified_at": now(), "status": "SAVED_POSITION_STATE_MODELS_AND_FULL_ACCOUNT_ECONOMICS_RECONCILED", "source_and_training": source,
