...
RECEIPT_FILE = (
    ...
    / "reports"
    / "frozen"
    / "510300_etf_microstructure_dual_shadow_reverse_validation_v1_freeze_receipt.json"
)
...
def main() -> int:
    if MANIFEST_FILE.exists() or RECEIPT_FILE.exists():
        raise FileExistsError("反向验证冻结清单或收据已存在，拒绝重复冻结")
    config = yaml.safe_load(CONFIG_FILE.read_text(encoding="utf-8"))
    if config["protocol"]["project_id"] != PROJECT_ID:
        raise ValueError("反向验证配置项目编号不匹配")
    tracked_relatives = [
        ...
        "scripts/freeze_510300_etf_microstructure_dual_shadow_reverse_validation_v1.py",
    ...
    manifest = {
        ...
        "state": "FROZEN_BEFORE_EXTERNAL_RESULT_COMPUTATION",
