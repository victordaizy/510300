...
from scripts.freeze_multi_asset_annual_excess_40pct_high_sharpe_v8 import (
...
CONFIG = ROOT / "config" / "multi_asset_annual_excess_40pct_high_sharpe_v9.yaml"
MANIFEST = ROOT / "config" / "multi_asset_annual_excess_40pct_high_sharpe_v9_manifest.json"
TRACKED_FILES = [
    "config/multi_asset_annual_excess_40pct_high_sharpe_v9.yaml",
    "docs/MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V9_SPEC.md",
    "scripts/freeze_multi_asset_annual_excess_40pct_high_sharpe_v9.py",
]
...
def load_contract() -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V9扩展协议必须是YAML对象")
    protocol = payload.get("protocol", {})
    objective = payload.get("objective", {})
    lane = payload.get("new_lane", {})
    safety = payload.get("safety", {})
    failures: list[str] = []
    if protocol.get("protocol_id") != "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V9":
        failures.append("protocol_id")
    if protocol.get("parent_protocol_id") != "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V8":
    ...
    if lane.get("selection_formula_unchanged_from_v2") is not True:
        failures.append("selection_formula")
    if float(lane.get("target_weight_ceiling_per_selected_asset", float("nan"))) != 0.50:
        failures.append("weight_ceiling")
    if float(lane.get("maximum_order_fraction_of_prior_median_dollar_turnover", float("nan"))) != 0.0005:
        failures.append("capacity_fraction")
    if lane.get("capacity_exceeding_order_policy") != "SLICE_WITHIN_CAPACITY_NO_CAPACITY_RELAXATION":
        failures.append("slicing")
    ...
    if failures:
        raise ValueError(f"V9扩展协议被弱化或损坏：{sorted(set(failures))}")
    return payload
...
def tracked_hashes() -> dict[str, str]:
    ...
    if missing:
        raise FileNotFoundError(f"缺少V9冻结文件：{missing}")
...
def verify_v2_failure(contract: dict[str, Any]) -> dict[str, Any]:
    path = ROOT / contract["inputs"]["v2_failure_report"]
    ...
    if not all(checks.values()):
        raise RuntimeError(f"V2失败报告不符合V9登记条件：{checks}")
    return {
        "checks": checks,
...
def freeze() -> dict[str, Any]:
    ...
    if parent["failure_count"] or v2["failure_count"]:
        raise RuntimeError("V9父协议或V2冻结清单验证失败")
    failure = verify_v2_failure(contract)
    files = tracked_hashes()
    manifest = {
        "schema_version": "1.0.0",
        "manifest_id": "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V9_MANIFEST",
    ...
    if MANIFEST.exists():
        ...
        for field in (
            ...
            if existing.get(field) != manifest.get(field):
                raise RuntimeError("现有V9清单与当前内容不同；不得覆盖")
        return existing
...
def verify() -> dict[str, Any]:
    if not MANIFEST.is_file():
        return {
            "status": "FAIL_MISSING_MULTI_ASSET_40PCT_HIGH_SHARPE_V9_MANIFEST",
            "failure_count": 1,
    ...
    if v2["failure_count"]:
        failures.append("v2_manifest")
    return {
        "status": (
            "PASS_MULTI_ASSET_40PCT_HIGH_SHARPE_V9_MANIFEST_VERIFIED"
            if not failures
            else "FAIL_MULTI_ASSET_40PCT_HIGH_SHARPE_V9_MANIFEST"
        ),
        ...
        "v2_manifest_status": v2["status"],
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="冻结或验证40个百分点高夏普V9扩展协议")
