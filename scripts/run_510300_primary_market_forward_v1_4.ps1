...
$collector = if ($defaultCollector) {
    Join-Path $projectRoot "scripts\collect_510300_primary_market_v1_4.py"
}
else {
...
if ($defaultCollector) {
    $collectorArguments += @("-m", "scripts.collect_510300_primary_market_v1_4")
}
else {
...
Write-Output "PCF/IOPV V1.4 目录兼容研究采集与成熟度分析完成；未生成信号、仓位映射或订单。"
exit 0
