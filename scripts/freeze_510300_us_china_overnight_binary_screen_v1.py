"""冻结510300美国中国ETF隔夜信息二元筛选的协议、程序、测试与输入。"""
...
SOURCE_PATHS = [
    ...
    ROOT / "scripts" / "download_us_china_overnight_inputs_v1.py",
    ROOT / "scripts" / "freeze_510300_us_china_overnight_binary_screen_v1.py",
    ROOT / "tests" / "test_us_china_overnight_binary_screen_v1.py",
]
...
def main() -> int:
    ...
    if protocol["study_id"] != "510300_US_CHINA_OVERNIGHT_BINARY_SCREEN_V1":
    ...
    if candidate_hash != EXPECTED_CANDIDATE_SHA256:
        raise ValueError("数据下载前固定的候选合同发生漂移")
    ...
    result_paths = [
        ROOT / config["artifacts"]["features"],
    ...
    manifest = {
        ...
        "same_calendar_day_us_bar_forbidden": bool(
            contract["information_clock"]["same_calendar_day_us_bar_forbidden"]
        ),
        ...
        "post_run_threshold_sign_clock_or_window_rescue": "FORBIDDEN",
