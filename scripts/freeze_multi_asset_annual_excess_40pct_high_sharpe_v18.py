...
from scripts.freeze_multi_asset_annual_excess_40pct_high_sharpe_v17 import (
    verify as verify_parent_manifest,
)


CONFIG = ROOT / "config" / "multi_asset_annual_excess_40pct_high_sharpe_v18.yaml"
MANIFEST = ROOT / "config" / "multi_asset_annual_excess_40pct_high_sharpe_v18_manifest.json"
TRACKED_FILES = [
    "config/multi_asset_annual_excess_40pct_high_sharpe_v18.yaml",
    "docs/MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V18_SPEC.md",
    "scripts/freeze_multi_asset_annual_excess_40pct_high_sharpe_v18.py",
]
...
def load_contract() -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V18扩展协议必须是YAML对象")
    objective = payload.get("objective", {})
    lane = payload.get("new_lane", {})
    failures: list[str] = []
    if payload.get("protocol", {}).get("protocol_id") != "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V18":
        failures.append("protocol_id")
    if payload.get("protocol", {}).get("parent_protocol_id") != "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V17":
    ...
    if float(lane.get("maximum_total_gross_exposure", float("nan"))) != 2.0:
    ...
    if failures:
        raise ValueError(f"V18扩展协议被弱化或损坏：{sorted(set(failures))}")
    return payload
...
def tracked_hashes() -> dict[str, str]:
    ...
    if missing:
        raise FileNotFoundError(f"缺少V18冻结文件：{missing}")
...
def verify_v7r_failure(contract: dict[str, Any]) -> dict[str, Any]:
    manifest_path = ROOT / contract["inputs"]["v7r_manifest"]
    report_path = ROOT / contract["inputs"]["v7r_visible_report"]
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    report = json.loads(report_path.read_text(encoding="utf-8"))
    required = contract["registered_v7r_failure"]
    checks = {
    ...
    if not all(checks.values()):
        raise RuntimeError(f"V7R失败不符合V18登记条件：{checks}")
    return {
        "checks": checks,
...
def freeze() -> dict[str, Any]:
    ...
    if parent["failure_count"]:
        raise RuntimeError("V18父协议冻结清单验证失败")
    failure = verify_v7r_failure(contract)
    files = tracked_hashes()
    manifest = {
        "schema_version": "1.0.0",
        "manifest_id": "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V18_MANIFEST",
    ...
    if MANIFEST.exists():
        ...
        for field in (
            ...
            "v7r_failure", "objective", "new_lane",
        ):
            if existing.get(field) != manifest.get(field):
                raise RuntimeError("现有V18清单与当前内容不同；不得覆盖")
        return existing
...
def verify() -> dict[str, Any]:
    if not MANIFEST.is_file():
        return {
            "status": "FAIL_MISSING_MULTI_ASSET_40PCT_HIGH_SHARPE_V18_MANIFEST",
            "failure_count": 1,
    ...
    try:
        if existing.get("v7r_failure") != verify_v7r_failure(contract):
            failures.append("v7r_failure")
    except Exception:
        failures.append("v7r_failure")
    return {
        "status": (
            "PASS_MULTI_ASSET_40PCT_HIGH_SHARPE_V18_MANIFEST_VERIFIED"
            if not failures else "FAIL_MULTI_ASSET_40PCT_HIGH_SHARPE_V18_MANIFEST"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="冻结或验证40个百分点高夏普V18扩展协议")
