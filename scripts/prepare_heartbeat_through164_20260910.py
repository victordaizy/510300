...
def main():
    ...
    require(index['latest_completed_round']['round'] == 164 and not index['goal_achieved'] and not index['running_studies'], '164完成状态不同')
    previous = (ROOT/'docs/510300_RESEARCH_HEARTBEAT_THROUGH163_20260910.md').read_text(encoding='utf-8')
    authority = previous.split('\n\n本回合为PROGRESS：', 1)[0]
    best = previous.split('\n\n均衡比较候选仍143', 1)[1].split('\n\n163 study', 1)[0].replace('最后比较163', '最后比较164')
...
'''
    prompt = prompt.strip()
    with (ROOT/'docs/510300_RESEARCH_HEARTBEAT_THROUGH164_20260910.md').open('x', encoding='utf-8') as stream:
        stream.write(prompt)
    ...
    write_json(ROOT/'reports/research/510300_heartbeat_update_through164_args.json', args, exclusive=True)
    print(json.dumps({'字数': len(prompt), '原状态': saved['status'], '完成轮次': 164}, ensure_ascii=False))
