...
from scripts.freeze_multi_asset_annual_excess_40pct_high_sharpe_v14 import (
    verify as verify_parent_manifest,
)


CONFIG = ROOT / "config" / "multi_asset_annual_excess_40pct_high_sharpe_v15.yaml"
MANIFEST = ROOT / "config" / "multi_asset_annual_excess_40pct_high_sharpe_v15_manifest.json"
TRACKED_FILES = [
    "config/multi_asset_annual_excess_40pct_high_sharpe_v15.yaml",
    "docs/MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V15_SPEC.md",
    "scripts/freeze_multi_asset_annual_excess_40pct_high_sharpe_v15.py",
]
...
def load_contract() -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V15扩展协议必须是YAML对象")
    objective = payload.get("objective", {})
    lane = payload.get("new_lane", {})
    failures: list[str] = []
    if payload.get("protocol", {}).get("protocol_id") != "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V15":
        failures.append("protocol_id")
    if payload.get("protocol", {}).get("parent_protocol_id") != "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V14":
    ...
    if lane.get("momentum_horizons_calendar_days") != [20, 60, 120, 240]:
        failures.append("horizons")
    if int(lane.get("minimum_positive_votes_for_eligibility", 0)) != 3:
        failures.append("vote_threshold")
    if lane.get("allocation_rule") != "EQUAL_WEIGHT_AMONG_ELIGIBLE_PRODUCTS":
        failures.append("allocation_rule")
    ...
    if failures:
        raise ValueError(f"V15扩展协议被弱化或损坏：{sorted(set(failures))}")
    return payload
...
def tracked_hashes() -> dict[str, str]:
    ...
    if missing:
        raise FileNotFoundError(f"缺少V15冻结文件：{missing}")
...
def verify_v5_rejection(contract: dict[str, Any]) -> dict[str, Any]:
    manifest_path = ROOT / contract["inputs"]["v5_manifest"]
    report_path = ROOT / contract["inputs"]["v5_visible_report"]
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    report = json.loads(report_path.read_text(encoding="utf-8"))
    required = contract["registered_v5_rejection"]
    checks = {
    ...
    if not all(checks.values()):
        raise RuntimeError(f"相对强度V5拒绝报告不符合V15登记条件：{checks}")
...
def freeze() -> dict[str, Any]:
    ...
    if parent["failure_count"]:
        raise RuntimeError("V15父协议冻结清单验证失败")
    rejection = verify_v5_rejection(contract)
    files = tracked_hashes()
    manifest = {
        "schema_version": "1.0.0",
        "manifest_id": "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V15_MANIFEST",
    ...
    if MANIFEST.exists():
        ...
        for field in (
            ...
            "v5_rejection", "objective", "new_lane",
        ):
            if existing.get(field) != manifest.get(field):
                raise RuntimeError("现有V15清单与当前内容不同；不得覆盖")
        return existing
...
def verify() -> dict[str, Any]:
    if not MANIFEST.is_file():
        return {
            "status": "FAIL_MISSING_MULTI_ASSET_40PCT_HIGH_SHARPE_V15_MANIFEST",
            "failure_count": 1,
    ...
    try:
        if existing.get("v5_rejection") != verify_v5_rejection(contract):
            failures.append("v5_rejection")
    except Exception:
        failures.append("v5_rejection")
    return {
        "status": (
            "PASS_MULTI_ASSET_40PCT_HIGH_SHARPE_V15_MANIFEST_VERIFIED"
            if not failures else "FAIL_MULTI_ASSET_40PCT_HIGH_SHARPE_V15_MANIFEST"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="冻结或验证40个百分点高夏普V15扩展协议")
