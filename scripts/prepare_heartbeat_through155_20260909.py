"""依据155完成索引生成持续任务更新参数，保持原排程与目标任务。"""
import json
import tomllib
...
def main():
    ...
    require(index["latest_completed_round"]["round"] == 155 and not index["goal_achieved"] and not index["running_studies"], "155完成状态不同")
    prompt = (ROOT / "docs/510300_RESEARCH_HEARTBEAT_THROUGH155_20260909.md").read_text(encoding="utf-8").rstrip()
    ...
    target = ROOT / "reports/research/510300_heartbeat_update_through155_args.json"
