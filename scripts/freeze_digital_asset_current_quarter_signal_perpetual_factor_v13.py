...
from scripts.freeze_digital_asset_cross_sectional_perpetual_basis_v9 import (
    verify as verify_v9_manifest,
)
...
MANIFEST = (
    ROOT
    / "config"
    / "digital_asset_current_quarter_signal_perpetual_factor_v13_manifest.json"
)
TRACKED_FILES = [
    "config/digital_asset_current_quarter_signal_perpetual_factor_v13.yaml",
    "docs/DIGITAL_ASSET_CURRENT_QUARTER_SIGNAL_PERPETUAL_FACTOR_V13_SPEC.md",
    "research/digital_asset_current_quarter_signal_perpetual_factor_v13.py",
    "scripts/freeze_digital_asset_current_quarter_signal_perpetual_factor_v13.py",
    "scripts/run_digital_asset_current_quarter_signal_perpetual_factor_v13.py",
    "tests/test_digital_asset_current_quarter_signal_perpetual_factor_v13.py",
]
VISIBLE_SOURCE_DEPENDENCIES = [
    "scripts/collect_digital_asset_current_quarter_signal_perpetual_v13_inputs.py",
    "data/raw/digital_asset_current_quarter_signal_perpetual_v13/collection_status.json",
    "data/raw/digital_asset_current_quarter_signal_perpetual_v13/perpetual_1h_2021_2026.parquet",
    "data/raw/digital_asset_current_quarter_signal_perpetual_v13/funding_rates_2021_2026.parquet",
    ...
    "config/multi_asset_annual_excess_40pct_high_sharpe_v23_manifest.json",
    ...
    "config/digital_asset_cross_sectional_perpetual_basis_v9_manifest.json",
...
def file_record(relative: str) -> dict[str, Any]:
    path = ROOT / relative
    if not path.is_file():
        raise FileNotFoundError(f"缺少V13冻结文件：{relative}")
    return {
        "path": relative,
...
def prefreeze_data_coverage(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    hourly = coverage.get("perpetual_hourly", {})
    hourly_by_symbol = hourly.get("by_symbol", {})
    funding = coverage.get("funding", {})
    ...
    checks = {
        ...
        "hourly_expected_rows": hourly.get("expected_hourly_rows_per_symbol")
        == int(gate["required_expected_hourly_rows_per_symbol"]),
        "funding_expected_rows": funding.get("expected_funding_rows_per_symbol")
        == int(gate["required_funding_rows_per_symbol"]),
        "hourly_rows": all(
            hourly_by_symbol.get(symbol, {}).get("row_count")
            == int(gate["required_perpetual_hourly_rows_per_symbol"])
            for symbol in symbols
        ),
        "funding_rows": all(
            funding_by_symbol.get(symbol, {}).get("row_count")
            == int(gate["required_funding_rows_per_symbol"])
            for symbol in symbols
        ),
        "hourly_exact_grid": all(
            hourly_by_symbol.get(symbol, {}).get("exact_expected_grid") is True
            and hourly_by_symbol.get(symbol, {}).get("missing_hour_count") == 0
            and hourly_by_symbol.get(symbol, {}).get("extra_hour_count") == 0
            and hourly_by_symbol.get(symbol, {}).get("duplicate_time_count") == 0
            for symbol in symbols
        ),
        "funding_exact_grid": all(
            funding_by_symbol.get(symbol, {}).get("exact_expected_grid") is True
            and funding_by_symbol.get(symbol, {}).get("missing_settlement_count") == 0
            and funding_by_symbol.get(symbol, {}).get("extra_settlement_count") == 0
            and funding_by_symbol.get(symbol, {}).get("duplicate_time_count") == 0
            and funding_by_symbol.get(symbol, {}).get(
                "maximum_absolute_time_offset_milliseconds", 1001.0
            )
            <= 1000.0
            for symbol in symbols
        ),
        "hourly_endpoints": all(
            hourly_by_symbol.get(symbol, {}).get("first_time")
            == _iso_timestamp(gate["required_grid_start"])
            and hourly_by_symbol.get(symbol, {}).get("last_time")
            == _iso_timestamp(gate["required_grid_end"])
            for symbol in symbols
        ),
        "funding_endpoints": all(
            funding_by_symbol.get(symbol, {}).get("first_time")
            == _iso_timestamp(gate["required_grid_start"])
            and funding_by_symbol.get(symbol, {}).get("last_time")
            == _iso_timestamp(gate["required_funding_end"])
            for symbol in symbols
        ),
        "v11_reuse_verified": coverage.get("reused_v11_status")
        == "PASS_INPUT_ACQUISITION_ONLY",
        ...
        "post_2023_opened_after_parent_freeze": status.get(
            "post_2023_values_read_after_v23_lane_freeze"
        )
        is True,
    }
    output_mapping = {
    ...
    return {
        ...
        "expected_funding_rows_per_symbol": int(
            gate["required_funding_rows_per_symbol"]
        ),
...
def _assert_outputs_absent(contract: dict[str, Any]) -> None:
    ...
    if existing:
        raise RuntimeError(f"冻结前已存在V13结果：{existing}")


def _dependencies() -> dict[str, dict[str, Any]]:
    return {
        "parent": verify_parent_manifest(),
        "v11_signal_engine": verify_v11_manifest(),
        "v9_perpetual_engine": verify_v9_manifest(),
...
def freeze() -> dict[str, Any]:
    ...
    if MANIFEST.exists():
        ...
        if verification["failure_count"]:
            raise RuntimeError(f"现有V13清单验证失败：{verification['failures']}")
        return json.loads(MANIFEST.read_text(encoding="utf-8"))
    _assert_outputs_absent(contract)
    dependencies = _dependencies()
    if any(item["failure_count"] for item in dependencies.values()):
        raise RuntimeError("V13依赖冻结清单验证失败")
    coverage = prefreeze_data_coverage(contract)
    if not coverage["all_checks_pass"]:
        raise RuntimeError(f"V13冻结前覆盖门失败：{coverage['checks']}")
    files = current_tracked_hashes()
    manifest = {
        "schema_version": "1.0.0",
        "manifest_id": "DIGITAL_ASSET_CURRENT_QUARTER_SIGNAL_PERPETUAL_FACTOR_V13_MANIFEST",
        ...
        "base_total_perpetual_cost_bps_per_leg": float(
            contract["costs"]["base_total_perpetual_cost_bps_per_leg"]
        ),
        "stress_total_perpetual_cost_bps_per_leg": float(
            contract["costs"]["stress_total_perpetual_cost_bps_per_leg"]
        ),
...
def verify() -> dict[str, Any]:
    if not MANIFEST.is_file():
        return {
            "status": "FAIL_MISSING_CURRENT_QUARTER_SIGNAL_PERPETUAL_FACTOR_V13_MANIFEST",
            "failure_count": 1,
    ...
    return {
        "status": (
            "PASS_CURRENT_QUARTER_SIGNAL_PERPETUAL_FACTOR_V13_MANIFEST_VERIFIED"
            if not failures
            else "FAIL_CURRENT_QUARTER_SIGNAL_PERPETUAL_FACTOR_V13_MANIFEST"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="冻结或验证V13")
