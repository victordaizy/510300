"""保存简洁且准确的当前研究接续，减少下一回合重复阅读和旧状态干扰。"""
import json
...
def main():
    path = ROOT/'reports/research/510300_heartbeat_update_through192_args.json'
    ...
    require(index['latest_completed_round']['round']==192 and not index['running_studies'], '本轮收尾状态不同')
    ...
    require('第193轮专用接续' not in args['prompt'], '本轮参数已经完善')
    lines = [
    ...
    for period, key in [('主历史', 'all_metrics'), ('较早历史', 'earlier_diagnostics')]:
        for row in result[key]:
            if row['model'] in ['HOLDING_MARKET_COUPLING_EXIT', 'ACCOUNT_VOLATILITY_EXPOSURE']:
                lines.append(f"{period}、{row['model']}、{row['cost']}：净夏普{row['net_sharpe']}，复合年化{row['annualized_return']:.10%}，最大回撤{row['max_drawdown']:.10%}。")
    lines += [
        f"scripts/verify_round192_20260913.py已完成核对，回执时间{checked['verified_at']}，25组正规方程最大误差{checked['maximum_normal_equation_error']}，5646个决定、1220个持仓状态、812个有效预测、58周期、116成交；这是实现核对，不是独立绩效验证。全部模型中文数值在结果目录全部已拟合模型系数.md，也已加入deliverables/510300持仓与市场联合作用_第192轮_20260913/持仓与市场联合作用_结果及全部中文规则.md。",
    ...
    (ROOT/'docs/510300_RESEARCH_HEARTBEAT_THROUGH192_20260913.md').write_text(args['prompt'], encoding='utf-8')
    print('第192轮实际结果及第193轮专用接续已保存。')
