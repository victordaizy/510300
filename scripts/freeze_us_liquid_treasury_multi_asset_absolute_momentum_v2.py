...
MANIFEST = ROOT / "config" / "us_liquid_treasury_multi_asset_absolute_momentum_v2_manifest.json"
TRACKED_FILES = [
    "config/us_liquid_treasury_multi_asset_absolute_momentum_v2.yaml",
    "docs/US_LIQUID_TREASURY_MULTI_ASSET_ABSOLUTE_MOMENTUM_V2_SPEC.md",
    "research/us_liquid_treasury_multi_asset_absolute_momentum_v2.py",
    "scripts/download_us_liquid_treasury_multi_asset_history_v2.py",
    "scripts/freeze_us_liquid_treasury_multi_asset_absolute_momentum_v2.py",
    "scripts/run_us_liquid_treasury_multi_asset_absolute_momentum_v2.py",
    "tests/test_us_liquid_treasury_multi_asset_absolute_momentum_v2.py",
]
VISIBLE_SOURCE_DEPENDENCIES = [
    ...
    "config/multi_asset_annual_excess_40pct_high_sharpe_v8_manifest.json",
    "config/us_leveraged_multi_asset_absolute_momentum_v1_manifest.json",
    "config/us_leveraged_multi_asset_absolute_momentum_v1r_fx_source_correction_manifest.json",
...
def file_record(relative: str) -> dict[str, Any]:
    path = ROOT / relative
    if not path.is_file():
        raise FileNotFoundError(f"缺少V2冻结文件：{relative}")
    return {
        "path": relative,
...
def prefreeze_signal_coverage(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": (
            "PASS_PREFREEZE_US_LIQUID_TREASURY_SIGNAL_COVERAGE"
            if all(checks.values())
            else "FAIL_PREFREEZE_US_LIQUID_TREASURY_SIGNAL_COVERAGE"
        ),
        "checks": checks,
...
def _assert_visible_outputs_absent(contract: dict[str, Any]) -> None:
    ...
    if existing:
        raise RuntimeError(f"冻结前已存在V2可见结果：{existing}")
...
def freeze() -> dict[str, Any]:
    ...
    if MANIFEST.exists():
        ...
        if verification["failure_count"]:
            raise RuntimeError(f"现有V2清单验证失败：{verification['failures']}")
    ...
    if any(item["failure_count"] for item in dependencies.values()):
        raise RuntimeError("V2父协议、继承候选或保守评价器冻结清单验证失败")
    ...
    if not coverage["all_checks_pass"]:
        raise RuntimeError(f"V2冻结前信号覆盖门失败：{coverage['checks']}")
    manifest = {
        "schema_version": "1.0.0",
        "manifest_id": "US_LIQUID_TREASURY_MULTI_ASSET_ABSOLUTE_MOMENTUM_V2_MANIFEST",
...
def verify() -> dict[str, Any]:
    if not MANIFEST.is_file():
        return {
            "status": "FAIL_MISSING_US_LIQUID_TREASURY_V2_MANIFEST",
            "failure_count": 1,
    ...
    return {
        "status": (
            "PASS_US_LIQUID_TREASURY_V2_MANIFEST_VERIFIED"
            if not failures
            else "FAIL_US_LIQUID_TREASURY_V2_MANIFEST"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="冻结或验证美国高流动性美债多资产V2")
