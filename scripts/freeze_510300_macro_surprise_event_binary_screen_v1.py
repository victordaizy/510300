"""冻结510300宏观预期意外事件二元筛选的协议、程序、测试与输入。"""
...
SOURCE_PATHS = [
    ROOT / "research" / "macro_surprise_event_binary_screen_v1.py",
    ROOT / "research" / "us_china_overnight_binary_screen_v1.py",
    ROOT / "research" / "intraday_binary_livermore_screen_v1.py",
    ROOT / "scripts" / "download_macro_surprise_event_inputs_v1.py",
    ROOT / "scripts" / "freeze_510300_macro_surprise_event_binary_screen_v1.py",
    ROOT / "tests" / "test_macro_surprise_event_binary_screen_v1.py",
]
...
def main() -> int:
    ...
    if protocol["study_id"] != "510300_MACRO_SURPRISE_EVENT_BINARY_SCREEN_V1":
    ...
    if candidate_hash != EXPECTED_CANDIDATE_SHA256:
        raise ValueError("事件下载前固定的候选合同发生漂移")
    ...
    result_paths = [
        ROOT / config["artifacts"]["event_features"],
        ROOT / config["artifacts"]["daily_event_features"],
    ...
    manifest = {
        ...
        "series_ids": [
            item["series_id"] for item in contract["event_contract"]["series"]
        ],
        "candidate_ids": candidate_ids,
        "candidate_count": len(candidate_ids),
        "allowed_target_states": list(contract["scope"]["allowed_target_states"]),
        "same_release_date_execution_forbidden": bool(
            contract["timing"]["same_release_date_execution_forbidden"]
        ),
        "cost_scenarios": list(contract["evaluation"]["cost_scenarios"]),
        "start_offsets": list(
            contract["evaluation"]["start_date_perturbations_trading_days"]
        ),
        "period_ids": [period["id"] for period in contract["evaluation"]["periods"]],
        "hard_gates": {
        ...
        "post_run_series_sign_scale_or_hold_rescue": "FORBIDDEN",
