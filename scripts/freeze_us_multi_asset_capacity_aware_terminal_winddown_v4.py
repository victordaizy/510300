...
MANIFEST = ROOT / "config" / "us_multi_asset_capacity_aware_terminal_winddown_v4_manifest.json"
TRACKED_FILES = [
    "config/us_multi_asset_capacity_aware_terminal_winddown_v4.yaml",
    "docs/US_MULTI_ASSET_CAPACITY_AWARE_TERMINAL_WINDDOWN_V4_SPEC.md",
    "research/us_multi_asset_capacity_aware_terminal_winddown_v4.py",
    "scripts/freeze_us_multi_asset_capacity_aware_terminal_winddown_v4.py",
    "scripts/run_us_multi_asset_capacity_aware_terminal_winddown_v4.py",
    "tests/test_us_multi_asset_capacity_aware_terminal_winddown_v4.py",
]
VISIBLE_SOURCE_DEPENDENCIES = [
    "data/raw/us_liquid_treasury_multi_asset_v2/collection_status.json",
    "data/raw/us_liquid_treasury_multi_asset_v2/product_master.parquet",
    "data/raw/us_liquid_treasury_multi_asset_v2/visible_panel_through_2019.parquet",
    ...
    "config/multi_asset_annual_excess_40pct_high_sharpe_v10_manifest.json",
    "config/us_multi_asset_capacity_aware_absolute_momentum_v3_manifest.json",
    "config/us_liquid_treasury_multi_asset_absolute_momentum_v2_manifest.json",
...
def file_record(relative: str) -> dict[str, Any]:
    path = ROOT / relative
    if not path.is_file():
        raise FileNotFoundError(f"缺少V4冻结文件：{relative}")
    return {
        "path": relative,
...
def prefreeze_signal_coverage(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": (
            "PASS_PREFREEZE_US_CAPACITY_AWARE_TERMINAL_WINDDOWN_SIGNAL_COVERAGE"
            if all(checks.values())
            else "FAIL_PREFREEZE_US_CAPACITY_AWARE_TERMINAL_WINDDOWN_SIGNAL_COVERAGE"
        ),
        "checks": checks,
...
def _assert_visible_outputs_absent(contract: dict[str, Any]) -> None:
    ...
    if existing:
        raise RuntimeError(f"冻结前已存在V4可见结果：{existing}")
...
def freeze() -> dict[str, Any]:
    ...
    if MANIFEST.exists():
        ...
        if verification["failure_count"]:
            raise RuntimeError(f"现有V4清单验证失败：{verification['failures']}")
    ...
    if any(item["failure_count"] for item in dependencies.values()):
        raise RuntimeError("V4父协议、V3或保守评价器冻结清单验证失败")
    ...
    if not coverage["all_checks_pass"]:
        raise RuntimeError(f"V4冻结前信号覆盖门失败：{coverage['checks']}")
    manifest = {
        "schema_version": "1.0.0",
        "manifest_id": "US_MULTI_ASSET_CAPACITY_AWARE_TERMINAL_WINDDOWN_V4_MANIFEST",
        ...
        "maximum_order_fraction": float(
            contract["universe"][
                "maximum_order_fraction_of_prior_median_dollar_turnover"
            ]
        ),
        "terminal_winddown_common_trading_days": int(
            contract["portfolio"]["terminal_winddown_common_trading_days"]
        ),
...
def verify() -> dict[str, Any]:
    if not MANIFEST.is_file():
        return {
            "status": "FAIL_MISSING_US_CAPACITY_AWARE_TERMINAL_WINDDOWN_V4_MANIFEST",
            "failure_count": 1,
    ...
    if existing.get("terminal_winddown_common_trading_days") != 20:
        failures.append("terminal_winddown_common_trading_days")
    ...
    return {
        "status": (
            "PASS_US_CAPACITY_AWARE_TERMINAL_WINDDOWN_V4_MANIFEST_VERIFIED"
            if not failures
            else "FAIL_US_CAPACITY_AWARE_TERMINAL_WINDDOWN_V4_MANIFEST"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="冻结或验证美国多资产容量感知期末减仓V4")
