...
$manifest = Join-Path $projectRoot "config\priority_forward_research_operations_v1_11_manifest.json"
...
try {
    $verification = & $python -m scripts.run_priority_forward_codex_automation_v1_11 --verify-only --expected-manifest-sha256 $ExpectedManifestSha256
...
$arguments = "-m scripts.run_priority_forward_codex_automation_v1_11 --phase morning --config `"$config`" --manifest `"$manifest`" --expected-manifest-sha256 $ExpectedManifestSha256"
$preview = [ordered]@{
...
$directory = Join-Path $projectRoot "reports\audit\priority_forward_v1_11_deployment"
