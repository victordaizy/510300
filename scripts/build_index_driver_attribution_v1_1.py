...
from research.index_driver_attribution_v1_1 import (  # noqa: E402
...
def _verify_manifest() -> dict:
    if not MANIFEST_FILE.exists():
        raise FileNotFoundError("归因 V1.1 尚未冻结；先运行 V1.1 冻结脚本")
    ...
    if changed:
        raise RuntimeError(f"归因 V1.1 冻结文件指纹变化：{changed}")
    return manifest
...
def main() -> int:
    ...
    report["implementation_revision"] = (
        "COALESCE_OVERLAPPING_SAME_CITIC_L1_INTERVALS"
    )
    _atomic_text_write(
