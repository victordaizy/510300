...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行510300 UP20预测V1.1全期广度版")
    parser.add_argument("--no-write", action="store_true", help="完整复算但不写入")
    parser.add_argument("--quiet", action="store_true", help="不显示拟合进度")
...
def main() -> int:
    ...
    try:
        ...
        payload = {
            "status": "NO_VIEW_V1_1_DATA_OR_PROTOCOL_CONTRACT_FAILED",
        ...
        json.dumps(
            {
                ...
                "primary_stress_net_sharpe": primary["stress"][
                    "sharpe_zero_cash_rate"
                ],
                "recent_stress_net_sharpe": primary["recent_period"]["stress"][
                    "net_sharpe"
                ],
                ...
                "historical_account_simulation_target_achieved": report[
                    "adjudication"
