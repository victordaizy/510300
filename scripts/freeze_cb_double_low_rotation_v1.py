...
MANIFEST = ROOT / "config" / "cb_double_low_rotation_v1_manifest.json"
TRACKED_FILES = [
    "config/cb_double_low_rotation_v1.yaml",
    "docs/CB_DOUBLE_LOW_ROTATION_V1_SPEC.md",
    "research/cb_double_low_rotation_v1.py",
    "scripts/run_cb_double_low_rotation_v1.py",
    "scripts/freeze_cb_double_low_rotation_v1.py",
]
VISIBLE_SOURCE_DEPENDENCIES = [
    "data/raw/cb_double_low_v1/collection_status.json",
    "data/raw/cb_double_low_v1/bond_master.parquet",
    "data/raw/cb_double_low_v1/visible_panel_through_2022.parquet",
    "data/raw/a_share_hash_holdout_v2/training_H00300.parquet",
    "scripts/download_cb_double_low_history_v1.py",
    "config/multi_asset_annual_excess_40pct_high_sharpe_v4_manifest.json",
...
def file_record(relative: str) -> dict[str, Any]:
    path = ROOT / relative
    if not path.is_file():
        raise FileNotFoundError(f"缺少可转债双低冻结文件：{relative}")
    return {
        "path": relative,
...
def prefreeze_signal_coverage(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        "minimum_selection_dates": int(signal_audit["selection_date_count"])
        >= int(gate["minimum_selection_dates"]),
        "minimum_eligible_bond_date_rows": int(
            signal_audit["eligible_bond_date_rows"]
        )
        >= int(gate["minimum_eligible_bond_date_rows"]),
        "minimum_unique_eligible_bonds": int(signal_audit["unique_eligible_bonds"])
        >= int(gate["minimum_unique_eligible_bonds"]),
    ...
    return {
        "status": (
            "PASS_PREFREEZE_CB_SIGNAL_COVERAGE"
            if all(checks.values())
            else "FAIL_PREFREEZE_CB_SIGNAL_COVERAGE"
        ),
        "checks": checks,
        "all_checks_pass": bool(all(checks.values())),
        "selection_rows": int(len(selections)),
        "selection_date_count": int(signal_audit["selection_date_count"]),
        "full_target_selection_date_count": int(
            signal_audit["full_target_selection_date_count"]
        ),
        "eligible_bond_date_rows": int(signal_audit["eligible_bond_date_rows"]),
        "unique_eligible_bonds": int(signal_audit["unique_eligible_bonds"]),
...
def freeze() -> dict[str, Any]:
    ...
    if MANIFEST.exists():
        ...
        if verification["failure_count"]:
            raise RuntimeError(f"现有可转债双低清单验证失败：{verification['failures']}")
    ...
    if not coverage["all_checks_pass"]:
        raise RuntimeError(f"冻结前可转债信号覆盖门失败：{coverage['checks']}")
    manifest = {
        "schema_version": "1.0.0",
        "manifest_id": "CB_DOUBLE_LOW_ROTATION_V1_MANIFEST",
...
def verify() -> dict[str, Any]:
    if not MANIFEST.is_file():
        return {
            "status": "FAIL_MISSING_CB_DOUBLE_LOW_ROTATION_V1_MANIFEST",
            "failure_count": 1,
    ...
    return {
        "status": (
            "PASS_CB_DOUBLE_LOW_ROTATION_V1_MANIFEST_VERIFIED"
            if not failures
            else "FAIL_CB_DOUBLE_LOW_ROTATION_V1_MANIFEST"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="冻结或验证可转债双低周频轮动V1")
