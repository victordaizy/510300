...
OUT=ROOT/'reports/research/510300_forward_eps_csi_directory_v1'
RAW=ROOT/'data/raw/510300_forward_eps_csi_directory_v1'
MANIFEST=ROOT/'config/510300_forward_eps_csi_directory_v1_manifest.json'
...
def freeze():
    ...
    paths=[Path(__file__),ROOT/'docs/510300_FORWARD_EPS_CSI_HISTORY_V1.md',MEMBERS,
           ...
           ROOT/'reports/research/510300_forward_eps_history_directory_v1/org_scope_probe/response.json',
           ROOT/'reports/research/510300_forward_eps_history_directory_v1/org_scope_probe/receipt.json']
    save(MANIFEST,{'registered_at':now(),'institution':'国信证券','institution_code':'80000007',
    ...
    print('已登记历史沪深300并集',len(universe),'个证券的国信研报目录。',flush=True)
...
def page(year,number):
    ...
    params={'code':'*','orgCode':'80000007','qType':0,'pageSize':50,'pageNo':number,
...
def year_run(year):
    ...
    for n in range(1,total+1):
        ...
        for x in d['data']:
            if x['orgCode']!='80000007':raise ValueError('机构过滤未生效')
        ...
        if n%10==0:print('全行业国信目录',year,n,'/',total,'页',flush=True)
...
def run():
    ...
    d.to_parquet(OUT/'all_guosen_stock_reports.parquet',index=False)
    ...
    q.to_csv(OUT/'历史成分股_国信原始研报队列.csv',index=False,encoding='utf-8-sig')
    ...
    result={'completed_at':now(),'status':'HISTORICAL_MEMBER_ANALYST_DIRECTORY_COMPLETE' if not errors else 'HISTORICAL_MEMBER_ANALYST_DIRECTORY_PARTIAL',
            ...
            'outputs':[ident(OUT/'all_guosen_stock_reports.parquet'),ident(OUT/'historical_member_original_pdf_queue.parquet')]}
