...
def verify_acquisition(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise EvidenceContractError("V1.0.1 采集 manifest 必须是对象")
    verify_manifest_payload(payload)
    if payload.get("status") != (
        "PASS_CORRECTED_PIT_COVERAGE_DAILY_AND_ACTION_CANDIDATE_ACQUISITION"
    ):
        raise EvidenceContractError("V1.0.1 采集 manifest 不是通过状态")
    if payload.get("performance_values_read") is not False:
        raise EvidenceContractError("采集阶段意外读取绩效")
...
def build(config_path: Path = DEFAULT_CONFIG) -> dict[str, Any]:
    ...
    outputs = {
        key: _project_path(str(value)) for key, value in config["ledger_outputs"].items()
    }
    existing = [str(path) for path in outputs.values() if path.exists()]
    if existing:
        raise EvidenceContractError(f"V1.0.1 四态账本输出已存在，禁止覆盖：{existing}")
    ...
    report = "\n".join(
        [
            ...
            f"- 显式点时成员日：{member_day_count:,}",
            f"- `TRADED_VALID`：{state_totals[ConstituentReturnState.TRADED_VALID.value]:,}",
            f"- `OFFICIAL_SUSPENSION`：{state_totals[ConstituentReturnState.OFFICIAL_SUSPENSION.value]:,}（没有准入交易所官方源）",
    ...
    receipt: dict[str, Any] = {
        "receipt_id": "510300_STRESS_TRANSMISSION_HAZARD_V2_FOUR_STATE_LEDGER_V1_0_1",
        "created_at": now,
        "status": "PASS_FOUR_STATE_LEDGER_WITH_EXPLICIT_NO_VIEW_CHAIN_AFTER_SELECTOR_CORRECTION",
        "selector_correction": correction["selector_correction"],
        "outputs": evidence,
        "metrics": {
            "market_session_count": int(len(coverage)),
            "point_in_time_member_day_count": member_day_count,
        ...
        "invariants": {
            ...
            "current_constituent_panel_used_for_universe": False,
    ...
    print("V1.0.1 成分收益四态与覆盖账本已构建；绩效与交易继续关闭。")
    return receipt
...
def main() -> int:
    ...
    try:
        ...
        print(f"V1.0.1 四态账本构建失败：{type(exc).__name__}: {exc}")
        return 1
