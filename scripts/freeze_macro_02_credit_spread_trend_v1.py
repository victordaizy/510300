"""冻结MACRO-02协议、实现和输入哈希；冻结前不得读取真实未来收益。"""
...
CONFIG = ROOT / "config" / "macro_02_credit_spread_trend_v1.yaml"
MANIFEST = ROOT / "config" / "macro_02_credit_spread_trend_v1_manifest.json"
FROZEN_FILES = [
    "config/macro_02_credit_spread_trend_v1.yaml",
    "docs/510300_MACRO_02_CREDIT_SPREAD_TREND_V1_SPEC.md",
    "research/daily_macro_02_credit_spread_trend_v1.py",
    "scripts/download_china_credit_spread_daily.py",
    "scripts/run_macro_02_credit_spread_trend_v1.py",
    "tests/test_macro_02_credit_spread_trend_v1.py",
]
...
def main() -> int:
    ...
    manifest = {
        ...
        "credit_factor_outcomes_seen_before_freeze": False,
        "data_distributions_seen_before_freeze": [
            "信用利差日期范围、最小值、最大值及传输验证状态",
            "3个官方固定日期数值与既有国债批次一致性",
            "510300行情与分红数据质量",
        ],
        "known_data_limitation": (
            "1006行来自未验证TLS传输；官方检查点和同源批次核对通过，"
            "但没有独立供应商验证。"
        ),
    ...
    print(
        json.dumps(
            {"status": "FROZEN", "manifest": str(MANIFEST.relative_to(ROOT))},
            ensure_ascii=False,
        )
    )
    return 0
