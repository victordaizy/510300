...
$orchestrator = Join-Path $projectRoot "scripts\run_priority_forward_codex_automation_v1_10.py"
...
function Get-TaskAudit {
    ...
    return [ordered]@{
        ...
        runtime_patch_id = "PCF_IOPV_STRICT_TLS_ROUTE_V1_3_AND_STATUS_MODULE_ENTRYPOINT_V1_8_1"
        pcf_transport_contract = "SSE_STRICT_TRANSPORT_CONTRACT_V1"
        authority_status_module_entrypoint = $true
...
$task = New-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description "工作日运行零付费 V1.10 冻结入口；PCF严格TLS，状态使用Python模块入口，错过窗口不补跑。"

Register-ScheduledTask -TaskName $TaskName -InputObject $task -Force | Out-Null
$registered = Get-ScheduledTask -TaskName $TaskName -ErrorAction Stop
$audit = Get-TaskAudit -Task $registered
if (-not $audit.exact_match) {
    throw "计划任务注册后与 V1.10 入口不一致"
}
$audit | ConvertTo-Json -Depth 5
