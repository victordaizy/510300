...
RECEIPT_FILE = (
    ...
    / "reports"
    / "frozen"
    / "510300_etf_microstructure_dual_shadow_reverse_validation_v1_0_2_freeze_receipt.json"
)
PRIOR_RUN_DIR = (
    PROJECT_ROOT
    / "data"
    / "external_validation"
    / "510300_etf_microstructure_dual_shadow_reverse_validation_v1_0_1"
    / "snapshots"
    / "20260828T125111_0800"
)
...
def main() -> int:
    ...
    if config["protocol"]["project_id"] != PROJECT_ID:
        raise ValueError("V1.0.2配置项目编号不匹配")
    ...
    if (
        ...
        or prior_status.get("error") != "两来源净值差异超过冻结容差"
        or int(prior_status.get("raw_response_files_preserved", -1)) != 2346
    ):
        raise ValueError("V1.0.1失败收据与V1.0.2机械修正声明不一致")

    tracked_relatives = [
        "config/510300_etf_microstructure_dual_shadow_reverse_validation_v1_0_2.yaml",
        "scripts/collect_510300_etf_microstructure_dual_shadow_reverse_validation_v1_0_2.py",
        "research/evaluate_510300_etf_microstructure_dual_shadow_reverse_validation_v1_0_2.py",
        "scripts/freeze_510300_etf_microstructure_dual_shadow_reverse_validation_v1_0_2.py",
        "config/510300_etf_microstructure_dual_shadow_reverse_validation_v1_0_1.yaml",
        "config/510300_etf_microstructure_dual_shadow_reverse_validation_v1_0_1_manifest.json",
    ...
    if len(raw_files) != 2346:
        raise ValueError(f"V1.0.1原始响应文件数不是2346：{len(raw_files)}")
    ...
    receipt = {
        ...
        "next_entrypoint": (
            ...
            "collect_510300_etf_microstructure_dual_shadow_reverse_validation_v1_0_2.py "
