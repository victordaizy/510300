...
from research.post_close_stale_price_capture_v2 import ProtocolError, record_daily_capture
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="校验并记录已由获准实时行情适配器捕获的A50窗口；本脚本不连接行情或券商"
    )
    parser.add_argument("payload", type=Path, help="UTF-8 JSON原始窗口载荷")
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"状态": "前向窗口未写入", "原因": str(exc)}, ensure_ascii=False, indent=2))
        return 2
    print(json.dumps({"状态": "前向窗口已记录", "结果": result}, ensure_ascii=False, indent=2))
    return 0
