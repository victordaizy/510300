...
from scripts.freeze_a_share_limit_up_continuation_v1 import verify
...
def write_failure_report(
    ...
    report = {
        ...
        "decision": {
            ...
            "next_step": "保留失败并关闭复验；若是合同实现错误只能另起版本修正",
        },
    ...
    atomic_text(
        ...
        "\n".join(
            [
                ...
                f"错误：{exc}",
                "",
                "没有形成绩效；复验、Paper、Shadow、订单和实盘均关闭。",
                "",
            ]
        ),
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="运行A股涨停延续V1可见期")
    parser.add_argument(
