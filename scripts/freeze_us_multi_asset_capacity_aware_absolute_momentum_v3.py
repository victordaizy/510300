...
MANIFEST = ROOT / "config" / "us_multi_asset_capacity_aware_absolute_momentum_v3_manifest.json"
TRACKED_FILES = [
    "config/us_multi_asset_capacity_aware_absolute_momentum_v3.yaml",
    "docs/US_MULTI_ASSET_CAPACITY_AWARE_ABSOLUTE_MOMENTUM_V3_SPEC.md",
    "research/us_multi_asset_capacity_aware_absolute_momentum_v3.py",
    "scripts/freeze_us_multi_asset_capacity_aware_absolute_momentum_v3.py",
    "scripts/run_us_multi_asset_capacity_aware_absolute_momentum_v3.py",
    "tests/test_us_multi_asset_capacity_aware_absolute_momentum_v3.py",
]
VISIBLE_SOURCE_DEPENDENCIES = [
    ...
    "config/multi_asset_annual_excess_40pct_high_sharpe_v9_manifest.json",
...
def file_record(relative: str) -> dict[str, Any]:
    path = ROOT / relative
    if not path.is_file():
        raise FileNotFoundError(f"缺少V3冻结文件：{relative}")
    return {
        "path": relative,
...
def prefreeze_signal_coverage(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": (
            "PASS_PREFREEZE_US_CAPACITY_AWARE_SIGNAL_COVERAGE"
            if all(checks.values())
            else "FAIL_PREFREEZE_US_CAPACITY_AWARE_SIGNAL_COVERAGE"
        ),
        "checks": checks,
...
def _assert_visible_outputs_absent(contract: dict[str, Any]) -> None:
    ...
    if existing:
        raise RuntimeError(f"冻结前已存在V3可见结果：{existing}")
...
def freeze() -> dict[str, Any]:
    ...
    if MANIFEST.exists():
        ...
        if verification["failure_count"]:
            raise RuntimeError(f"现有V3清单验证失败：{verification['failures']}")
    ...
    if any(item["failure_count"] for item in dependencies.values()):
        raise RuntimeError("V3父协议、V2或保守评价器冻结清单验证失败")
    ...
    if not coverage["all_checks_pass"]:
        raise RuntimeError(f"V3冻结前信号覆盖门失败：{coverage['checks']}")
    manifest = {
        "schema_version": "1.0.0",
        "manifest_id": "US_MULTI_ASSET_CAPACITY_AWARE_ABSOLUTE_MOMENTUM_V3_MANIFEST",
...
def verify() -> dict[str, Any]:
    if not MANIFEST.is_file():
        return {
            "status": "FAIL_MISSING_US_CAPACITY_AWARE_V3_MANIFEST",
            "failure_count": 1,
    ...
    return {
        "status": (
            "PASS_US_CAPACITY_AWARE_V3_MANIFEST_VERIFIED"
            if not failures
            else "FAIL_US_CAPACITY_AWARE_V3_MANIFEST"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="冻结或验证美国多资产容量感知绝对动量V3")
