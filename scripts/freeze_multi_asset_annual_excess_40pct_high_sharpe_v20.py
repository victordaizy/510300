...
CONFIG = ROOT / "config" / "multi_asset_annual_excess_40pct_high_sharpe_v20.yaml"
MANIFEST = ROOT / "config" / "multi_asset_annual_excess_40pct_high_sharpe_v20_manifest.json"
TRACKED_FILES = [
    "config/multi_asset_annual_excess_40pct_high_sharpe_v20.yaml",
    "docs/MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V20_SPEC.md",
    "scripts/freeze_multi_asset_annual_excess_40pct_high_sharpe_v20.py",
]
...
def load_contract() -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V20扩展协议必须是YAML对象")
    objective = payload.get("objective", {})
    lane = payload.get("new_lane", {})
    failures: list[str] = []
    if payload.get("protocol", {}).get("protocol_id") != "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V20":
        failures.append("protocol_id")
    if payload.get("protocol", {}).get("parent_protocol_id") != "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V19":
    ...
    if int(lane.get("volatility_lookback_calendar_days", 0)) != 180:
        failures.append("volatility_lookback")
    if int(lane.get("selected_count", 0)) != 3:
        failures.append("selected_count")
    if int(lane.get("selection_rebalance_months", 0)) != 6:
        failures.append("rebalance_months")
    if float(lane.get("within_month_stop_loss", float("nan"))) != 0.05:
        failures.append("stop_loss")
    if float(lane.get("target_long_gross", float("nan"))) != 1.0:
        failures.append("target_long_gross")
    if lane.get("short_or_derivative_allowed") is not False:
        failures.append("short_or_derivative_allowed")
    ...
    if failures:
        raise ValueError(f"V20扩展协议被弱化或损坏：{sorted(set(failures))}")
    return payload
...
def tracked_hashes() -> dict[str, str]:
    ...
    if missing:
        raise FileNotFoundError(f"缺少V20冻结文件：{missing}")
...
def verify_v9_failure(contract: dict[str, Any]) -> dict[str, Any]:
    manifest_path = ROOT / contract["inputs"]["v9_manifest"]
    report_path = ROOT / contract["inputs"]["v9_visible_report"]
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    report = json.loads(report_path.read_text(encoding="utf-8"))
    required = contract["registered_v9_failure"]
    checks = {
    ...
    if not all(checks.values()):
        raise RuntimeError(f"V9失败不符合V20登记条件：{checks}")
    return {
        "checks": checks,
...
def freeze() -> dict[str, Any]:
    ...
    if parent["failure_count"]:
        raise RuntimeError("V20父协议冻结清单验证失败")
    failure = verify_v9_failure(contract)
    files = tracked_hashes()
    manifest = {
        "schema_version": "1.0.0",
        "manifest_id": "MULTI_ASSET_ANNUAL_EXCESS_40PCT_HIGH_SHARPE_V20_MANIFEST",
    ...
    if MANIFEST.exists():
        ...
        for field in (
            ...
            if existing.get(field) != manifest.get(field):
                raise RuntimeError("现有V20清单与当前内容不同；不得覆盖")
        return existing
...
def verify() -> dict[str, Any]:
    if not MANIFEST.is_file():
        return {
            "status": "FAIL_MISSING_MULTI_ASSET_40PCT_HIGH_SHARPE_V20_MANIFEST",
            "failure_count": 1,
    ...
    try:
        if existing.get("v9_failure") != verify_v9_failure(contract):
            failures.append("v9_failure")
    except Exception:
        failures.append("v9_failure")
    parent = verify_parent_manifest()
    if parent["failure_count"]:
    ...
    return {
        "status": (
            "PASS_MULTI_ASSET_40PCT_HIGH_SHARPE_V20_MANIFEST_VERIFIED"
            if not failures
            else "FAIL_MULTI_ASSET_40PCT_HIGH_SHARPE_V20_MANIFEST"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="冻结或验证40个百分点高夏普V20扩展协议")
