...
TRACKED_FILES = [
    ...
    "docs/510300_BINARY_STATE_FEASIBILITY_V1_SPEC.md",
    "research/binary_state_feasibility_v1.py",
    "scripts/run_510300_binary_state_feasibility_v1.py",
    "scripts/freeze_510300_binary_state_feasibility_v1.py",
    "tests/test_510300_binary_state_feasibility_v1.py",
...
def main() -> int:
    ...
    if existing:
        raise FileExistsError(f"历史产物已出现，禁止事后冻结：{existing}")

    data_files = {
        section["file"]: sha256_file(ROOT / section["file"])
        for section in config["data_contracts"].values()
    }
    ...
    print(json.dumps({
        "状态": "FROZEN_BEFORE_FIRST_HISTORICAL_RUN",
