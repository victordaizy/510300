...
def frozen_paths(config: dict[str, Any]) -> list[Path]:
    return [
        ...
        ROOT / "scripts/collect_csi300_pit_fundamental_underreaction_official_facts_v1_3.py",
        ROOT / "scripts/freeze_csi300_pit_fundamental_underreaction_official_facts_v1_3.py",
        ROOT / "tests/test_csi300_pit_fundamental_underreaction_official_facts_v1_3.py",
    ]
...
def validate_existing(path: Path) -> None:
    ...
    print(f"V1.3官方财务事实冻结清单已存在且匹配：{_base.relative_path(path)}")
...
def main() -> int:
    ...
    if checkpoint_root.exists() and any(checkpoint_root.rglob("*.json.gz")):
        raise RuntimeError("已发现V1.3官方PDF采集检查点，拒绝事后冻结")
    ...
    parent_bundle = {
        **parent,
        "superseded_official_fact_v1_2": superseded,
        "representative_revision_gate": config["representative_revision_gate"],
    }
    manifest: dict[str, Any] = {
        "manifest_id": "CSI300_PIT_FUNDAMENTAL_UNDERREACTION_OFFICIAL_FACTS_V1_3_MANIFEST",
    ...
    print(f"V1.3官方财务事实协议已冻结：{_base.relative_path(manifest_path)}")
    print(f"V1.3官方财务事实内容SHA-256：{manifest['content_sha256']}")
    print("V1.2检查点未修改；冻结过程未读取市场价格或未来收益。")
    return 0
