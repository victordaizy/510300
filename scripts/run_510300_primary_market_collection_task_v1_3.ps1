...
$runner = if ([string]::IsNullOrWhiteSpace($RunnerPath)) {
    Join-Path $sourceProjectRoot "scripts\run_510300_primary_market_forward_v1_3.ps1"
}
else {
...
$taskStatusFile = Join-Path $projectRoot "reports\data_quality\510300_primary_market_task_status_v1_3.json"
$taskReceiptDirectory = Join-Path $projectRoot "reports\data_quality\primary_market_task_runs_v1_3"
$tradingCalendarFile = Join-Path $projectRoot "data\reference\sse_trade_calendar_2026.csv"
$logDirectory = Join-Path $projectRoot "output\primary_market_forward_logs_v1_3"
