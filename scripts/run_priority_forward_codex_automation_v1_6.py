...
DEFAULT_MANIFEST = ROOT / "config/priority_forward_research_operations_v1_6_manifest.json"
DEFAULT_CONFIG = ROOT / "config/priority_forward_supervisor_v1_2.yaml"
...
def verify_manifest(manifest_path: Path, expected_manifest_sha256: str) -> dict[str, Any]:
    ...
    if manifest_path != DEFAULT_MANIFEST.resolve():
        raise RuntimeError(f"V1.6 入口只接受固定清单：{DEFAULT_MANIFEST}")
    if not manifest_path.is_file():
        raise RuntimeError(f"V1.6 冻结清单不存在：{manifest_path}")
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("V1.6 清单顶层必须是对象")
    if manifest.get("status") not in ACCEPTED_MANIFEST_STATUSES:
        raise RuntimeError(f"V1.6 清单状态不可执行：{manifest.get('status')}")
    if manifest.get("data_purchase_budget_cny") != 0:
        raise RuntimeError("V1.6 清单的数据采购预算不是 0 CNY")
    if manifest.get("maximum_active_research_streams") != 2:
        raise RuntimeError("V1.6 清单的活动研究流上限不是 2")
    files = manifest.get("files")
    if not isinstance(files, list) or not files:
        raise RuntimeError("V1.6 清单没有受控文件")
    current_records: list[dict[str, Any]] = []
    failures: list[dict[str, Any]] = []
    seen_paths: set[str] = set()
    for item in files:
        if not isinstance(item, dict):
            raise RuntimeError("V1.6 清单文件记录必须是对象")
        relative_path = str(item.get("path") or "")
        if not relative_path or relative_path in seen_paths:
            raise RuntimeError(f"V1.6 清单文件路径为空或重复：{relative_path!r}")
    ...
    self_relative = Path(__file__).resolve().relative_to(ROOT.resolve()).as_posix()
    if self_relative not in seen_paths:
        failures.append({"path": self_relative, "status": "ENTRYPOINT_NOT_TRACKED"})
    delegate_relative = DELEGATE.resolve().relative_to(ROOT.resolve()).as_posix()
    if delegate_relative not in seen_paths:
        failures.append({"path": delegate_relative, "status": "DELEGATE_NOT_TRACKED"})
    config_relative = DEFAULT_CONFIG.resolve().relative_to(ROOT.resolve()).as_posix()
    if config_relative not in seen_paths:
        failures.append({"path": config_relative, "status": "SUPERVISOR_CONFIG_NOT_TRACKED"})
    result = {
        "status": (
            "PASS_V1_6_FROZEN_ENTRYPOINT_VERIFIED"
            if not failures
            else "FAILED_V1_6_FROZEN_ENTRYPOINT_VERIFICATION"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="运行 V1.6 冻结入口")
    ...
    if config_path != DEFAULT_CONFIG.resolve():
        raise RuntimeError(f"V1.6 入口只接受固定编排配置：{DEFAULT_CONFIG}")
