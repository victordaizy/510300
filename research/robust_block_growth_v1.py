...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "最弱阶段增长预算冻结来源改变")
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "最弱阶段增长预算账户财富或终点清仓不符")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "按近期最弱子阶段增长分配预算"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(ledger.date)), "最弱阶段增长预算与保存对照日历不同")
            ...
            print(f"{period}／{cost_id}：一条新最弱阶段增长预算账户及四条保存对照完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "ROBUST_BLOCK_GROWTH_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
        ...
        "new_budget_optimizations": sum(row["status"] == "BUDGET_OPTIMIZATION_COMPLETE" for row in budget_estimations),
    ...
    print(json.dumps({"主历史": primary, "较早历史": [m for m in earlier if m["model"] == PRIMARY],
        "账户": coverage, "最弱阶段增长预算": target_summaries, "核心耗时": result["run_seconds"]}, ensure_ascii=False), flush=True)
