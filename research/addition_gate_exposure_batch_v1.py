"""四套固定加仓条件投入倍率一次计算、独立重建申请并保存完整比较。"""
import contextlib
import json
import subprocess
import sys
...
from research.addition_gate_exposure_batch_inputs_v1 import CANDIDATES, MODELS, PRIMARY, SETTINGS, allowance_column, gate_frames, simulate_gate_account
...
OUT = ROOT/'reports/research/510300_addition_gate_exposure_batch_v1'
CONFIG = ROOT/'config/510300_addition_gate_exposure_batch_v1.json'
SOURCE = ROOT/'reports/research/510300_finite_rebalance_band_batch_v1'
PARENTS = {m: SOURCE for m in MODELS}
CONTROLS = {'ADD_GATE_BAND20_LOWER01': (ROOT/'reports/research/510300_addition_only_return_gate_batch_v1', '原205仅强势加仓'), MODELS[0]: (SOURCE, '原201零门槛合并'), MODELS[1]: (SOURCE, '原201二十个百分点合并'),
...
def prepare():
    ...
    test = subprocess.run([sys.executable, '-m', 'pytest', 'tests/test_addition_gate_exposure_batch_v1.py', '-q', '-p', 'no:cacheprovider'],
    ...
    cfg.update(study_id='510300_ADDITION_GATE_EXPOSURE_BATCH_V1', round=206, registered_at=now(), primary=PRIMARY,
        ...
        evidence_class='RETROSPECTIVE_FIXED_ADDITION_GATE_CAPPED_EXPOSURE_MULTIPLIERS',
        prior_goal_turn_classification='PROGRESS_ROUND205_COMPLETED_32_ACCOUNTS_AND_JOINT_GAP_IMPROVED',
        rules='docs/510300_ADDITION_GATE_EXPOSURE_BATCH_V1.md')
    text = (ROOT/'docs/510300_ADDITION_GATE_EXPOSURE_BATCH_NEXT_20260913.md').read_text(encoding='utf-8')
    ...
    text += '\n\n## 来源全部中文因素和原进出场\n\n下文保留第201轮及内部参考规则，说明原目标如何产生。本批固定二十个百分点门槛及至少1%的加仓准入，只对原始目标乘候选倍率并限制100%；空仓按新目标正常进入，退出结构保持，不加入第202轮亏损保护。旧轮次状态属于来源记录。\n\n'
    text += (ROOT/old['rules']).read_text(encoding='utf-8')
    with (ROOT/cfg['rules']).open('x', encoding='utf-8') as stream:
        stream.write(text)
    paths = [Path(__file__), ROOT/'research/addition_gate_exposure_batch_inputs_v1.py',
        ROOT/'tests/test_addition_gate_exposure_batch_v1.py', ROOT/'tests/test_two_close_zero_exit_v1.py',
    ...
    require(index['latest_completed_round']['round'] == 205 and not index['running_studies'], '前序完成状态不同')
    write_json(CONFIG, cfg, exclusive=True)
    index['running_studies'] = [{'round': 206, 'study': cfg['study_id'], 'status': 'FROZEN_NOT_STARTED', 'config': str(CONFIG.relative_to(ROOT))}]
    index['next_work'].update(registered=True, status='ADDITION_GATE_EXPOSURE_BATCH_FROZEN', source=cfg['rules'])
    write_json(path, index)
    print('第206轮四套投入倍率已冻结，计划十六条账户。', flush=True)
...
def run():
    ...
    ranked.to_csv(OUT/'eight_setting_joint_comparison.csv', index=False, encoding='utf-8-sig')
...
def verify():
    ...
    receipt = {'verified_at': now(), 'status': 'PASS_SIXTEEN_CAPPED_EXPOSURE_ACCOUNTS_AND_UNFILTERED_INITIAL_ENTRIES',
