...
def simulate_entry_opportunity(data, dividends, config, cost, start, rule, spec, controller=None, entry_gate=None):
    ...
    def decide(t):
        nonlocal pending_reasons, entry_rearmed, last_entry_mode
        ...
        if account.shares:
            ...
            if entry_gate is not None:
                entry_check = entry_gate(t, account, selected_mode, qty)
                if not entry_check["entry_allowed"]:
                    qty, action = 0, "完整交易收益预测不为正，跳过本次机会并等待信号重现"
                    entry_rearmed, last_entry_mode = False, mode
