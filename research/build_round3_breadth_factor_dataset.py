...
OUTPUT_FILE = ROOT / "data" / "features" / "510300_round3_breadth_dataset.parquet"
REPORT_FILE = ROOT / "reports" / "research" / "round3_breadth_factor_dataset.json"
...
def main() -> int:
    for path in (SETTINGS_FILE, REGISTRY_FILE, BREADTH_FILE, BASE_DATASET_FILE):
        if not path.exists():
            raise FileNotFoundError(f"第三轮宽度数据集输入缺失：{path}")
    ...
    if len(dataset) != len(breadth):
        raise ValueError(f"宽度与可执行标签日期未完整对齐：{len(breadth)} -> {len(dataset)}")
    for horizon in (5, 20):
    ...
    if dataset[factor_columns].isna().any().any():
        raise ValueError("第三轮登记因子与标签合并后出现空值")
