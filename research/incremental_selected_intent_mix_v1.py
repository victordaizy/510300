...
from research.incremental_selected_intent_mix_inputs_v1 import CANDIDATES, MIX_PATH, MODELS, PRIMARY, SETTINGS, SOURCES, allowance_column, gate_frames, simulate_gate_account
...
OUT = ROOT/'reports/research/510300_incremental_selected_intent_mix_v1'
CONFIG = ROOT/'config/510300_incremental_selected_intent_mix_v1.json'
...
def prepare():
    ...
    test = subprocess.run([sys.executable, '-m', 'pytest', 'tests/test_incremental_selected_intent_mix_v1.py', '-q', '-p', 'no:cacheprovider'],
    ...
    cfg.update(study_id='510300_INCREMENTAL_SELECTED_INTENT_MIX_V1', round=209, registered_at=now(), primary=PRIMARY,
        candidate_models=list(CANDIDATES), candidate_configurations=8, candidate_settings=SETTINGS, parent_models=MODELS, source_folders=SOURCES,
        ...
        evidence_class='RETROSPECTIVE_INCREMENTAL_SELECTED_PLANNED_INTENT_MIX',
        prior_goal_turn_classification='PROGRESS_ROUND207_208_COMPLETED_56_ACCOUNTS_AND_NEW_VIRTUAL_MIX_POINT_PASS',
        rules='docs/510300_INCREMENTAL_SELECTED_INTENT_MIX_V1.md')
    text = (ROOT/'docs/510300_INCREMENTAL_SELECTED_INTENT_MIX_NEXT_20260913.md').read_text(encoding='utf-8')
    ...
    with (ROOT/cfg['rules']).open('x', encoding='utf-8') as stream:
        stream.write(text)
    paths = [Path(__file__), MIX_PATH, *appended_rules, ROOT/'research/incremental_selected_intent_mix_inputs_v1.py',
        ROOT/'tests/test_incremental_selected_intent_mix_v1.py', ROOT/'tests/test_two_close_zero_exit_v1.py',
    ...
    require(index['latest_completed_round']['round'] == 208 and not index['running_studies'], '前序完成状态不同')
    write_json(CONFIG, cfg, exclusive=True)
    index['running_studies'] = [{'round': 209, 'study': cfg['study_id'], 'status': 'FROZEN_NOT_STARTED', 'config': str(CONFIG.relative_to(ROOT))}]
    index['next_work'].update(registered=True, status='INCREMENTAL_SELECTED_INTENT_MIX_FROZEN', source=cfg['rules'])
    write_json(path, index)
    print('第209轮八套筛选计划合并已冻结，计划三十二条账户。', flush=True)
...
def verify():
    ...
    def expected(period, cost, frame, start, model):
        ...
        allowed = np.ones(len(indices), dtype=bool)
    ...
    receipt = {'verified_at': now(), 'status': 'PASS_THIRTY_TWO_SELECTED_INTENT_ACCOUNTS_AND_NO_DUPLICATE_GATES',
