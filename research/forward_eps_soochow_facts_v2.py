...
from research.forward_eps_soochow_layout_v2 import parse

SOURCE = ROOT / "reports/research/510300_forward_eps_soochow_originals_v1"
OUT = ROOT / "reports/research/510300_forward_eps_soochow_facts_v2"


def freeze() -> None:
    OUT.mkdir(parents=True, exist_ok=False)
    paths = [Path(__file__), ROOT / "research/forward_eps_soochow_layout_v2.py",
             ROOT / "research/forward_eps_soochow_facts_v1.py", ROOT / "tests/test_forward_eps_soochow_layout_v2.py",
             ROOT / "tests/test_forward_eps_soochow_facts_v1.py", ROOT / "docs/510300_FORWARD_EPS_SOOCHOW_LAYOUT_CORRECTION_V2.md",
    ...
    save(OUT / "manifest.json", {"registered_at": now(), "same_original_queue": True,
         ...
         "v1_successes_preserved_exactly": True, "files": [identity(p) for p in paths]}, exclusive=True)
    print("东吴旧版分栏预测表修正已登记，完整队列未变。", flush=True)


def run(first_250: bool = False) -> None:
    for item in read(OUT / "manifest.json")["files"]:
        assert identity(ROOT / item["path"])["sha256"] == item["sha256"], "第二版来源冻结输入变化"
    ...
    for row in queue.to_dict("records"):
        ...
        if path.exists():
            ...
            try:
                ...
                preserved += parsed["adapter"] == "V1_SUCCESS_PRESERVED_EXACTLY"
    ...
    result = {"study_id": "510300_FORWARD_EPS_SOOCHOW_FACTS_V2", "completed_at": now(),
              ...
              "forecast_eps_facts": len(facts), "v1_successful_reports_preserved": preserved,
