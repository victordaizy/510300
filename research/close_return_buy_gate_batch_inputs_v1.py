...
for band in [0, 20]:
    for cap in [0, 1]:
        model = f'BUY_GATE_BAND{band:02d}_R{cap:02d}'
        SETTINGS[model] = {'parent': f'INTENT_MIX_BAND_{band:02d}', 'band': band/100, 'return_cap': cap/100}
        CANDIDATES[model] = f'{band}个百分点调仓、当日涨幅不超过{cap}%才新增买入'
PRIMARY = 'BUY_GATE_BAND00_R01'
...
def gate_frames(data, parents_by_cost, cfg, start):
    ...
    for cost, frame in frames.items():
        ...
        for model, setting in SETTINGS.items():
            ...
            allowed = market[f"buy_allowed_r{int(setting['return_cap']*100):02d}"].iloc[first-1:-1]
            summaries.append({'model': model, 'cost': cost, 'decision_origins': len(raw),
                ...
                'rebalance_band': setting['band'], 'return_cap': setting['return_cap']})
...
def gate_request(account, price, raw, cfg, model, allowed, daily_return):
    ...
    result.update(output_candidate=model, budget_source=setting['parent'], used_band=float(setting['band']),
        return_cap=float(setting['return_cap']), observed_daily_return=float(daily_return), buy_allowed=float(allowed),
...
def simulate_gate_account(data, dividends, cfg, cost, start, model, targets=None, prediction=None, horizon=1, event_mask=None):
    ...
    setting = SETTINGS[model]
    allowed = market[f"buy_allowed_r{int(setting['return_cap']*100):02d}"].to_numpy(bool)
