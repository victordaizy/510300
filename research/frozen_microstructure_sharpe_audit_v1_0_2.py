"""V1.0.2机械修正：把报告边界中的小写false绑定为Python False。"""
...
import frozen_microstructure_sharpe_audit_v1_0_1 as correction_v1_0_1


ROOT = Path(__file__).resolve().parents[1]
CORRECTION_MANIFEST = (
    ROOT / "config" / "510300_frozen_microstructure_sharpe_audit_v1_0_2_manifest.json"
)


def validate_correction_manifest() -> dict[str, Any]:
    if not CORRECTION_MANIFEST.exists():
        raise FileNotFoundError("V1.0.2机械修正清单不存在")
    manifest = json.loads(CORRECTION_MANIFEST.read_text(encoding="utf-8"))
    if not manifest.get("implementation_frozen", False):
        raise ValueError("V1.0.2机械修正尚未冻结")
    mismatches: dict[str, dict[str, str]] = {}
    for relative, expected in manifest["tracked_files"].items():
        path = ROOT / relative
        actual = "MISSING" if not path.exists() else correction_v1_0_1.base.sha256_file(path)
        if actual != expected:
            mismatches[relative] = {"expected": expected, "actual": actual}
    if mismatches:
        raise ValueError(f"V1.0.2机械修正哈希漂移：{json.dumps(mismatches, ensure_ascii=False)}")
    return manifest


def main() -> int:
    manifest = validate_correction_manifest()
    original_module = correction_v1_0_1.base
    ...
    def write_outputs_with_correction(
        ...
        report["correction_v1_0_2"] = {
            "project_id": manifest["project_id"],
            "status": "MECHANICAL_PYTHON_BOOLEAN_LITERAL_CORRECTION",
            "manifest_sha256": original_module.sha256_file(CORRECTION_MANIFEST),
            "only_change": "父模块命名空间false = False",
