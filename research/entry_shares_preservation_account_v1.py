"""正目标期间保持初始实际份额的完整模拟账户，沿用既有费用、整手、分红和T+1记账。"""
...
def preservation_request(account, price, target, config):
...
def simulate_preserved_shares(data: pd.DataFrame, dividends: pd.DataFrame, config: dict, cost: dict, start: str,
    ...
    def decision_at(t: int) -> dict:
        ...
        if model_id == "BUY_HOLD":
            ...
            if np.isfinite(value):
                ...
                result = preservation_request(account, price, value, config)
            else:
