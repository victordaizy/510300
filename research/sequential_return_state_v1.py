"""冻结单一双向累计收益状态规则，共读来源后运行四个完整账户。"""
import json
from pathlib import Path
from research.sequential_return_state_inputs_v1 import PRIMARY, CANDIDATES, sequential_return_frames
from research.saved_target_batch_runner_v1 import run_saved_target_batch
from research.intraday_overnight_increment_v1 import now, digest, require, write_json

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_sequential_return_state_v1"
CONFIG = ROOT / "config/510300_sequential_return_state_v1.json"
...
def freeze():
    require(not CONFIG.exists(), "第160轮双向累计收益状态规则已经冻结")
    prior_path = ROOT / "config/510300_median_slope_risk_v1.json"
    prior = json.loads(prior_path.read_text(encoding="utf-8"))
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "双向累计收益状态六项必要测试尚未通过")
    ...
    cfg.update(study_id="510300_SEQUENTIAL_RETURN_STATE_V1", round=160, primary=PRIMARY, candidate_configurations=1,
        ...
        risk_target=.1, direction="UPWARD_ALARM_LONG_DOWNWARD_ALARM_ZERO_ELSE_PREVIOUS_STATE", initial_state="KNOWN_NONPOSITIVE_AFTER_COMPLETE_SCALE",
        positive_direction_invalid_volatility="UNKNOWN_TARGET", missing_window="UNKNOWN_TODAY_RESET_INTERNAL_STATE_REQUIRE_COMPLETE_PREVIOUS_TWENTY",
        new_model_fits=0, model_fit_count_scope="ZERO_SUPERVISED_MODELS_DESCRIPTIVE_DAILY_RECURSION_ONLY",
        ...
        rules="docs/510300_SEQUENTIAL_RETURN_STATE_V1.md", independent_validation="NOT_ESTABLISHED",
        previous_goal_turn_classification="PROGRESS_ROUND159_COMPLETED_CLOSED_AND_DELIVERED_FULL_GOAL_NOT_MET")
    paths = [Path(__file__), prior_path, ROOT / "research/sequential_return_state_inputs_v1.py",
        ...
        ROOT / "research/intraday_overnight_increment_v1.py", ROOT / "tests/test_sequential_return_state_v1.py",
        ...
        ROOT / "docs/510300_SEQUENTIAL_RETURN_STATE_NEXT_20260910.md", ROOT / "config/510300_research_authority_v6.json",
    ...
    print("第160轮一个固定双向收益状态规则已冻结，尚未生成新历史目标或账户。", flush=True)


def run():
    return run_saved_target_batch(ROOT, OUT, CONFIG, CANDIDATES, {}, CONTROLS, sequential_return_frames)
