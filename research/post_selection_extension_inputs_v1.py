...
CONFIG = ROOT / 'config/510300_post_selection_extension_inputs_v1.json'
...
def main():
    ...
    require(not OUT.exists() and not CONFIG.exists(), '本项已开始，请接续结果，不重写已接纳输入')
    index_path=ROOT/'reports/research/510300_sharpe_1_2_latest_research.json'
    index=read(index_path)
    require(index['latest_completed_round']['round']==211 and not index['running_studies'], '前序状态不同')
    nodes=dependencies()
    ...
    with rule.open('x',encoding='utf-8') as stream:
        stream.write((ROOT/'docs/510300_POST_SELECTION_EXTENSION_INPUTS_NEXT_20260913.md').read_text(encoding='utf-8').replace(
            '本项目前只准备规则，尚未实现或运行。','本项在输入正式接纳前固定范围；已完成来源精度和旧因子一致性的只读预检。'))
    ...
    for field in ['open','high','low','close','volume','amount']:
        ...
        if threshold:
            require(np.array_equal(np.round(a/100)*100,b), '腾讯尾数未能由新浪按百单位舍入复现：'+field)
    ...
    pd.DataFrame(rows).to_csv(OUT/'source_precision_comparison.csv',index=False,encoding='utf-8-sig')
    ...
    candidate=pd.concat([old,extra[old.columns]],ignore_index=True)
