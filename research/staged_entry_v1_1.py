...
OUT = ROOT / "reports/research/510300_staged_entry_v1_1"
CONFIG = ROOT / "config/510300_staged_entry_v1_1.json"
...
def freeze():
    ...
    cfg.update(study_id="510300_PRICE_CONFIRMED_STAGED_ENTRY_V1_1_SOURCE_FIX", source_version=2, new_candidate_configurations=0, round=81, registered_at=now(), primary=PRIMARY,
        ...
        new_model_fits=0, new_reference_accounts=0, rules="docs/510300_STAGED_ENTRY_V1.md", source_amendment="docs/510300_STAGED_ENTRY_SOURCE_FIX_20260908.md", execution_control=EXECUTION_CONTROL,
    ...
    paths = [Path(__file__), ROOT / "research/staged_entry_inputs_v1_1.py", ROOT / "research/event_clock_account_v1.py",
        ...
        ROOT / cfg["features"], ROOT / cfg["dividends"], ROOT / cfg["rules"], ROOT / "tests/test_staged_entry_v1_1.py",
    ...
    paths += [ROOT / cfg["source_amendment"], ROOT / "config/510300_staged_entry_v1.json",
    ...
    print("第81轮相同规则的持仓字段修正已冻结；首次收益已见，修正账户尚未运行。", flush=True)
...
def run():
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            for model, target, name in [(PRIMARY, f.target.to_numpy(float), "先半仓、回升确认后补足"),
                ...
                current_factors = f.rename(columns={"date": "origin", "target": "staged_target"})
                if model == PRIMARY:
    ...
    write_json(OUT / "result.json", {"study_id": cfg["study_id"], "completed_at": now(), "status": "STAGED_ENTRY_ACCOUNTS_COMPLETE",
        "candidate_configurations": 1, "source_version": 2, "new_candidate_configurations": 0, "evaluation_accounts": 12, "new_accounts_generated": 2, "new_execution_control_accounts": 0, "reused_execution_control_accounts": 2, "reused_control_accounts": 10,
        "earlier_diagnostic_accounts": 12, "new_earlier_diagnostic_accounts": 2, "new_earlier_execution_control_accounts": 0, "reused_earlier_execution_control_accounts": 2, "reused_earlier_accounts": 10,
