...
OUT = ROOT / "reports/research/510300_forward_eps_corrected_account_attribution_v1"
...
def decompose(ledger: pd.DataFrame, market: pd.DataFrame, initial: float, start: pd.Timestamp) -> pd.DataFrame:
    ...
    values = pd.DataFrame({"date": selected.date.to_numpy(), "previous_nav": previous_nav,
        ...
        "cost_cny": selected.commission.to_numpy() + selected.slippage_cost.to_numpy()})
    for suffix, left, right, gain_left, gain_right, dividend, friction in [
        ("return", w_on, w_day, r_on, r_day, values.dividend_residual_cny.to_numpy() / previous_nav, values.cost_cny.to_numpy() / previous_nav),
        ("cny", q_on, q_day, values.unit_gain_on.to_numpy(), values.unit_gain_day.to_numpy(), values.dividend_residual_cny.to_numpy(), values.cost_cny.to_numpy()),
    ]:
...
def main():
    ...
    result = {"study_id": "510300_FORWARD_EPS_CORRECTED_ACCOUNT_ATTRIBUTION_V1", "completed_at": now(),
