...
from research.addition_only_return_gate_batch_inputs_v1 import CANDIDATES, MODELS, PRIMARY, SETTINGS, allowance_column, gate_frames, simulate_gate_account
...
OUT = ROOT/'reports/research/510300_addition_only_return_gate_batch_v1'
CONFIG = ROOT/'config/510300_addition_only_return_gate_batch_v1.json'
...
def prepare():
    ...
    test = subprocess.run([sys.executable, '-m', 'pytest', 'tests/test_addition_only_return_gate_batch_v1.py', '-q', '-p', 'no:cacheprovider'],
    ...
    cfg.update(study_id='510300_ADDITION_ONLY_RETURN_GATE_BATCH_V1', round=205, registered_at=now(), primary=PRIMARY,
        ...
        evidence_class='RETROSPECTIVE_ENTRY_PRESERVED_ADDITION_ONLY_RETURN_FILTERS',
        prior_goal_turn_classification='PROGRESS_ROUND203_204_COMPLETED_32_ACCOUNTS_AND_ENTRY_ADDITION_SEPARATION_EVIDENCE',
        rules='docs/510300_ADDITION_ONLY_RETURN_GATE_BATCH_V1.md')
    text = (ROOT/'docs/510300_ADDITION_ONLY_RETURN_GATE_BATCH_NEXT_20260913.md').read_text(encoding='utf-8')
    text = text.replace('本批目前只准备规则，尚未实现、测试、冻结或计算账户。', '本批在五项必要测试通过后、首次新账户计算前冻结规则。')
    text += '\n\n## 八套实际冻结设置\n\n|方案|已有持仓调仓门槛|已有持仓加仓的当日收益条件|\n|---|---:|---:|\n'
    for model, s in SETTINGS.items():
        text += f"|{CANDIDATES[model]}|{s['band']:.0%}|{'不超过' if s['direction']=='UPPER' else '至少为'}{s['return_threshold']:.0%}|\n"
    text += '\n\n## 来源全部中文因素和原进出场\n\n下文保留第201轮及内部参考规则，说明原目标如何产生。本批只选择零与二十个百分点门槛，仅增加上述已有持仓加仓过滤，初次入场完整沿用原规则；不加入第202轮亏损保护。旧轮次状态属于来源记录。\n\n'
    text += (ROOT/old['rules']).read_text(encoding='utf-8')
    with (ROOT/cfg['rules']).open('x', encoding='utf-8') as stream:
        stream.write(text)
    paths = [Path(__file__), ROOT/'research/addition_only_return_gate_batch_inputs_v1.py',
        ROOT/'tests/test_addition_only_return_gate_batch_v1.py', ROOT/'tests/test_two_close_zero_exit_v1.py',
    ...
    require(index['latest_completed_round']['round'] == 204 and not index['running_studies'], '前序完成状态不同')
    write_json(CONFIG, cfg, exclusive=True)
    index['running_studies'] = [{'round': 205, 'study': cfg['study_id'], 'status': 'FROZEN_NOT_STARTED', 'config': str(CONFIG.relative_to(ROOT))}]
    index['next_work'].update(registered=True, status='ADDITION_ONLY_RETURN_GATE_BATCH_FROZEN', source=cfg['rules'])
    write_json(path, index)
    print('第205轮八套仅加仓过滤已冻结，计划三十二条账户。', flush=True)
...
def verify():
    ...
    receipt = {'verified_at': now(), 'status': 'PASS_THIRTY_TWO_ADDITION_ONLY_ACCOUNTS_AND_UNFILTERED_INITIAL_ENTRIES',
