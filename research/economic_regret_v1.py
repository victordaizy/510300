...
from research.economic_regret_inputs_v1 import FEATURES, CN, economic_weights, economic_support, fit_economic_regret, economic_chinese_formula, EconomicExitController
from research.rearmed_cycle_exit_account_v1 import simulate_rearmed_exit
from research.simple_intraday_protection_v1 import make_rules

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_economic_regret_v1"
CONFIG = ROOT / "config/510300_economic_regret_v1.json"
...
PRIMARY = "ECONOMIC_REGRET_EXIT"
CONTROLS = {"REARM_RIDGE": (P32, "原平均继续收益学习退出"), "DIRECTIONAL_CONTINUATION_EXIT": (P79, "第79轮纯方向概率退出"),
...
def freeze():
    ...
    require(tests["exit_code"] == 0, "经济损失与成熟成交测试尚未通过")
    for item in old["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "原入场退出及模型来源身份改变")
    cfg.update(study_id="510300_ECONOMIC_REGRET_EXIT_V1", round=80, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        fit_intercept=True, solver="lbfgs", logistic_C=1., l1_ratio=0., tolerance=1e-8, maximum_iterations=1000, class_weight=None,
        model_loss="NORMALIZED_ABSOLUTE_CONTINUATION_ADVANTAGE_WEIGHTED_LOG_LOSS_L2", positive_class="ORIGINAL_CONTINUATION_TARGET_STRICTLY_POSITIVE", exit_score_threshold=.5, standardization_basis="ORIGINAL_CYCLE_EQUAL_WEIGHTS", score_is_calibrated_probability=False,
        feature_columns=FEATURES.copy(), feature_names=CN.copy(), planned_model_fits=114,
        fit_failure="NO_VIEW_MODEL_FIT_FAILED_KEEP_ORIGINAL_EXITS_NO_FALLBACK_SOLVER", rules="docs/510300_ECONOMIC_REGRET_EXIT_V1.md",
        samples=str((P31 / "all_reference_samples.parquet").relative_to(ROOT)), new_reference_accounts=0, source_budget_cny=0,
        previous_goal_turn_classification="PROGRESS_ROUND78_AND_79_COMPLETED_AND_DELIVERED", position_impact=0, goal_achieved=False,
    ...
    paths = [Path(__file__), ROOT / "research/economic_regret_inputs_v1.py", ROOT / "research/rearmed_cycle_exit_account_v1.py",
        ...
        ROOT / "tests/test_economic_regret_v1.py", ROOT / "tests/test_median_continuation_v1.py", OUT / "tests_receipt.json"]
    ...
    print("第80轮一个条件经济损失继续价值设置已冻结，尚未拟合或读取新账户收益。", flush=True)


def train_models(data, samples, originals, cfg):
    models, receipts, memberships = [], [], []
    chinese = ["# 每月经济损失继续价值模型：全部系数与中文计算规则", "", "只使用当时已完全自然结束的参考周期；没有模型不填预测数值。", ""]
    for original in originals:
        ...
        if eligible:
            memberships.extend({"fit_index": t, "cycle_id": int(r.cycle_id), "origin_index": int(r.origin_index), "exit_index": int(r.exit_index),
                "sample_weight": float(r.sample_weight), "economic_weight": float(r.economic_weight), "fit_status": status} for r in rows.itertuples())
        chinese += [f"## {record['fit_origin']}", "", f"已完成周期{len(ids)}个，合格状态{len(rows)}条。", ""]
        chinese += economic_chinese_formula(stored) + [""] if stored is not None else ["本月没有有效经济损失模型，继续执行原价格及时间退出。", ""]
    ...
    (OUT / "每月经济损失退出模型中文规则.md").write_text("\n".join(chinese) + "\n", encoding="utf-8")
    print(f"原成熟时点不变：经济损失拟合完成{sum(r['status']=='FIT_COMPLETE' for r in receipts)}次，未成熟或求解失败{sum(r['status']!='FIT_COMPLETE' for r in receipts)}次。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "经济损失冻结来源改变")
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], EconomicExitController(frame, models, cfg["confirmation_days"]))
            for saved_frame, column in [(ledger, "execution_reasons"), (decisions, "exit_reasons"), (cycles, "exit_reasons")]:
                saved_frame[column] = saved_frame[column].str.replace("连续两个收盘预测继续持有收益为负，学习条件请求退出", "连续两个收盘经济持有评分低于一半，学习条件请求退出", regex=False)
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "经济损失账户结算不符")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "经济损失退出违反次日可卖")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "经济错判损失退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "经济损失及对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新经济损失账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "ECONOMIC_REGRET_EXIT_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
