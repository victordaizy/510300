...
from research.shared_episode_continuation_inputs_v1 import FEATURES, CN, SIGNALS, TASK_FEATURES, calendar_episodes, grouped_samples, shared_training_rows, support_status, fit_shared_exit, SharedEpisodeExitController
from research.rearmed_cycle_exit_account_v1 import simulate_rearmed_exit
from research.simple_intraday_protection_v1 import make_rules

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_shared_episode_continuation_v1"
CONFIG = ROOT / "config/510300_shared_episode_continuation_v1.json"
...
PRIMARY = "SHARED_EPISODE_CONTINUATION_EXIT"
...
def freeze():
    ...
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "十项共同持仓时段共享与成熟成交测试尚未通过")
    cfg.pop("recent_cycles")
    cfg.pop("minimum_cycles")
    cfg.update(study_id="510300_SHARED_EPISODE_CONTINUATION_V1", round=108, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        fit_intercept=True, solver="svd", recent_groups=20, minimum_groups=10, required_tasks=SIGNALS.copy(), first_supported_origin="2015-03-02", missing_training_features="WHOLE_MONTH_NO_VIEW_PRESERVE_ALL_ORIGINAL_ROWS", model_loss="CALENDAR_EPISODE_EQUAL_THEN_CYCLE_EQUAL_THEN_STATE_EQUAL_WEIGHTED_RIDGE",
        feature_columns=FEATURES.copy(), feature_names=CN.copy(), added_feature_columns=TASK_FEATURES.copy(), dropped_feature_columns=[], planned_model_fits=138,
        fit_failure="NO_VIEW_MODEL_FIT_FAILED_KEEP_ORIGINAL_EXITS_NO_FALLBACK_SOLVER", rules="docs/510300_SHARED_EPISODE_CONTINUATION_V1.md",
        samples=str((P31 / "all_reference_samples.parquet").relative_to(ROOT)), new_reference_accounts=0, source_budget_cny=0,
        previous_goal_turn_classification="PROGRESS_ROUNDS106_TO107_COMPLETED_AND_DELIVERED", position_impact=0, goal_achieved=False,
    ...
    paths = [Path(__file__), ROOT / "research/shared_episode_continuation_inputs_v1.py", ROOT / "research/rearmed_cycle_exit_account_v1.py",
        ...
        ROOT / "tests/test_shared_episode_continuation_v1.py", ROOT / "tests/test_median_continuation_v1.py", OUT / "tests_receipt.json"]
    for period in ["evaluation", "earlier_diagnostic"]:
        for cost in cfg["costs"]:
            paths.extend(parent / period / cost / f"{model}_ledger.parquet" for model, (parent, name) in CONTROLS.items())
    paths.extend(P31 / "reference" / f"{signal}_ledger.parquet" for signal in SIGNALS)
    cfg["frozen_files"] = [{"path": str(p.relative_to(ROOT)), "sha256": digest(p)} for p in paths]
    write_json(CONFIG, cfg, exclusive=True)
    print("第108轮一个条件十项共同持仓时段共享继续价值设置已冻结，尚未拟合或读取新账户收益。", flush=True)
...
def train_models(data, samples, originals, cfg):
    ...
    for original in originals:
        ...
        rows, ids = shared_training_rows(samples, data.date.iloc[t], cfg)
        eligible = support_status(rows, ids, cfg)
        require(not len(rows) or rows.group_mature_date.le(data.date.iloc[t]).all(), "共同持仓时段尚未完全成熟")
        stored, failure = None, None
        status = "NO_VIEW_MINIMUM_MATURE_GROUPS_ROWS_OR_TASKS"
        ...
        if eligible and not missing_rows:
            try:
                stored = fit_shared_exit(rows, cfg)
                status = "FIT_COMPLETE"
        ...
        record = {"fit_index": t, "fit_origin": str(data.date.iloc[t].date()), "fit_time": data.date.iloc[t] + pd.Timedelta(hours=15, minutes=5),
            "status": status, "eligible_for_fit": eligible, "training_groups": ids, "training_group_count": len(ids), "training_cycle_count": int(rows.source_cycle_key.nunique()), "training_rows": len(rows),
            "latest_exit_index": int(rows.exit_index.max()) if len(rows) else None, "latest_group_mature_index": int(pd.DatetimeIndex(data.date).get_loc(rows.group_mature_date.max())) if len(rows) else None, "latest_group_mature_date": str(rows.group_mature_date.max().date()) if len(rows) else None, "latest_exit_date": str(rows.mature_date.max().date()) if len(rows) else None,
        ...
        receipts.append({k: v for k, v in record.items() if k not in ["model", "training_groups"]})
        if eligible:
            memberships.extend({"fit_index": t, "group_id": int(r.group_id), "signal": r.signal, "source_cycle_key": r.source_cycle_key, "cycle_id": int(r.cycle_id), "origin_index": int(r.origin_index), "exit_index": int(r.exit_index), "group_mature_date": r.group_mature_date,
    ...
    require(next(r["fit_origin"] for r in receipts if r["eligible_for_fit"]) == cfg["first_supported_origin"], "首次共享成熟支持时点与登记不同")
    ...
    print(f"原成熟时点不变：十项共同持仓时段共享拟合完成{sum(r['status']=='FIT_COMPLETE' for r in receipts)}次，未成熟或求解失败{sum(r['status']!='FIT_COMPLETE' for r in receipts)}次。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "十项共同持仓时段共享冻结来源改变")
    write_json(OUT / "RUN_STARTED.json", {"started_at": now(), "config_sha256": digest(CONFIG)}, exclusive=True)
    data = pd.read_parquet(ROOT / cfg["features"])
    dividends = normalize_dividends(pd.read_csv(ROOT / cfg["dividends"]))
    samples = pd.read_parquet(ROOT / cfg["samples"])
    ledgers = {signal: pd.read_parquet(P31 / "reference" / f"{signal}_ledger.parquet") for signal in SIGNALS}
    groups, assignments = calendar_episodes(ledgers, cfg["data_cutoff"])
    samples = grouped_samples(samples, assignments, groups)
    groups.to_parquet(OUT / "reference_calendar_groups.parquet", index=False)
    samples.to_parquet(OUT / "grouped_reference_samples.parquet", index=False)
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], SharedEpisodeExitController(frame, models, cfg["confirmation_days"]))
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "十项共同持仓时段共享账户结算不符")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "十项共同持仓时段共享退出违反次日可卖")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "十项共同持仓时段共享继续收益退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "十项共同持仓时段共享及对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新十项共同持仓时段共享账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "SHARED_EPISODE_CONTINUATION_EXIT_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
