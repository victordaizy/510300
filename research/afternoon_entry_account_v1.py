...
def simulate_afternoon_entry(data, dividends, config, cost, start, rule, spec, controller=None, preview=None, snapshots=None):
    ...
    records, decisions, cycles, afternoon_decisions = [], [], [], []
    ...
    for day in range(first, last + 1):
        ...
        after_open_shares = account.shares
        afternoon_fill_reference = None
        opening_execution = dict(execution)
        execution_clock, execution_origin = "OPEN", dates[day - 1]
        execution_reason = pending["exit_reasons"] if not terminal else "研究终点统一退出"
        if not terminal and not account.shares and preview is not None:
            signal_time = dates[day] + pd.Timedelta(hours=14, minutes=31)
            row = {"date": dates[day], "signal_time": signal_time, "entry_rearmed": entry_rearmed,
                   "days_since_exit": day - last_exit, "cash_at_signal": account.cash,
                   "requested_quantity": 0, "filled_quantity": 0, "entry_signal": False,
                   "factor_status": "NO_VIEW_NO_MINUTE_OBSERVATION", "execution_status": "NO_TRADE", "simulation_only": True}
            snapshot = snapshots.get(dates[day]) if snapshots is not None else None
            if not entry_rearmed:
                row["factor_status"] = "WAIT_PREVIOUS_ENTRY_CONDITION_TO_DISAPPEAR"
            elif day - last_exit < spec["cooldown"]:
                row["factor_status"] = "WAIT_ORIGINAL_EXIT_COOLDOWN"
            elif snapshot is not None and np.isfinite(snapshot["signal_price"]) and snapshot["signal_price"] > 0:
                require(snapshot["signal_label"] + pd.Timedelta(minutes=1) < signal_time, "下午信号柱尚未结束")
                row.update(preview(day, snapshot["signal_price"], signal_time))
                row["observation_label"] = snapshot["signal_label"]
                if row["entry_signal"]:
                    quantity = affordable_quantity(account.cash, fill_price(snapshot["signal_price"], 1, cost, config["tick"]), cost, config["lot"])
                    row["requested_quantity"] = quantity
                    px, vol = snapshot["execution_price"], snapshot["execution_volume"]
                    row.update(execution_label=snapshot["execution_label"], execution_volume=vol)
                    require(snapshot["execution_label"] - pd.Timedelta(minutes=1) > signal_time, "下午成交代理不晚于信号")
                    if quantity <= 0:
                        row["execution_status"] = "NO_ORDER_INSUFFICIENT_CASH_FOR_LOT"
                    elif not np.isfinite(px) or px <= 0:
                        row["execution_status"] = "UNFILLED_NO_EXECUTION_PRICE"
                    elif not np.isfinite(vol) or vol <= 0 or quantity > vol * config["minute_participation_cap"]:
                        row["execution_status"] = "UNFILLED_MINUTE_CAPACITY"
                    else:
                        require(execution["filled_quantity"] == 0 and account.shares == 0, "下午进入与开盘成交冲突")
                        before = account.value(px)
                        execution = execute_order(account, quantity, px, previous[day], distribution[day], day, cost, config)
                        row.update(execution_status=execution["status"], filled_quantity=execution["filled_quantity"],
                                   fill_price=execution["fill_price"], commission=execution["commission"], slippage_cost=execution["slippage_cost"])
                        execution_clock, execution_origin = "AFTERNOON_LATER_BAR_OPEN_PROXY", signal_time
                        execution_reason = "昨日完整及今日下午临时日内相对隔夜强弱均超过1，提前进入"
                        if execution["filled_quantity"]:
                            afternoon_fill_reference = px
                            cycle_number += 1
                            entry_rearmed, last_entry_mode = False, 1
                            cycle_dividends, pending_reasons = 0., []
                            cycle = {"cycle_id": cycle_number, "mode": 1, "entry_date": dates[day], "entry_index": day,
                                     "entry_origin": signal_time, "entry_quantity": account.shares,
                                     "entry_cost_cny": execution["notional"] + execution["commission"]}
                            peak_value = cycle["entry_cost_cny"]
            afternoon_decisions.append(row)
        if not terminal:
            for key, same in pay_events.get(day, []):
                if same:
                    value = account.receivables.pop(key, 0.)
        ...
        if afternoon_fill_reference is None:
            price_pnl = old_shares * (op[day] - previous_mark) + account.shares * (mark - op[day])
        else:
            price_pnl = old_shares * (op[day] - previous_mark) + after_open_shares * (afternoon_fill_reference - op[day]) + account.shares * (mark - afternoon_fill_reference)
        ...
        records.append({"date": dates[day], "open": op[day], "mark": mark, "mark_clock": "OPEN_TERMINAL" if terminal else "CLOSE",
                        ...
                        "accounting_error": error, "origin": execution_origin, "terminal_unliquidated": bool(terminal and account.shares),
                        "turnover": execution["notional"] / before, "cycle_id": cycle["cycle_id"] if cycle else None,
                        "mode": cycle["mode"] if cycle else 0, "execution_reasons": execution_reason, "execution_clock": execution_clock,
                        "opening_requested_quantity": opening_execution["requested_quantity"],
                        "opening_filled_quantity": opening_execution["filled_quantity"], "opening_execution_status": opening_execution["status"],
                        **execution})
    ...
    return pd.DataFrame(records), pd.DataFrame(decisions), pd.DataFrame(cycles), pd.DataFrame(afternoon_decisions)
