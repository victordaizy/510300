...
from research import forward_eps_guosen_layout_v2 as base
...
def get_paths(scope):
    ...
    out=ROOT/('reports/research/510300_forward_eps_'+scope+'_facts_v2')
    manifest=ROOT/('config/510300_forward_eps_'+scope+'_facts_v2_manifest.json')
...
def freeze(scope):
    ...
    inputs=[Path(__file__),ROOT/'research/forward_eps_guosen_history_v1.py',ROOT/'research/forward_eps_guosen_layout_v2.py',
            ROOT/'docs/510300_FORWARD_EPS_LAYOUT_CORRECTION_V2.md',ROOT/'tests/test_forward_eps_guosen_layout_v2.py',
    ...
    save(manifest,{'registered_at':now(),'scope':scope,'amendment':'仅选择独立年度标题行，排除说明性年份；接受有相同最新股本附注支持的每股收益行别名；保留原文金额单位的明确或未知状态。',
                   'source_parser_amended':True,'source_queue_and_costs_unchanged':True,'prior_records_preserved':True,
                   'files':[base.identity(p) for p in inputs]},exclusive=True)
    print('前瞻年度表头及EPS原文别名修正已登记',scope,flush=True)
...
def run(scope):
    ...
    if not df.empty:
        ...
        names={'report_id':'研报编号','ts_code':'证券代码','sec_name':'公司名称','institution':'预测机构','target_fiscal_year':'预测目标年度',
               'eps_value_exact':'预测每股收益_原文数值','eps_unit_as_reported':'原文单位','report_internal_date':'研报落款日','conservative_information_date':'较晚公开日期',
    ...
    result={'completed_at':now(),'status':'ABSOLUTE_YEAR_EPS_FACTS_EXTRACTED_WITH_EXPLICIT_GAPS','scope':scope,
            ...
            'new_accounts':0,'goal_achieved':False,'parser_version':'EXPLICIT_ANNUAL_HEADER_AND_EPS_LABEL_V2','strategy_thresholds_changed':False}
