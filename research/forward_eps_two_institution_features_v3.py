...
OUT = ROOT / "reports/research/510300_forward_eps_two_institution_features_v3"
SOURCES = {
    "guosen": {
    ...
    "soochow": {
        ...
        "facts": "510300_forward_eps_soochow_facts_v4",
    },
}
...
def load_reports(source: str) -> tuple[dict, list[Path]]:
    ...
    if source == "soochow":
        ...
        if verified["status"] != "PASS_ALL_V3_FACTS_PRESERVED_NEW_RAW_ROWS_YEARS_AND_CLOCKS":
            raise RuntimeError("东吴完整第四版事实尚未完成保存核对")
    report_map = {}
...
def run() -> None:
    ...
    result = {"study_id": "510300_FORWARD_EPS_TWO_INSTITUTION_FEATURES_V3", "completed_at": now(),
