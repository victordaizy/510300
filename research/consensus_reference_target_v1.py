...
OUT = ROOT / "reports/research/510300_consensus_reference_target_v1"
CONFIG = ROOT / "config/510300_consensus_reference_target_v1.json"
...
def freeze():
    require(not CONFIG.exists(), "共同目标已经冻结")
    old_path = ROOT / "config/510300_episode_trend_reference_v1.json"
    ...
    for key in ["features", "dividends"]:
        require(digest(ROOT / cfg[key]) == old_hashes[str(Path(cfg[key]))], "共同目标行情或分红不再等于既有来源")
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 4, "共同目标四项必要测试未通过")
    cfg.update(study_id="510300_CONSENSUS_REFERENCE_TARGET_V1", round=142, primary=PRIMARY, candidate_configurations=1,
        decision_clock="15:05:00", combination="MINIMUM_OF_TWO_KNOWN_PARENT_TARGETS", registered_at=now(), new_model_fits=0, new_reference_accounts=0,
        ...
        rules="docs/510300_CONSENSUS_REFERENCE_TARGET_V1.md", source_budget_cny=0,
        independent_validation="NOT_ESTABLISHED", goal_achieved=False, position_impact=0,
        previous_goal_turn_classification="PROGRESS_ROUND141_VERIFIED_AND_CLOSED_COST_SAVINGS_INSUFFICIENT")
    paths = [Path(__file__), old_path, ROOT / "config/510300_vintage_reference_risk_v1.json",
        ROOT / "research/consensus_reference_target_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ROOT / "research/adaptive_allocation_v1.py", ROOT / "research/intraday_overnight_increment_v1.py",
        ROOT / "tests/test_consensus_reference_target_v1.py", OUT / "tests_receipt.json",
        ROOT / "config/510300_research_authority_v6.json", ROOT / cfg["features"], ROOT / cfg["dividends"],
        ROOT / cfg["rules"], ROOT / "docs/510300_CONSENSUS_REFERENCE_TARGET_NEXT_20260909.md",
    ...
    print("第142轮共同目标已冻结，尚未生成新目标或账户。", flush=True)


def run():
    cfg = json.loads(CONFIG.read_text(encoding="utf-8"))
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "共同目标冻结来源改变")
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        factor_frames, summaries = consensus_target_frames(frame, parents_by_cost, cfg, start)
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "共同目标账户财富或终点清仓不符")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "两套正目标共同确认持仓"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(ledger.date)), "共同目标与保存对照日历不同")
            ...
            print(f"{period}／{cost_id}：一条新共同目标账户及三条保存对照完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "CONSENSUS_REFERENCE_TARGET_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
    ...
    print(json.dumps({"主历史": primary, "较早历史": [m for m in earlier if m["model"] == PRIMARY],
        "账户": coverage, "共同目标": target_summaries, "核心耗时": result["run_seconds"]}, ensure_ascii=False), flush=True)
