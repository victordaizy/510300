...
from research.three_policy_min_variance_inputs_v1 import budget_frame
...
OUT = ROOT / "reports/research/510300_three_policy_min_variance_v1"
CONFIG = ROOT / "config/510300_three_policy_min_variance_v1.json"
...
PRIMARY = "THREE_POLICY_MIN_VARIANCE"
NAME = "三类既有信号的非负最小方差预算"
CONTROLS = {"PANIC_LEARNED_HALF": "原急跌与学习各半", "REARM_RIDGE": "原学习退出及等待新机会", "PANIC_ONLY": "原急跌回升单独策略", "BUY_HOLD": "买入持有", "TWO_POLICY_MIN_VARIANCE": "第82轮两策略最小方差", "AFTER_COMPRESSION_BREAKOUT": "原压缩后实际突破"}


def control_folder(model):
    return P82 if model=="TWO_POLICY_MIN_VARIANCE" else P54 if model=="AFTER_COMPRESSION_BREAKOUT" else P46



def freeze():
    require(not CONFIG.exists(), "三策略风险预算已登记，不能重复冻结")
    ...
    cfg.update(study_id="510300_THREE_POLICY_MIN_VARIANCE_V1", round=88, objective="MINIMIZE_THREE_REFERENCE_FULL_CALENDAR_SAMPLE_VARIANCE", registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        risk_window=242, risk_clock="MONTH_FIRST_COMPLETE_CLOSE_NEXT_OPEN", state_cost="BASE", initial_budgets=[1/3, 1/3, 1/3],
        missing_or_zero_risk="NO_VIEW_KEEP_PREVIOUS_BUDGET", singular_covariance="NO_VIEW_KEEP_PREVIOUS_BUDGET", solver="ENUMERATE_SEVEN_ACTIVE_FACES_OF_ONE_CONVEX_OBJECTIVE", numerical_rank_tolerance="EPS_TIMES_3_TIMES_MAX_SCALED_EIGENVALUE_OR_ONE", rules="docs/510300_THREE_POLICY_MIN_VARIANCE_V1.md",
        new_model_fits=0, new_reference_accounts=0, previous_goal_turn_classification="PROGRESS_ROUND86_AND_87_COMPLETED_AND_DELIVERED",
    ...
    paths = [Path(__file__), ROOT / "research/three_policy_min_variance_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ...
        ROOT / "research/simple_signal_blend_v1.py", ROOT / cfg["features"], ROOT / cfg["dividends"], ROOT / cfg["rules"], ROOT / "docs/510300_TWO_POLICY_RISK_BUDGET_V1.md", ROOT / "docs/510300_COMPRESSION_CONFIRMED_ENTRY_V1.md", ROOT / "config/510300_compression_confirmed_entry_v1.json",
        ROOT / "tests/test_three_policy_min_variance_v1.py", OUT / "tests_receipt.json", ROOT / "config/510300_research_authority_v6.json"]
    for period in ["evaluation", "earlier_diagnostic"]:
        paths.extend([P46 / f"{period}_states.parquet", panic_folder(period) / "BASE/V6_PANIC_RECOVERY_decisions.parquet", P32 / period / "BASE/REARM_RIDGE_decisions.parquet", P54 / period / "BASE/AFTER_COMPRESSION_BREAKOUT_decisions.parquet"])
        for cost in cfg["costs"]:
            paths.extend(control_folder(model) / period / cost / f"{model}_ledger.parquet" for model in CONTROLS)
    cfg["frozen_files"] = [{"path": str(p.relative_to(ROOT)), "sha256": digest(p)} for p in paths]
    write_json(CONFIG, cfg, exclusive=True)
    print("第88轮单项非负最小方差预算已冻结，尚无新策略收益。", flush=True)
...
def run():
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        breakout_decisions=pd.read_parquet(P54 / period / "BASE/AFTER_COMPRESSION_BREAKOUT_decisions.parquet")
        require(not breakout_decisions.origin.duplicated().any(), "压缩突破交易意向日期重复")
        require(pd.DatetimeIndex(breakout_decisions.origin).equals(pd.DatetimeIndex(frame.date.iloc[first-1:-1])), "压缩突破意向缺失完整判断日")
        require(pd.DatetimeIndex(breakout_decisions.execution_date).equals(pd.DatetimeIndex(frame.date.iloc[first:])), "压缩突破意向没有对应下一开盘")
        mapped=np.column_stack([mapped,decision_state(frame,breakout_decisions)])
        reference_returns = np.full((len(frame), 3), np.nan)
        for column, model in enumerate(["PANIC_ONLY", "REARM_RIDGE", "AFTER_COMPRESSION_BREAKOUT"]):
        ...
        factors = budget_frame(frame.date, reference_returns, mapped, first, cfg["risk_window"])
        for i,name in enumerate(["panic","learned","breakout"]): factors[name+"_reference_return"]=reference_returns[:,i]
    ...
    write_json(OUT / "result.json", {"study_id": cfg["study_id"], "completed_at": now(), "status": "THREE_POLICY_MIN_VARIANCE_ACCOUNTS_COMPLETE",
