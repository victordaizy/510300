...
from research.profit_drawdown_interaction_inputs_v1 import FEATURES, CN, fit_interaction_exit, InteractionExitController, INTERACTION, attach_interaction
from research.rearmed_cycle_exit_account_v1 import simulate_rearmed_exit
from research.simple_intraday_protection_v1 import make_rules

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_profit_drawdown_interaction_v1"
CONFIG = ROOT / "config/510300_profit_drawdown_interaction_v1.json"
...
PRIMARY = "PROFIT_DRAWDOWN_INTERACTION_EXIT"
...
def freeze():
    ...
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "九项浮盈回撤交互与成熟成交测试尚未通过")
    cfg.update(study_id="510300_PROFIT_DRAWDOWN_INTERACTION_V1", round=102, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        fit_intercept=True, solver="svd", interaction_definition="POSITIVE_PART_OF_KNOWN_CYCLE_RETURN_TIMES_KNOWN_CYCLE_DRAWDOWN", missing_training_features="WHOLE_MONTH_NO_VIEW_PRESERVE_ALL_ORIGINAL_ROWS", model_loss="CYCLE_EQUAL_WEIGHTED_SQUARE_ERROR_WITH_ORIGINAL_RIDGE_PENALTY",
        feature_columns=FEATURES.copy(), feature_names=CN.copy(), added_feature_columns=[INTERACTION], dropped_feature_columns=[], planned_model_fits=114,
        fit_failure="NO_VIEW_MODEL_FIT_FAILED_KEEP_ORIGINAL_EXITS_NO_FALLBACK_SOLVER", rules="docs/510300_PROFIT_DRAWDOWN_INTERACTION_V1.md",
        samples=str((P31 / "all_reference_samples.parquet").relative_to(ROOT)), new_reference_accounts=0, source_budget_cny=0,
        previous_goal_turn_classification="PROGRESS_ROUNDS99_TO101_COMPLETED_AND_DELIVERED", position_impact=0, goal_achieved=False,
    ...
    paths = [Path(__file__), ROOT / "research/profit_drawdown_interaction_inputs_v1.py", ROOT / "research/rearmed_cycle_exit_account_v1.py",
        ...
        ROOT / "tests/test_profit_drawdown_interaction_v1.py", ROOT / "tests/test_median_continuation_v1.py", OUT / "tests_receipt.json"]
    ...
    print("第102轮一个条件九项浮盈回撤交互继续价值设置已冻结，尚未拟合或读取新账户收益。", flush=True)
...
def train_models(data, samples, originals, cfg):
    ...
    for original in originals:
        ...
        if eligible and not missing_rows:
            try:
                stored = fit_interaction_exit(rows, cfg)
                status = "FIT_COMPLETE"
    ...
    print(f"原成熟时点不变：九项浮盈回撤交互拟合完成{sum(r['status']=='FIT_COMPLETE' for r in receipts)}次，未成熟或求解失败{sum(r['status']!='FIT_COMPLETE' for r in receipts)}次。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "九项浮盈回撤交互冻结来源改变")
    ...
    samples = attach_interaction(samples)
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], InteractionExitController(frame, models, cfg["confirmation_days"]))
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "九项浮盈回撤交互账户结算不符")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "九项浮盈回撤交互退出违反次日可卖")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "九项浮盈回撤交互继续收益退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "九项浮盈回撤交互及对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新九项浮盈回撤交互账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "PROFIT_DRAWDOWN_INTERACTION_EXIT_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
