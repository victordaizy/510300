...
def simulate_afternoon_exit(data, dividends, config, cost, start, rule, spec, controller=None, preview=None, snapshots=None):
    ...
    for day in range(first, last + 1):
        ...
        if not terminal and account.shares and preview is not None:
            signal_time = dates[day] + pd.Timedelta(hours=14, minutes=31)
            row = {"date": dates[day], "signal_time": signal_time, "cycle_id": cycle["cycle_id"],
                   "requested_quantity": 0, "filled_quantity": 0, "learning_status": "NO_VIEW_NO_MINUTE_OBSERVATION",
                   "learned_exit_requested": False, "execution_status": "NO_TRADE", "simulation_only": True}
            snapshot = snapshots.get(dates[day]) if snapshots is not None else None
            if account.sellable(day) != account.shares:
                row["learning_status"] = "NOT_ALLOWED_BUY_DAY_T_PLUS_ONE"
            elif pending_reasons:
                row["learning_status"] = "EXISTING_EXIT_WAITS_FOR_NEXT_OPEN"
            elif snapshot is not None and np.isfinite(snapshot["signal_price"]) and snapshot["signal_price"] > 0:
                current_value = account.shares * snapshot["signal_price"] + cycle_dividends
                row.update(preview(day, cycle, current_value, max(peak_value, current_value), snapshot["signal_price"], signal_time))
                row["observation_label"] = snapshot["signal_label"]
                require(snapshot["signal_label"] + pd.Timedelta(minutes=1) < signal_time, "下午决策尚未等到所用分钟柱完成")
                if row["learned_exit_requested"]:
                    pending_reasons = ["昨日收盘和今日下午两次模型判断继续持有收益为负，提前请求全部退出"]
                    row["requested_quantity"] = -account.shares
                    ...
                    require(snapshot["execution_label"] - pd.Timedelta(minutes=1) > signal_time, "下午执行代理价格早于信号形成")
                    if not np.isfinite(px) or px <= 0:
                        row["execution_status"] = "UNFILLED_NO_EXECUTION_PRICE"
                    elif not np.isfinite(vol) or vol <= 0 or account.shares > vol * config["minute_participation_cap"]:
                        row["execution_status"] = "UNFILLED_MINUTE_CAPACITY"
                    else:
                        require(execution["filled_quantity"] == 0, "下午退出与开盘已有成交冲突")
                        execution = execute_order(account, -account.shares, px, previous[day], distribution[day], day, cost, config)
                        ...
                        execution_clock, execution_origin, execution_reason = "AFTERNOON_LATER_BAR_OPEN_PROXY", signal_time, "；".join(pending_reasons)
                        if execution["filled_quantity"]:
                            require(account.shares == 0, "下午退出必须完整卖出旧份额")
                            afternoon_fill_reference = px
                            proceeds = execution["notional"] - execution["commission"]
                            cycle.update(exit_date=dates[day], exit_origin=signal_time, exit_reasons=execution_reason,
                                         holding_intervals=day - cycle["entry_index"], dividend_cny=cycle_dividends,
                                         net_profit_cny=proceeds + cycle_dividends - cycle["entry_cost_cny"])
                            cycles.append(cycle)
                            cycle, last_exit, pending_reasons, peak_value, cycle_dividends = None, day, [], np.nan, 0.
