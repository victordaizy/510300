...
MANIFEST=ROOT/'config/510300_forward_eps_exposure_attribution_v1_manifest.json'
...
def market_and_exposures(ledger,price,initial_capital):
    ...
    old=ledger.shares_before.to_numpy(float);current=ledger.shares_after.to_numpy(float)
...
def freeze():
    paths=[Path(__file__),ROOT/'docs/510300_FORWARD_EPS_EXPOSURE_ATTRIBUTION_V1.md',
           ROOT/'tests/test_forward_eps_exposure_attribution_v1.py',
...
def run(scope):
    ...
    out=ROOT/('reports/research/510300_forward_eps_exposure_attribution_v1_'+scope)
