...
def rows_with_headers(text: str, field: str) -> list[dict]:
    ...
    if field == "eps":
        ...
        pattern = r"(?<![一-龥])(?P<label>归属于母公司的净利润|归属母公司净利润|归母净利润|净利润)(?:\s*\((?P<unit>人民币百万元|百万元)\))?(?=\s)"
    else:
...
def appendices(pdf_path: Path, pages: list[str]) -> tuple[list[dict], list[dict], list[dict]]:
    eps, profits, pe = [], [], []
    selected = [i for i, text in enumerate(pages) if re.search(r"(?:财务预测表|盈利预测表)", compact(text))]
    if not selected:
...
def parse(pages: list[str], metadata: dict, row: dict, pdf_path: Path) -> dict:
    try:
        kept = original.parse(pages, metadata, row)
        kept["adapter"] = "V1_SUCCESS_PRESERVED_EXACTLY"
        return kept
    ...
    return {"facts": facts, "table": table, "report_internal_date": internal,
            ...
            "adapter": "LEGACY_SUMMARY_OR_COORDINATE_APPENDIX_V2", "previous_v1_reason": v1_reason}
