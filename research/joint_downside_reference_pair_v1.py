"""第137轮按两套已保存收益合成后的下行幅度分配预算。"""
import json
import time
from pathlib import Path
import numpy as np
import pandas as pd
from research.joint_downside_reference_pair_inputs_v1 import joint_downside_reference_frames, PRIMARY, MODELS
PARENT = MODELS[0]
...
OUT = ROOT / "reports/research/510300_joint_downside_reference_pair_v1"
CONFIG = ROOT / "config/510300_joint_downside_reference_pair_v1.json"
...
def freeze():
    require(not CONFIG.exists(), "月度共同下行预算已经冻结")
    old_path = ROOT / "config/510300_covariance_reference_pair_v1.json"
    ...
    for key in ["features", "dividends"]:
        require(digest(ROOT / cfg[key]) == old_hashes[str(Path(cfg[key]))], "共同下行预算行情或分红不再等于已核对来源")
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "共同下行预算六项必要测试未通过")
    cfg.update(study_id="510300_JOINT_DOWNSIDE_REFERENCE_PAIR_V1", round=137, primary=PRIMARY, candidate_configurations=1,
        risk_window=242, downside_benchmark=0., risk_cost="BASE", initial_budgets=[.5, .5], registered_at=now(), new_model_fits=0, new_reference_accounts=0,
        ...
        rules="docs/510300_JOINT_DOWNSIDE_REFERENCE_PAIR_V1.md", input_receipt="reports/research/510300_covariance_reference_pair_v1/saved_verification_receipt.json",
    ...
    paths = [Path(__file__), old_path, ROOT / "research/joint_downside_reference_pair_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ROOT / "research/adaptive_allocation_v1.py", ROOT / "research/intraday_overnight_increment_v1.py", ROOT / "tests/test_joint_downside_reference_pair_v1.py",
    ...
    paths.extend([ROOT / "research/recent_reference_selection_inputs_v1.py", ROOT / "research/two_policy_min_variance_inputs_v1.py",
        ROOT / "docs/510300_JOINT_DOWNSIDE_REFERENCE_PAIR_NEXT_20260909.md"])
    ...
    print("第137轮月度共同下行预算已冻结，尚未生成新目标或账户。", flush=True)


def run():
    cfg = json.loads(CONFIG.read_text(encoding="utf-8"))
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "共同下行预算冻结来源改变")
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        factor_frames, summaries = joint_downside_reference_frames(frame, base_ledgers, parents_by_cost, cfg, start)
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "共同下行预算账户财富或终点清仓不符")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "两套参考合成收益的月度下行预算"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(ledger.date)), "共同下行预算与保存对照日历不同")
            ...
            print(f"{period}／{cost_id}：一条新共同下行预算账户及五条保存对照完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "JOINT_DOWNSIDE_REFERENCE_PAIR_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
    ...
    print(json.dumps({"主历史": primary, "较早历史": [m for m in earlier if m["model"] == PRIMARY],
        "账户": coverage, "共同下行预算": target_summaries, "核心耗时": result["run_seconds"]}, ensure_ascii=False), flush=True)
