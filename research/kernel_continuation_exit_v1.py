...
from research.kernel_continuation_exit_inputs_v1 import FEATURES, CN, fit_kernel_exit, kernel_chinese_formula, KernelExitController
from research.rearmed_cycle_exit_account_v1 import simulate_rearmed_exit
from research.simple_intraday_protection_v1 import make_rules

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_kernel_continuation_exit_v1"
CONFIG = ROOT / "config/510300_kernel_continuation_exit_v1.json"
...
PRIMARY = "KERNEL_CONTINUATION_EXIT"
...
def freeze():
    ...
    require(tests["exit_code"] == 0, "八项核状态与成熟成交测试尚未通过")
    for item in old["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "原入场退出及模型来源身份改变")
    cfg.update(study_id="510300_KERNEL_CONTINUATION_EXIT_V1", round=87, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        fit_intercept=True, solver="CHOLESKY_WEIGHTED_RBF_WITH_UNPENALIZED_INTERCEPT", kernel_gamma=0.125, model_loss="CYCLE_EQUAL_WEIGHTED_SQUARE_ERROR_WITH_RKHS_NORM_PENALTY",
        feature_columns=FEATURES.copy(), feature_names=CN.copy(), dropped_feature_columns=[], added_feature_columns=[], planned_model_fits=114,
        fit_failure="NO_VIEW_MODEL_FIT_FAILED_KEEP_ORIGINAL_EXITS_NO_FALLBACK_SOLVER", rules="docs/510300_KERNEL_CONTINUATION_EXIT_V1.md",
        samples=str((P31 / "all_reference_samples.parquet").relative_to(ROOT)), new_reference_accounts=0, source_budget_cny=0,
        previous_goal_turn_classification="PROGRESS_ROUND86_COMPLETED_AND_DELIVERED", position_impact=0, goal_achieved=False,
    ...
    paths = [Path(__file__), ROOT / "research/kernel_continuation_exit_inputs_v1.py", ROOT / "research/rearmed_cycle_exit_account_v1.py",
        ...
        ROOT / "tests/test_kernel_continuation_exit_v1.py", ROOT / "tests/test_median_continuation_v1.py", OUT / "tests_receipt.json"]
    ...
    print("第87轮一个条件八项核状态继续价值设置已冻结，尚未拟合或读取新账户收益。", flush=True)


def train_models(data, samples, originals, cfg):
    models, receipts, memberships = [], [], []
    chinese = ["# 每月八项核状态继续价值模型：全部系数与中文计算规则", "", "只使用当时已完全自然结束的参考周期；没有模型不填预测数值。", ""]
    for original in originals:
        ...
        if eligible:
            try:
                stored = fit_kernel_exit(rows, cfg)
                status = "FIT_COMPLETE"
        ...
        chinese += kernel_chinese_formula(stored) + [""] if stored is not None else ["本月没有有效八项核状态模型，继续执行原价格及时间退出。", ""]
    state_rows=[]
    for record in models:
        model=record["model"]
        if model is not None:
            for origin, cycle_id, center, coefficient in zip(model["training_origin_indexes"],model["training_cycle_ids"],model["centers"],model["dual_coefficients"],strict=True):
                state_rows.append({"模型生效日":record["fit_origin"],"参考周期编号":cycle_id,"参考状态日":str(data.date.iloc[origin].date()),
                    "训练状态系数":coefficient,**{name+"标准化数值":value for name,value in zip(CN,center,strict=True)}})
    pd.DataFrame(state_rows).to_csv(OUT/"全部月度参考状态及系数.csv",index=False,encoding="utf-8-sig")
    ...
    (OUT / "每月八项核状态退出模型中文规则.md").write_text("\n".join(chinese) + "\n", encoding="utf-8")
    print(f"原成熟时点不变：八项核状态拟合完成{sum(r['status']=='FIT_COMPLETE' for r in receipts)}次，未成熟或求解失败{sum(r['status']!='FIT_COMPLETE' for r in receipts)}次。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "八项核状态冻结来源改变")
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], KernelExitController(frame, models, cfg["confirmation_days"]))
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "八项核状态账户结算不符")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "八项核状态退出违反次日可卖")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "八项核状态继续收益退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "八项核状态及对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新八项核状态账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "KERNEL_CONTINUATION_EXIT_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
