...
import numpy as np
import pandas as pd
from research.trend_coherence_blend_inputs_v1 import trend_coherence_frames, PRIMARY, MODELS
PARENT = MODELS[0]
...
OUT = ROOT / "reports/research/510300_trend_coherence_blend_v1"
CONFIG = ROOT / "config/510300_trend_coherence_blend_v1.json"
...
P144 = ROOT / "reports/research/510300_continuation_strength_blend_v1"
P32 = ROOT / "reports/research/510300_rearmed_session_exit_v1"
CONTROLS = {MODELS[0]: (P131, "第131轮普通波动乘数策略"), MODELS[1]: (P143, "第143轮趋势波动连续预算"),
    "CONTINUATION_STRENGTH_BLEND": (P144, "第144轮继续持有预测预算"), "BUY_HOLD": (P32, "买入持有")}


def freeze():
    require(not CONFIG.exists(), "趋势连贯预算已经冻结")
    old_path = ROOT / "config/510300_continuation_strength_blend_v1.json"
    ...
    for key in ["features", "dividends"]:
        require(digest(ROOT / cfg[key]) == old_hashes[str(Path(cfg[key]))], "趋势连贯预算行情或分红不再等于既有来源")
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 5, "趋势连贯预算五项必要测试未通过")
    cfg.update(study_id="510300_TREND_COHERENCE_BLEND_V1", round=145, primary=PRIMARY, candidate_configurations=1,
        decision_clock="15:05:00", combination="POSITIVE_LOG_TREND_R_SQUARED_BUDGET", trend_window=120, registered_at=now(), new_model_fits=0, new_reference_accounts=0,
        ...
        rules="docs/510300_TREND_COHERENCE_BLEND_V1.md", source_budget_cny=0,
        independent_validation="NOT_ESTABLISHED", goal_achieved=False, position_impact=0,
        previous_goal_turn_classification="PROGRESS_ROUND144_COMPLETED_CLOSED_CURRENT_BEST143_FULL_GOAL_NOT_MET")
    paths = [Path(__file__), old_path, ROOT / "config/510300_vintage_reference_risk_v1.json",
        ROOT / "research/trend_coherence_blend_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ROOT / "research/adaptive_allocation_v1.py", ROOT / "research/intraday_overnight_increment_v1.py",
        ROOT / "tests/test_trend_coherence_blend_v1.py", OUT / "tests_receipt.json",
        ROOT / "config/510300_research_authority_v6.json", ROOT / cfg["features"], ROOT / cfg["dividends"],
        ROOT / cfg["rules"], ROOT / "docs/510300_TREND_COHERENCE_BLEND_NEXT_20260909.md",
        P131 / "saved_verification_receipt.json", P143 / "saved_verification_receipt.json", P143 / "saved_diagnostic_receipt.json"]
    ...
    from research.trend_coherence_blend_inputs_v1 import rolling_trend_coherence
    ...
    for period in ["evaluation", "earlier_diagnostic"]:
        ...
        for cost in cfg["costs"]:
            for model, folder in [(MODELS[0], P131), (MODELS[1], P143)]:
                ...
                require(digest(path) == old_hashes[str(path.relative_to(ROOT))], "趋势连贯预算父来源不再等于144已绑定文件")
    ...
    print("第145轮趋势连贯预算已冻结，尚未生成新目标或账户。", flush=True)


def run():
    cfg = json.loads(CONFIG.read_text(encoding="utf-8"))
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "趋势连贯预算冻结来源改变")
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        factor_frames, summaries = trend_coherence_frames(frame, parents_by_cost, cfg, start)
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "趋势连贯预算账户财富或终点清仓不符")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "按上升趋势连贯程度分配来源"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(ledger.date)), "趋势连贯预算与保存对照日历不同")
            ...
            print(f"{period}／{cost_id}：一条新趋势连贯预算账户及四条保存对照完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "TREND_COHERENCE_BLEND_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
    ...
    print(json.dumps({"主历史": primary, "较早历史": [m for m in earlier if m["model"] == PRIMARY],
        "账户": coverage, "趋势连贯预算": target_summaries, "核心耗时": result["run_seconds"]}, ensure_ascii=False), flush=True)
