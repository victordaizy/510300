...
OUT = ROOT / "reports/research/510300_two_policy_risk_budget_v1"
CONFIG = ROOT / "config/510300_two_policy_risk_budget_v1.json"
P46 = ROOT / "reports/research/510300_panic_learned_equal_blend_v1"
PRIMARY = "TWO_POLICY_RISK_BUDGET"
NAME = "两条原策略按已实现风险分配预算"
CONTROLS = {"PANIC_LEARNED_HALF": "原急跌与学习各半", "REARM_RIDGE": "原学习退出及等待新机会", "PANIC_ONLY": "原急跌回升单独策略", "BUY_HOLD": "买入持有"}
...
def freeze():
    ...
    cfg.update(study_id="510300_TWO_POLICY_RISK_BUDGET_V1", round=76, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        risk_window=242, risk_clock="MONTH_FIRST_COMPLETE_CLOSE_NEXT_OPEN", state_cost="BASE", initial_budgets=[.5, .5],
        missing_or_zero_risk="NO_VIEW_KEEP_PREVIOUS_BUDGET", rules="docs/510300_TWO_POLICY_RISK_BUDGET_V1.md",
        new_model_fits=0, new_reference_accounts=0, previous_goal_turn_classification="PROGRESS_ROUND75_COMPLETED_AND_DELIVERED",
    ...
    paths = [Path(__file__), ROOT / "research/two_policy_risk_budget_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ...
        ROOT / "tests/test_two_policy_risk_budget_v1.py", OUT / "tests_receipt.json", ROOT / "config/510300_research_authority_v6.json"]
    ...
    print("第76轮一个242日月度风险预算设置已冻结，尚无新策略收益。", flush=True)
...
def run():
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            print(f"{period}／{cost_id}：风险预算新账户和四个保存对照完成。", flush=True)
    ...
    write_json(OUT / "result.json", {"study_id": cfg["study_id"], "completed_at": now(), "status": "TWO_POLICY_RISK_BUDGET_ACCOUNTS_COMPLETE",
