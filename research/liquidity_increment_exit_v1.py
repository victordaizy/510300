...
from research.liquidity_increment_exit_inputs_v1 import FEATURES, CN, fit_liquidity_exit, liquidity_chinese_formula, LiquidityExitController, LIQUIDITY, liquidity_features, attach_sample_liquidity
from research.rearmed_cycle_exit_account_v1 import simulate_rearmed_exit
from research.simple_intraday_protection_v1 import make_rules

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_liquidity_increment_exit_v1"
CONFIG = ROOT / "config/510300_liquidity_increment_exit_v1.json"
...
PRIMARY = "LIQUIDITY_INCREMENT_EXIT"
...
def freeze():
    ...
    require(tests["exit_code"] == 0, "九项流动性与成熟成交测试尚未通过")
    for item in old["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "原入场退出及模型来源身份改变")
    cfg.update(study_id="510300_LIQUIDITY_INCREMENT_EXIT_V1", round=86, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        fit_intercept=True, solver="svd", liquidity_window=20, amount_unit="CNY", amount_scale_cny=100000000., liquidity_transform="LOG_OF_MEAN_ABS_TOTAL_SIMPLE_RETURN_PER_100M_CNY", missing_training_features="WHOLE_MONTH_NO_VIEW_PRESERVE_ALL_ORIGINAL_ROWS", model_loss="CYCLE_EQUAL_WEIGHTED_SQUARE_ERROR_WITH_ORIGINAL_RIDGE_PENALTY",
        feature_columns=FEATURES.copy(), feature_names=CN.copy(), added_feature_columns=[LIQUIDITY], dropped_feature_columns=[], planned_model_fits=114,
        fit_failure="NO_VIEW_MODEL_FIT_FAILED_KEEP_ORIGINAL_EXITS_NO_FALLBACK_SOLVER", rules="docs/510300_LIQUIDITY_INCREMENT_EXIT_V1.md",
        samples=str((P31 / "all_reference_samples.parquet").relative_to(ROOT)), new_reference_accounts=0, source_budget_cny=0,
        previous_goal_turn_classification="PROGRESS_ROUND85_COMPLETED_AND_DELIVERED", position_impact=0, goal_achieved=False,
    ...
    paths = [Path(__file__), ROOT / "research/liquidity_increment_exit_inputs_v1.py", ROOT / "research/rearmed_cycle_exit_account_v1.py",
        ...
        ROOT / "tests/test_liquidity_increment_exit_v1.py", ROOT / "tests/test_median_continuation_v1.py", OUT / "tests_receipt.json"]
    for period in ["evaluation", "earlier_diagnostic"]:
        for cost in cfg["costs"]:
            paths.extend(parent / period / cost / f"{model}_ledger.parquet" for model, (parent, name) in CONTROLS.items())
    source = pd.read_parquet(ROOT / cfg["features"])
    factors = liquidity_features(source)
    source[LIQUIDITY] = factors[LIQUIDITY]
    original_samples = pd.read_parquet(ROOT / cfg["samples"])
    original_samples = original_samples[original_samples.signal.eq("D60_INTRA")].copy()
    extended = attach_sample_liquidity(original_samples, source)
    write_json(OUT / "input_receipt.json", {"checked_at": now(), "amount_unit_counts": source.amount_unit.value_counts(dropna=False).to_dict(),
        "source_rows": len(source), "feature_available_rows": int(np.isfinite(factors[LIQUIDITY]).sum()), "reference_sample_rows": len(extended),
        "reference_sample_missing_liquidity": int(extended[LIQUIDITY].isna().sum()), "unit_multiplier_applied": 1,
        "only_past_and_current_close": True, "no_future_liquidity_outcomes_reused": True}, exclusive=True)
    paths.append(OUT / "input_receipt.json")
    cfg["frozen_files"] = [{"path": str(p.relative_to(ROOT)), "sha256": digest(p)} for p in paths]
    write_json(CONFIG, cfg, exclusive=True)
    print("第86轮一个条件九项流动性继续价值设置已冻结，尚未拟合或读取新账户收益。", flush=True)


def train_models(data, samples, originals, cfg):
    models, receipts, memberships = [], [], []
    chinese = ["# 每月九项流动性继续价值模型：全部系数与中文计算规则", "", "只使用当时已完全自然结束的参考周期；没有模型不填预测数值。", ""]
    for original in originals:
        ...
        if eligible and not missing_rows:
            try:
                stored = fit_liquidity_exit(rows, cfg)
                status = "FIT_COMPLETE"
        ...
        chinese += liquidity_chinese_formula(stored) + [""] if stored is not None else ["本月没有有效九项流动性模型，继续执行原价格及时间退出。", ""]
    ...
    (OUT / "每月九项流动性退出模型中文规则.md").write_text("\n".join(chinese) + "\n", encoding="utf-8")
    print(f"原成熟时点不变：九项流动性拟合完成{sum(r['status']=='FIT_COMPLETE' for r in receipts)}次，未成熟或求解失败{sum(r['status']!='FIT_COMPLETE' for r in receipts)}次。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "九项流动性冻结来源改变")
    write_json(OUT / "RUN_STARTED.json", {"started_at": now(), "config_sha256": digest(CONFIG)}, exclusive=True)
    data = pd.read_parquet(ROOT / cfg["features"])
    factors = liquidity_features(data)
    factors.to_parquet(OUT / "liquidity_factors.parquet", index=False)
    data[LIQUIDITY] = factors[LIQUIDITY]
    dividends = normalize_dividends(pd.read_csv(ROOT / cfg["dividends"]))
    samples = pd.read_parquet(ROOT / cfg["samples"])
    samples = samples[samples.signal.eq("D60_INTRA")].copy()
    samples = attach_sample_liquidity(samples, data)
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], LiquidityExitController(frame, models, cfg["confirmation_days"]))
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "九项流动性账户结算不符")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "九项流动性退出违反次日可卖")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "九项流动性继续收益退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "九项流动性及对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新九项流动性账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "LIQUIDITY_INCREMENT_EXIT_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
        ...
        "new_model_fits": sum(r["eligible_for_fit"] for r in receipts), "completed_fits": sum(r["status"] == "FIT_COMPLETE" for r in receipts),
