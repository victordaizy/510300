"""第134轮将下行风险与连续两策略的既有目标各占一半。"""
import json
import time
from pathlib import Path
import numpy as np
import pandas as pd
from research.equal_reference_pair_inputs_v1 import equal_reference_targets, PRIMARY, MODELS
PARENT = MODELS[0]
...
OUT = ROOT / "reports/research/510300_equal_reference_pair_v1"
CONFIG = ROOT / "config/510300_equal_reference_pair_v1.json"
...
def freeze():
    require(not CONFIG.exists(), "固定各半参考组合已经冻结")
    ...
    for key in ["features", "dividends"]:
        require(digest(ROOT / cfg[key]) == old_hashes[str(Path(cfg[key]))], "各半组合行情或分红不再等于已核对来源")
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "各半组合六项必要测试未通过")
    cfg.update(study_id="510300_EQUAL_REFERENCE_PAIR_V1", round=134, primary=PRIMARY, candidate_configurations=1,
        reference_weights=[.5, .5], registered_at=now(), new_model_fits=0, new_reference_accounts=0,
        ...
        rules="docs/510300_EQUAL_REFERENCE_PAIR_V1.md", input_receipt="reports/research/510300_equal_reference_pair_preflight_20260909/result.json",
        source_budget_cny=0, independent_validation="NOT_ESTABLISHED", goal_achieved=False, position_impact=0,
        previous_goal_turn_classification="PROGRESS_ROUND133_FOUR_ACCOUNTS_DELIVERED_NEXT_SOURCE_CLOCKS_COMPLETE")
    paths = [Path(__file__), old_path, ROOT / "research/equal_reference_pair_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ROOT / "research/adaptive_allocation_v1.py", ROOT / "research/intraday_overnight_increment_v1.py", ROOT / "tests/test_equal_reference_pair_v1.py",
    ...
    for item in receipt["sources"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "各半组合已绑定的父目标或平方风险来源变化")
    ...
    print("第134轮固定各半参考组合已冻结，尚未生成新目标或账户。", flush=True)


def run():
    cfg = json.loads(CONFIG.read_text(encoding="utf-8"))
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "各半组合冻结来源改变")
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            parents = {model: pd.read_parquet(parent_folder / period / cost_id / f"{model}_decisions.parquet").assign(source_cost=cost_id, source_model=model)
                for model, parent_folder in [(PARENT, P132), (MODELS[1], P91)]}
            factors, target_summary = equal_reference_targets(frame, parents, cfg, start, cost_id)
            ...
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "各半组合账户财富或终点清仓不符")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "下行风险与连续两策略目标各半组合"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(ledger.date)), "各半组合与保存对照日历不同")
            ...
            print(f"{period}／{cost_id}：一条新各半组合账户及四条保存对照完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "EQUAL_REFERENCE_PAIR_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
    ...
    print(json.dumps({"主历史": primary, "较早历史": [m for m in earlier if m["model"] == PRIMARY],
        "账户": coverage, "来源目标": target_summaries, "核心耗时": result["run_seconds"]}, ensure_ascii=False), flush=True)
