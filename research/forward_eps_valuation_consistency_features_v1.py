...
from research.forward_eps_two_institution_features_v2 import SOURCES

OUT = ROOT / "reports/research/510300_forward_eps_valuation_consistency_features_v1"
PARENT = ROOT / "reports/research/510300_forward_eps_two_institution_features_v2"
...
def freeze():
    ...
    paths = [Path(__file__), ROOT / "tests/test_forward_eps_valuation_consistency_features_v1.py",
             ROOT / "docs/510300_FORWARD_EPS_VALUATION_CONSISTENCY_V1.md",
             ROOT / "research/forward_eps_report_pe_identity_v1.py",
             ROOT / "research/forward_eps_report_valuation_fields_v1.py",
             ROOT / "research/forward_eps_two_institution_features_v2.py", PARENT / "manifest.json",
             ROOT / "reports/research/510300_forward_eps_report_pe_identity_v1/result.json",
             ROOT / "reports/research/510300_forward_eps_report_pe_identity_v1/original_page_adjudication.json"]
    save(OUT / "manifest.json", {"registered_at": now(), "old_results_and_identity_anomalies_already_observed": True,
         "round_19_new_account_results_observed": False, "round_20_results_observed": False,
         "minimum_comparable_forecast_years": 2, "reference_and_all_available_forecast_intervals_must_intersect": True,
         "missing_pe_not_imputed": True, "explicit_zero_pe_disqualifies_report_pe": True,
         "same_institution_company_current_report_selection": True, "old_eps_and_profit_features_preserved": True,
         "rule_not_complete_currency_share_or_point_in_time_validation": True,
         "files": [identity(p) for p in paths]}, exclusive=True)
    print("PE关系条件已固定：保留原值，以至少两个预测年度及参考价共同区间为条件。", flush=True)
...
def run():
    ...
    save(OUT / "result.json", {"study_id": "510300_FORWARD_EPS_VALUATION_CONSISTENCY_FEATURES_V1", "completed_at": now(),
