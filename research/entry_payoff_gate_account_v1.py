...
def simulate_entry_payoff_gate(data, dividends, config, cost, start, rule, spec, entry_views):
    ...
    def decide(t):
        ...
        admission = {"entry_model_status": view.entry_model_status, "predicted_entry_return": view.predicted_entry_return,
        ...
        if account.shares:
            ...
            selected_mode = mode
            if np.isfinite(view.predicted_entry_return) and view.predicted_entry_return > 0:
                qty = affordable_quantity(account.cash, fill_price(cl[t], 1, cost, config["tick"]), cost, config["lot"])
                action = "原进入条件成立且预测完整交易净收益为正，下一开盘请求买入"
            elif np.isfinite(view.predicted_entry_return):
                action = "预测完整交易净收益不为正，本收盘不申请进入"
            else:
        ...
        if not account.shares and mode and not np.isfinite(view.predicted_entry_return):
