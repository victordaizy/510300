"""仅复用保存的连续参考，计算一个切点预算及四个实际账户。"""
import json
from pathlib import Path
import numpy as np
import pandas as pd
from research.tangency_reference_budget_inputs_v1 import budget_frame
from research.adaptive_allocation_v1 import normalize_dividends, save_account, summarize
from research.event_clock_account_v1 import simulate_event_account
from research.intraday_overnight_increment_v1 import digest, now, require, write_json

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_tangency_reference_budget_v1"
CONFIG = ROOT / "config/510300_tangency_reference_budget_v1.json"
...
PRIMARY = "TANGENCY_REFERENCE_BUDGET"
NAME = "过去均值与协方差切点预算"
CONTROLS = {"CONTINUOUS_REFERENCE_MIN_VARIANCE": (P91, "原第91轮连续最小方差"), "UNIVERSAL_REFERENCE_BUDGET": (P97, "原第97轮混合增长预算"), "PANIC_LEARNED_HALF": (P46, "原两条策略各半"), "BUY_HOLD": (P46, "买入持有")}
...
def freeze():
    ...
    cfg.update(study_id="510300_TANGENCY_REFERENCE_BUDGET_V1", round=98, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        ...
        numerical_method="ANALYTIC_INTERIOR_CRITICAL_POINT_AND_ENDPOINTS", state_cost="BASE", missing_moment_rule="NO_VIEW_KEEP_PREVIOUS_BUDGET", cash_rule="BOTH_PAST_MEANS_NONPOSITIVE",
        rules="docs/510300_TANGENCY_REFERENCE_BUDGET_V1.md", new_model_fits=0, new_reference_accounts=0, goal_achieved=False, independent_validation="NOT_ESTABLISHED", position_impact=0)
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 4, "切点预算必要测试未通过")
    paths = [Path(__file__), ROOT / "research/tangency_reference_budget_inputs_v1.py", ROOT / "research/event_clock_account_v1.py", ROOT / "research/adaptive_allocation_v1.py", ROOT / "research/intraday_overnight_increment_v1.py",
        ROOT / "tests/test_tangency_reference_budget_v1.py", OUT / "tests_receipt.json", ROOT / cfg["features"], ROOT / cfg["dividends"], ROOT / cfg["rules"], ROOT / "docs/510300_TWO_POLICY_RISK_BUDGET_V1.md",
    ...
    print("第98轮单一切点设置已冻结，不新增预测训练或参考账户。", flush=True)
...
def run():
    ...
    factors = budget_frame(data.date, returns, states, first_ref, cfg["risk_window"])
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "TANGENCY_REFERENCE_BUDGET_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
