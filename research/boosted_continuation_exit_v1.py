...
from research.boosted_continuation_exit_inputs_v1 import FEATURES, CN, fit_boosted_exit, BoostedExitController
...
OUT = ROOT / "reports/research/510300_boosted_continuation_exit_v1"
CONFIG = ROOT / "config/510300_boosted_continuation_exit_v1.json"
...
PRIMARY = "BOOSTED_CONTINUATION_EXIT"
...
def freeze():
    ...
    cfg.update({k: train[k] for k in ["recent_cycles", "minimum_cycles", "minimum_rows", "feature_columns", "feature_names", "random_seed"]})
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "八项非线性提升与成熟成交测试尚未通过")
    cfg.update(study_id="510300_BOOSTED_CONTINUATION_EXIT_V1", round=106, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        model_parameters=model_for("HGB", train["random_seed"]).get_params(), feature_transform="RAW_FINITE_EIGHT_FEATURES_NO_STANDARDIZATION_OR_CLIPPING", missing_training_features="WHOLE_MONTH_NO_VIEW_PRESERVE_ALL_ORIGINAL_ROWS", model_loss="CYCLE_EQUAL_WEIGHTED_SQUARE_ERROR_WITH_FIXED_BOOSTING_PARAMETERS",
        feature_columns=FEATURES.copy(), feature_names=CN.copy(), added_feature_columns=[], dropped_feature_columns=[], planned_model_fits=114,
        fit_failure="NO_VIEW_MODEL_FIT_FAILED_KEEP_ORIGINAL_EXITS_NO_FALLBACK_SOLVER", rules="docs/510300_BOOSTED_CONTINUATION_EXIT_V1.md",
        samples=str((P31 / "all_reference_samples.parquet").relative_to(ROOT)), new_reference_accounts=0, source_budget_cny=0,
        previous_goal_turn_classification="PROGRESS_ROUNDS104_TO105_COMPLETED_AND_DELIVERED", position_impact=0, goal_achieved=False,
    ...
    paths = [Path(__file__), ROOT / "research/boosted_continuation_exit_inputs_v1.py", ROOT / "research/rearmed_cycle_exit_account_v1.py",
        ...
        ROOT / "tests/test_boosted_continuation_exit_v1.py", ROOT / "tests/test_median_continuation_v1.py", OUT / "tests_receipt.json"]
    ...
    print("第106轮一个条件八项非线性提升继续价值设置已冻结，尚未拟合或读取新账户收益。", flush=True)
...
def train_models(data, samples, originals, cfg):
    ...
    for original in originals:
        ...
        if eligible and not missing_rows:
            try:
                stored = fit_boosted_exit(rows, cfg)
                status = "FIT_COMPLETE"
        ...
        if stored is not None:
            coefficients.append({"拟合收盘": record["fit_origin"], "树轮数": stored["iterations"], "全部节点数": sum(len(tree["value"]) for tree in stored["trees"]), "初始平均继续价值": stored["baseline"], **stored["training_check"]})
        if (len(models) % 12) == 0:
            print(f"提升退出进度：已处理{len(models)}个原月度时点。", flush=True)
    ...
    pd.DataFrame(coefficients).to_csv(OUT / "每月提升模型摘要.csv", index=False, encoding="utf-8-sig")
    print(f"原成熟时点不变：八项非线性提升拟合完成{sum(r['status']=='FIT_COMPLETE' for r in receipts)}次，未成熟或求解失败{sum(r['status']!='FIT_COMPLETE' for r in receipts)}次。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "八项非线性提升冻结来源改变")
    ...
    samples.to_parquet(OUT / "original_reference_samples.parquet", index=False)
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], BoostedExitController(frame, models, cfg["confirmation_days"]))
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "八项非线性提升账户结算不符")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "八项非线性提升退出违反次日可卖")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "八项非线性提升继续收益退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "八项非线性提升及对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新八项非线性提升账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "BOOSTED_CONTINUATION_EXIT_EXIT_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
