...
def simulate_source_aware_cycle(data, dividends, config, cost, start, rule, spec, controller=None):
    ...
    for key in ["entry_view", "rearm_allowed"]:
        if key in rule:
            require(len(rule[key]) == len(data), "来源状态与市场日历长度不符")
    ...
    def decide(t):
        ...
        entry_view = bool(rule["entry_view"][t]) if "entry_view" in rule else True
        rearm_allowed = bool(rule["rearm_allowed"][t]) if "rearm_allowed" in rule else bool(mode == 0 or (last_entry_mode and mode != last_entry_mode))
        require(entry_view or mode == 0, "无观点日意外提供进入模式")
        if not account.shares and rearm_allowed:
            entry_rearmed = True
        ...
        if account.shares:
            ...
            if not pending_reasons:
                ...
                if learned.get("additional_exit_requested", False):
                    additional_reason = learned.get("additional_exit_reason")
                    require(isinstance(additional_reason, str) and bool(additional_reason.strip()), "额外退出条件缺少具体原因")
        ...
        if not entry_view and not account.shares:
            row["reference_weight"] = None
        row["entry_observation_state"] = "VIEW_ALLOWED" if entry_view else "NO_VIEW"
        row["rearm_condition_known_false"] = rearm_allowed
