...
from research.month_edge_opportunity_inputs_v1 import CANDIDATES,MODELS,PARENT,PRIMARY,FULL,CALENDAR_SOURCE,calendar_frames
...
OUT=ROOT/'reports/research/510300_month_edge_opportunity_v1'
CONFIG=ROOT/'config/510300_month_edge_opportunity_v1.json'
...
CONTROLS={PARENT:(SOURCE,'第181轮原账户风险预算'),
    ...
    'K2_MONTH_EDGE':(CALENDAR_SOURCE,'原月内两端独立账户'),
    'BUY_HOLD':(ROOT/'reports/research/510300_rearmed_session_exit_v1','买入持有')}


def prepare():
    require(not CONFIG.exists() and not (OUT/'RUN_STARTED.json').exists(),'第190轮已有冻结或运行')
    OUT.mkdir(parents=True,exist_ok=True)
    receipt=OUT/'tests_receipt.json'
    require(not receipt.exists(),'必要测试已有回执')
    began=time.perf_counter()
    t=subprocess.run([sys.executable,'-m','pytest','tests/test_month_edge_opportunity_v1.py','-q','-p','no:cacheprovider'],
                     cwd=ROOT,capture_output=True,text=True,encoding='utf-8')
    (OUT/'tests_output.txt').write_text(t.stdout+t.stderr,encoding='utf-8')
    print(t.stdout,flush=True)
    require(t.returncode==0 and '5 passed' in t.stdout,'第190轮测试未通过')
    ...
    cfg.update(study_id='510300_MONTH_EDGE_OPPORTUNITY_V1',round=190,primary=PRIMARY,candidate_models=list(CANDIDATES),
        parent_models=MODELS,candidate_configurations=2,registered_at=now(),decision_clock='09:00:00',source_signal_clock='15:05:00',
        rules='docs/510300_MONTH_EDGE_OPPORTUNITY_V1.md',month_first_sessions=3,month_end_natural_days=5,
        ...
        previous_goal_turn_classification='PROGRESS_ROUND189_FOUR_ACCOUNTS_JOINT_TARGET_FAILED',
    ...
    with rules.open('x',encoding='utf-8') as stream:
        stream.write((ROOT/'docs/510300_MONTH_EDGE_OPPORTUNITY_NEXT_20260913.md').read_text(encoding='utf-8'))
        stream.write('\n\n## 来源全部中文因素及进入退出规则\n\n')
        stream.write((ROOT/old['rules']).read_text(encoding='utf-8'))
    paths=[Path(__file__),ROOT/'research/month_edge_opportunity_inputs_v1.py',ROOT/'research/joint_account_acceptance_v1.py',
        ...
        ROOT/'tests/test_month_edge_opportunity_v1.py',receipt,OUT/'tests_output.txt',rules,
        ROOT/cfg['features'],ROOT/cfg['dividends'],source_config,ROOT/old['rules'],SOURCE/'saved_verification_receipt.json',
        ROOT/'config/510300_research_authority_v6.json',ROOT/'config/510300_calendar_learned_equal_blend_v1.json',ROOT/'docs/510300_CALENDAR_LEARNED_EQUAL_BLEND_V1.md',
    ...
    require(i['latest_completed_round']['round']==189,'第190轮前序不同')
    i['running_studies']=[{'round':190,'study':cfg['study_id'],'status':'FROZEN_NOT_STARTED','config':str(CONFIG.relative_to(ROOT))}]
    i['next_work'].update(registered=True,status='MONTH_EDGE_OPPORTUNITY_FROZEN',source=cfg['rules'])
    write_json(p,i)
    print('第190轮已冻结。',flush=True)


def run():
    r=run_saved_target_batch(ROOT,OUT,CONFIG,CANDIDATES,PARENTS,CONTROLS,calendar_frames)
    cfg=json.loads(CONFIG.read_text(encoding='utf-8'))
    print(json.dumps(save_joint_assessment(OUT,cfg,r),ensure_ascii=False),flush=True)


def verify():
    require(not (OUT/'saved_verification_receipt.json').exists(),'第190轮已核对')
    cfg=json.loads(CONFIG.read_text(encoding='utf-8'))
    ...
    def expected(period,cost_id,frame,start):
        ...
        ordinals=pd.Series(np.arange(len(dates))).groupby(dates.to_period('M'),sort=False).cumcount()+1
        ...
        for origin,value in zip(origins,parent.reference_weight):
            ...
            active=bool(ordinals.iloc[origin+1]<=3 or execution_day.days_in_month-execution_day.day<5)
            calendar[origin]=float(active)
            if active_model==PRIMARY:
                target[origin]=float(active) if value==0 else value
            else:
                require(active_model==FULL,'月内两端候选身份不同')
                target[origin]=1. if active else value
        ...
        require(pd.DatetimeIndex(factors.parent_signal_time.iloc[origins]).equals(dates[origins]+pd.Timedelta(hours=15,minutes=5)),'来源收盘时钟不同')
        require(pd.DatetimeIndex(factors.decision_time.iloc[origins]).equals(dates[origins+1]+pd.Timedelta(hours=9)),'日历最终决定不是执行日九点')
        require((factors.parent_signal_time.iloc[origins]<factors.decision_time.iloc[origins]).all(),'日历决定没有晚于来源收盘')
    ...
    for active_model in CANDIDATES:
        ...
        total_count+=count
    require(len(accounts)==8 and total_count==11292,'第190轮核对范围不同')
    for name,rows in [('saved_account_checks.csv',accounts),('saved_actual_cycles.csv',cycles),('saved_comparison_differences.csv',diffs),('saved_month_edge_checks.csv',checks)]:
        pd.DataFrame(rows).to_csv(OUT/name,index=False,encoding='utf-8-sig')
    receipt={'verified_at':now(),'status':'PASS_EIGHT_SIMULATED_ACCOUNTS_MONTH_EDGE_AND_COSTS',
