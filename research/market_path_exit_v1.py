"""单一条件不含已付买入费用的八项周期内退出模型的完整历史账户检验。"""
...
from research.market_path_exit_inputs_v1 import FEATURES, CN, fit_market_path_exit, MarketPathExitController
from research.rearmed_cycle_exit_account_v1 import simulate_rearmed_exit
from research.simple_intraday_protection_v1 import make_rules

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_market_path_exit_v1"
CONFIG = ROOT / "config/510300_market_path_exit_v1.json"
...
def freeze():
    ...
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "不含已付买入费用的八项周期内与成熟成交测试尚未通过")
    cfg.update(study_id="510300_MARKET_PATH_EXIT_V1", round=117, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        fit_intercept=False, fit_cycle_intercepts=True, cycle_intercept_penalty=0., prediction_intercept="EQUAL_MEAN_OF_MATURE_CYCLE_INTERCEPTS", solver="svd", state_preflight_receipt="reports/research/510300_market_path_state_preflight_20260908/result.json", saved_market_samples="reports/research/510300_market_path_state_preflight_20260908/augmented_reference_samples.parquet", saved_market_states="reports/research/510300_market_path_state_preflight_20260908/reference_market_states.parquet", missing_training_features="WHOLE_MONTH_NO_VIEW_PRESERVE_ALL_ORIGINAL_ROWS", model_loss="WITHIN_CYCLE_CENTERED_SQUARE_ERROR_WITH_BETA_RIDGE_AND_UNPENALIZED_CYCLE_INTERCEPTS",
        feature_columns=FEATURES.copy(), feature_names=CN.copy(), added_feature_columns=FEATURES[1:3], dropped_feature_columns=["cycle_return", "cycle_drawdown"], planned_model_fits=114,
        fit_failure="NO_VIEW_MODEL_FIT_FAILED_KEEP_ORIGINAL_EXITS_NO_FALLBACK_SOLVER", rules="docs/510300_MARKET_PATH_EXIT_V1.md",
        samples=str((P31 / "all_reference_samples.parquet").relative_to(ROOT)), new_reference_accounts=0, source_budget_cny=0,
        previous_goal_turn_classification="PROGRESS_ROUND116_COMPLETED_AND_SUNK_COST_STATE_IDENTIFIED", position_impact=0, goal_achieved=False,
    ...
    paths = [Path(__file__), ROOT / "research/market_path_exit_inputs_v1.py", ROOT / "research/rearmed_cycle_exit_account_v1.py",
        ...
        ROOT / "tests/test_market_path_exit_v1.py", ROOT / "tests/test_median_continuation_v1.py", OUT / "tests_receipt.json"]
    ...
    print("第117轮一个条件不含已付买入费用的八项周期内继续价值设置已冻结，尚未拟合或读取新账户收益。", flush=True)
...
def train_models(data, samples, originals, cfg):
    ...
    for original in originals:
        ...
        if eligible and not missing_rows:
            try:
                stored = fit_market_path_exit(rows, cfg)
                status = "FIT_COMPLETE"
    ...
    print(f"原成熟时点不变：不含已付买入费用的八项周期内拟合完成{sum(r['status']=='FIT_COMPLETE' for r in receipts)}次，未成熟或求解失败{sum(r['status']!='FIT_COMPLETE' for r in receipts)}次。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "不含已付买入费用的八项周期内冻结来源改变")
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], MarketPathExitController(frame, models, cfg["confirmation_days"]))
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "不含已付买入费用的八项周期内账户结算不符")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "不含已付买入费用的八项周期内退出违反次日可卖")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "不含已付买入费用的八项周期内继续收益退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "不含已付买入费用的八项周期内及对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新不含已付买入费用的八项周期内账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "MARKET_PATH_EXIT_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
