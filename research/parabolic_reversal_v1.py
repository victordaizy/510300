...
OUT = ROOT / "reports/research/510300_parabolic_reversal_v1"
P46 = ROOT / "reports/research/510300_panic_learned_equal_blend_v1"
CONFIG = ROOT / "config/510300_parabolic_reversal_v1.json"
PRIMARY = "PARABOLIC_REVERSAL"
NAME = "抛物线价格转向进入与退出"
...
def freeze():
    ...
    cfg.update(study_id="510300_PARABOLIC_REVERSAL_V1", round=74, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        ...
        rules="docs/510300_PARABOLIC_REVERSAL_V1.md", previous_goal_turn_classification="PROGRESS_ROUND73_COMPLETED_AND_DELIVERED",
        position_impact=0, goal_achieved=False, independent_validation="NOT_ESTABLISHED")
    paths = [Path(__file__), ROOT / "research/parabolic_reversal_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ROOT / "research/adaptive_allocation_v1.py", ROOT / "research/intraday_overnight_increment_v1.py",
        ROOT / cfg["features"], ROOT / cfg["dividends"], ROOT / cfg["rules"], ROOT / "tests/test_parabolic_reversal_v1.py",
    ...
    require(tests["exit_code"] == 0, "转向规则必要测试尚未通过")
    ...
    print("第74轮一个0.02步长、0.2上限转向设置已冻结，尚无新策略收益。", flush=True)


def run():
    cfg = json.loads(CONFIG.read_text(encoding="utf-8"))
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "转向规则冻结来源改变")
    write_json(OUT / "RUN_STARTED.json", {"started_at": now(), "config_sha256": digest(CONFIG)}, exclusive=True)
    data = pd.read_parquet(ROOT / cfg["features"])
    dividends = normalize_dividends(pd.read_csv(ROOT / cfg["dividends"]))
    factors = parabolic_frame(data, cfg["acceleration"], cfg["maximum_acceleration"])
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "转向规则完整账户结算失败")
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "转向规则对照日历不同")
            ...
            print(f"{period}／{cost_id}：转向规则新账户与四个保存对照完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "PARABOLIC_REVERSAL_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
