"""完整现金证据支持降维预算，直接运行四个新账户。"""
import json
from pathlib import Path
import numpy as np
import pandas as pd
from research.known_cash_risk_budget_inputs_v1 import budget_frame, observed_cash_days
from research.adaptive_allocation_v1 import normalize_dividends, save_account, summarize
from research.event_clock_account_v1 import simulate_event_account
from research.intraday_overnight_increment_v1 import digest, now, require, write_json

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_known_cash_risk_budget_v1"
CONFIG = ROOT / "config/510300_known_cash_risk_budget_v1.json"
P91 = ROOT / "reports/research/510300_continuous_reference_min_variance_v1"
P97 = ROOT / "reports/research/510300_universal_reference_budget_v1"
...
PRIMARY = "KNOWN_CASH_RISK_BUDGET"
NAME = "已知现金支路与可识别风险预算"
CONTROLS = {"CONTINUOUS_REFERENCE_MIN_VARIANCE": (P91, "原第91轮连续最小方差"), "UNIVERSAL_REFERENCE_BUDGET": (P97, "原第97轮混合增长预算"), "TANGENCY_REFERENCE_BUDGET": (P98, "原第98轮正定切点预算"), "BUY_HOLD": (P46, "买入持有")}


def freeze():
    require(not CONFIG.exists(), "切点预算已经登记")
    old = json.loads((ROOT / "config/510300_continuous_reference_min_variance_v1.json").read_text(encoding="utf-8"))
    cfg = {k: old[k] for k in ["evaluation_start", "data_cutoff", "initial_capital", "lot", "tick", "limit_fraction", "annual_days", "cash_annual_rate_assumption", "high_sharpe_target", "costs", "features", "dividends", "earlier_start", "earlier_terminal", "weight_band", "reference_start", "risk_window"]}
    cfg.update(study_id="510300_KNOWN_CASH_RISK_BUDGET_V1", round=99, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        budget_objective="MAX_NONNEGATIVE_PAST_MEAN_OVER_SAMPLE_STANDARD_DEVIATION", moment_clock="MONTH_FIRST_COMPLETE_CLOSE_NEXT_OPEN",
        numerical_method="CONFIRMED_CASH_RISK_REDUCTION_OR_ORIGINAL_FULL_RANK_TANGENCY", single_risk_size_tie="MAX_PAST_MEAN_WITH_SAME_ESTIMATED_SHARPE", cash_evidence="EVERY_DAY_NO_SHARES_NO_FILLED_TRADE_ZERO_NET_RETURN", state_cost="BASE", missing_moment_rule="NO_VIEW_KEEP_PREVIOUS_BUDGET", cash_rule="BOTH_PAST_MEANS_NONPOSITIVE",
        rules="docs/510300_KNOWN_CASH_RISK_BUDGET_V1.md", new_model_fits=0, new_reference_accounts=0, goal_achieved=False, independent_validation="NOT_ESTABLISHED", position_impact=0)
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 5, "切点预算必要测试未通过")
    paths = [Path(__file__), ROOT / "research/known_cash_risk_budget_inputs_v1.py", ROOT / "research/tangency_reference_budget_inputs_v1.py", ROOT / "research/event_clock_account_v1.py", ROOT / "research/adaptive_allocation_v1.py", ROOT / "research/intraday_overnight_increment_v1.py",
        ROOT / "tests/test_known_cash_risk_budget_v1.py", OUT / "tests_receipt.json", ROOT / cfg["features"], ROOT / cfg["dividends"], ROOT / cfg["rules"], ROOT / "docs/510300_TWO_POLICY_RISK_BUDGET_V1.md",
        ROOT / "config/510300_continuous_reference_min_variance_v1.json", P91 / "evaluation_factors.parquet", ROOT / "reports/research/510300_saved_budget_state_differences_20260908/risk_support_receipt.json"]
    ...
    print("第99轮单一切点设置已冻结，不新增预测训练或参考账户。", flush=True)


def run():
    cfg = json.loads(CONFIG.read_text(encoding="utf-8"))
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "切点冻结输入改变")
    ...
    cash_days = np.column_stack([observed_cash_days(pd.read_parquet(P91 / "continuous_references/BASE" / f"{model}_ledger.parquet"), data.date, first_ref) for model in ["PANIC_ONLY", "REARM_RIDGE"]])
    pd.DataFrame({"date": data.date, "panic_confirmed_cash_day": cash_days[:, 0], "learned_confirmed_cash_day": cash_days[:, 1]}).to_parquet(OUT / "known_cash_evidence.parquet", index=False)
    factors = budget_frame(data.date, returns, states, cash_days, first_ref, cfg["risk_window"])
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main), ("earlier_diagnostic", data[data.date.le(cfg["earlier_terminal"])], cfg["earlier_start"], earlier)]:
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "切点预算账户未完整结算")
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "KNOWN_CASH_RISK_BUDGET_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
