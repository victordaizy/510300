...
from research.contextual_parent_advantage_inputs_v1 import build_policy, FEATURES, CN
from research.event_clock_account_v1 import simulate_event_account
from research.intraday_overnight_increment_v1 import digest, now, require, write_json
from research.simple_signal_blend_v1 import decision_state

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_contextual_parent_advantage_v1"
CONFIG = ROOT / "config/510300_contextual_parent_advantage_v1.json"
P82 = ROOT / "reports/research/510300_two_policy_min_variance_v1"
P88 = ROOT / "reports/research/510300_three_policy_min_variance_v1"
P89 = ROOT / "reports/research/510300_past_sharpe_parent_selector_v1"
PRIMARY = "CONTEXTUAL_PARENT_ADVANTAGE"
NAME = "当前市场状态预测两父组合相对收益"
PARENT_MODELS=["TWO_POLICY_MIN_VARIANCE","THREE_POLICY_MIN_VARIANCE"]
CONTROLS={"TWO_POLICY_MIN_VARIANCE":"原两策略最小方差","THREE_POLICY_MIN_VARIANCE":"原三策略最小方差","PAST_SHARPE_PARENT_SELECTOR":"第89轮过去净夏普选择","BUY_HOLD":"买入持有"}


def control_folder(model):
    return P82 if model=="TWO_POLICY_MIN_VARIANCE" else P89 if model=="PAST_SHARPE_PARENT_SELECTOR" else P88



def freeze():
    require(not CONFIG.exists(), "条件相对收益选择已登记，不能重复冻结")
    ...
    cfg.update(study_id="510300_CONTEXTUAL_PARENT_ADVANTAGE_V1",round=90,objective="PREDICT_THREE_MINUS_TWO_FUTURE_COMPOUND_PARENT_NET_RETURN",registered_at=now(),primary=PRIMARY,candidate_configurations=1,
        target_horizon=20,training_window=504,minimum_training_rows=252,ridge_alpha=1.,feature_clip=5.,feature_columns=FEATURES,feature_names=CN,
        training_weight="EQUAL_MATURE_DAILY_ORIGINS",solver="svd",fit_intercept=True,selection_clock="MONTH_FIRST_COMPLETE_CLOSE_NEXT_OPEN",state_cost="BASE",initial_selection=PARENT_MODELS[0],
        no_model_policy="EXPLICIT_TWO_POLICY_BASELINE",positive_threshold=0.,parent_models=PARENT_MODELS,
        rules="docs/510300_CONTEXTUAL_PARENT_ADVANTAGE_V1.md",new_reference_accounts=0,previous_goal_turn_classification="PROGRESS_ROUND89_COMPLETED_AND_DELIVERED",
        environment={"sklearn":sklearn.__version__,"numpy":np.__version__,"pandas":pd.__version__},independent_validation="NOT_ESTABLISHED",goal_achieved=False,position_impact=0)
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0, "组合选择必要测试未通过")
    paths=[Path(__file__),ROOT/"research/contextual_parent_advantage_inputs_v1.py",ROOT/"research/event_clock_account_v1.py",
        ...
        ROOT/cfg["features"],ROOT/cfg["dividends"],ROOT/cfg["rules"],ROOT/"tests/test_contextual_parent_advantage_v1.py",OUT/"tests_receipt.json",
        ROOT/"config/510300_research_authority_v6.json",ROOT/"config/510300_two_policy_min_variance_v1.json",ROOT/"config/510300_three_policy_min_variance_v1.json",ROOT/"config/510300_past_sharpe_parent_selector_v1.json",
    ...
    print("第90轮三因子条件相对收益规则已冻结，尚无新策略收益。", flush=True)
...
def run():
    ...
    main, earlier, yearly, eras, updates, coverage, all_models = [], [], [], [], [], [], []
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        factors,labels,models=build_policy(frame,reference_returns,np.column_stack(targets),first,cfg)
        labels.to_parquet(OUT/f"{period}_training_labels.parquet",index=False)
        write_json(OUT/f"{period}_saved_models.json",{"models":models},exclusive=True)
        all_models.extend({"period":period,**record} for record in models)
        text=[f"# {period}：每月三个市场因子与相对收益预测", "", "每项先减本月训练均值、除总体标准差，限制负五至正五，再乘系数。三项与截距相加，正值选三策略，否则两策略。原点不足或模型无效执行两策略基线。", ""]
        for record in models:
            text += [f"## {pd.Timestamp(record['fit_origin']).date()}", "", f"成熟训练行数{record['training_rows']}。", ""]
            m=record["model"]
            if m is None:text += ["没有有效模型，按原两策略基线执行。", ""]
            else:
                text += [f"截距{m['intercept']:.12f}。", ""]
                text.extend(f"- {name}：均值{mean:.12f}，标准差{scale:.12f}，标准化后的系数{coefficient:.12f}。" for name,mean,scale,coefficient in zip(CN,m["mean"],m["scale"],m["coefficients"],strict=True))
                text += [""]
        (OUT/f"{period}_每月三因子中文系数.md").write_text("\n".join(text)+"\n",encoding="utf-8")
    ...
    pd.DataFrame([{k:v for k,v in r.items() if k!="model"} for r in all_models]).to_csv(OUT/"training_receipts.csv",index=False,encoding="utf-8-sig")
    primary = [m for m in main if m["model"] == PRIMARY]
    write_json(OUT / "result.json", {"study_id": cfg["study_id"], "completed_at": now(), "status": "CONTEXTUAL_PARENT_ADVANTAGE_ACCOUNTS_COMPLETE",
        ...
        "new_model_fits": sum(r["fit_attempted"] for r in all_models), "completed_fits": sum(r["status"]=="FIT_COMPLETE" for r in all_models), "failed_fits": sum(r["status"]=="NO_VIEW_MODEL_FIT_FAILED" for r in all_models), "no_view_fit_origins": sum(r["status"]!="FIT_COMPLETE" for r in all_models), "new_reference_accounts": 0, "all_metrics": main, "earlier_diagnostics": earlier, "primary": primary,
