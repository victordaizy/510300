...
OUT = ROOT/'reports/research/510300_runs_reference_blend_v1'
CONFIG = ROOT/'config/510300_runs_reference_blend_v1.json'
...
def freeze():
    require(not CONFIG.exists(), '第166轮固定等额组合已经冻结')
    prior_path = ROOT/'config/510300_return_runs_state_v1.json'
    prior = json.loads(prior_path.read_text(encoding='utf-8'))
    tests = json.loads((OUT/'tests_receipt.json').read_text(encoding='utf-8'))
    require(tests['exit_code'] == 0 and tests['passed'] == 6, '第166轮六项必要测试尚未通过')
    require((P165/'acceptance_outcome.json').is_file() and (P165/'saved_verification_receipt.json').is_file(), '第165轮独立方法尚未完成核对及关闭')
    ...
    cfg.update(study_id='510300_RUNS_REFERENCE_BLEND_V1', round=166, primary=PRIMARY, candidate_configurations=1,
        candidate_models=list(CANDIDATES), registered_at=now(), decision_clock='15:05:00', parent_models=MODELS,
        blend_weights={model: .5 for model in MODELS}, new_model_fits=0, new_reference_accounts=0,
        combination='FIXED_EQUAL_SAVED_CLOSE_TARGETS_WITH_NO_CASH_REALLOCATION',
        unknown_parent='ANY_UNKNOWN_PARENT_MAKES_COMBINED_TARGET_UNKNOWN', parent_feedback='NONE',
        source_budget_cny=0, goal_achieved=False, position_impact=0, rules='docs/510300_RUNS_REFERENCE_BLEND_V1.md',
        ...
        previous_goal_turn_classification='PROGRESS_ROUND165_COMPLETED_CLOSED_AND_DELIVERED_FULL_GOAL_NOT_MET')
    paths = [Path(__file__), prior_path, ROOT/'research/runs_reference_blend_inputs_v1.py',
        ...
        ROOT/'research/intraday_overnight_increment_v1.py', ROOT/'tests/test_runs_reference_blend_v1.py',
        ...
        ROOT/'docs/510300_RUNS_REFERENCE_BLEND_NEXT_20260910.md', ROOT/'config/510300_research_authority_v6.json',
        P165/'acceptance_outcome.json']
    ...
    for model, folder in PARENTS.items():
        ...
        paths.extend([parent_cfg_path, ROOT/parent_cfg['rules'], folder/'saved_verification_receipt.json'])
        for period in ['evaluation', 'earlier_diagnostic']:
            for cost in cfg['costs']:
                paths.append(folder/period/cost/f'{model}_decisions.parquet')
    for period in ['evaluation', 'earlier_diagnostic']:
        for cost in cfg['costs']:
    ...
    print('第166轮两套目标各半的固定规则已冻结，尚未生成新组合账户。', flush=True)


def run():
    return run_saved_target_batch(ROOT, OUT, CONFIG, CANDIDATES, PARENTS, CONTROLS, runs_reference_blend_frames)
