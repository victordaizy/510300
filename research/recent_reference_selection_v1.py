"""第135轮按已发生的历史净夏普选择两套来源或现金。"""
import json
import time
from pathlib import Path
import numpy as np
import pandas as pd
from research.recent_reference_selection_inputs_v1 import selected_reference_frames, PRIMARY, MODELS
PARENT = MODELS[0]
...
OUT = ROOT / "reports/research/510300_recent_reference_selection_v1"
CONFIG = ROOT / "config/510300_recent_reference_selection_v1.json"
...
def freeze():
    require(not CONFIG.exists(), "月度历史表现选择已经冻结")
    old_path = ROOT / "config/510300_equal_reference_pair_v1.json"
    ...
    for key in ["features", "dividends"]:
        require(digest(ROOT / cfg[key]) == old_hashes[str(Path(cfg[key]))], "月度选择行情或分红不再等于已核对来源")
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "月度选择六项必要测试未通过")
    cfg.update(study_id="510300_RECENT_REFERENCE_SELECTION_V1", round=135, primary=PRIMARY, candidate_configurations=1,
        selection_window=242, scoring_cost="BASE", initial_parent=PARENT, registered_at=now(), new_model_fits=0, new_reference_accounts=0,
        ...
        rules="docs/510300_RECENT_REFERENCE_SELECTION_V1.md", input_receipt="reports/research/510300_recent_reference_selection_preflight_20260909/result.json",
        source_budget_cny=0, independent_validation="NOT_ESTABLISHED", goal_achieved=False, position_impact=0,
        previous_goal_turn_classification="PROGRESS_ROUNDS133_134_EIGHT_ACCOUNTS_DELIVERED_NEXT_RANKING_PREFLIGHT_COMPLETE")
    paths = [Path(__file__), old_path, ROOT / "research/recent_reference_selection_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ROOT / "research/adaptive_allocation_v1.py", ROOT / "research/intraday_overnight_increment_v1.py", ROOT / "tests/test_recent_reference_selection_v1.py",
    ...
    for item in receipt["sources"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "月度选择已绑定的父目标或平方风险来源变化")
    ...
    print("第135轮月度历史表现选择已冻结，尚未生成新目标或账户。", flush=True)


def run():
    cfg = json.loads(CONFIG.read_text(encoding="utf-8"))
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "月度选择冻结来源改变")
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        factor_frames, summaries = selected_reference_frames(frame, base_ledgers, parents_by_cost, cfg, start)
        target_summaries.extend({"period": period, **item} for item in summaries)
        base_factors = factor_frames["BASE"]
        base_factors[base_factors.selection_update_scheduled].to_csv(OUT / f"{period}_selection_updates.csv", index=False, encoding="utf-8-sig")
        for cost_id, cost in cfg["costs"].items():
            ...
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "月度选择账户财富或终点清仓不符")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "按历史净夏普选择两套策略或现金"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(ledger.date)), "月度选择与保存对照日历不同")
            ...
            print(f"{period}／{cost_id}：一条新月度选择账户及四条保存对照完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "RECENT_REFERENCE_SELECTION_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
    ...
    print(json.dumps({"主历史": primary, "较早历史": [m for m in earlier if m["model"] == PRIMARY],
        "账户": coverage, "月度选择": target_summaries, "核心耗时": result["run_seconds"]}, ensure_ascii=False), flush=True)
