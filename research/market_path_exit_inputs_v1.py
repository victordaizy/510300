"""周期内学习使用不含已付买入费用的两个市场路径状态，实际账户风控独立保留。"""
...
FEATURES[1:3] = ["market_cycle_return", "market_cycle_drawdown"]
CN = OLD_CN.copy()
CN[1:3] = ["不含已付买入费用的市场涨跌", "不含已付买入费用的市场回撤"]
from research.intraday_overnight_increment_v1 import require


def fit_market_path_exit(rows, cfg):
    ...
    return {"kind": "MARKET_PATH_WITHIN_CYCLE_RIDGE", "features": FEATURES.copy(), "mean": mean.tolist(), "scale": scale.tolist(),
...
def market_path_prediction(model, values):
    require(model["kind"] == "MARKET_PATH_WITHIN_CYCLE_RIDGE" and model["features"] == FEATURES, "周期内预测模型身份不同")
...
class MarketPathExitController:
    ...
    def __call__(self, t, cycle, current_value, peak_value):
        ...
        if record and record["status"] == "FIT_COMPLETE":
            ...
            if np.isfinite(x).all():
                value, status = market_path_prediction(record["model"], x), "PREDICTION_AVAILABLE"
            else:
