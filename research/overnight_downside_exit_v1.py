...
from research.overnight_downside_exit_inputs_v1 import FEATURES, CN, fit_downside_exit, OvernightDownsideExitController, ADDED, attach_downside, downside_frame
from research.rearmed_cycle_exit_account_v1 import simulate_rearmed_exit
from research.simple_intraday_protection_v1 import make_rules

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_overnight_downside_exit_v1"
CONFIG = ROOT / "config/510300_overnight_downside_exit_v1.json"
...
PRIMARY = "OVERNIGHT_DOWNSIDE_EXIT"
...
def freeze():
    ...
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "九项隔夜下行风险占比与成熟成交测试尚未通过")
    cfg.update(study_id="510300_OVERNIGHT_DOWNSIDE_EXIT_V1", round=107, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        fit_intercept=True, solver="svd", downside_window=20, added_factor_definition="NEGATIVE_OVERNIGHT_LOG_SQUARE_SUM_DIVIDED_BY_ALL_OVERNIGHT_AND_INTRADAY_LOG_SQUARE_SUM", missing_training_features="WHOLE_MONTH_NO_VIEW_PRESERVE_ALL_ORIGINAL_ROWS", model_loss="CYCLE_EQUAL_WEIGHTED_SQUARE_ERROR_WITH_ORIGINAL_RIDGE_PENALTY",
        ...
        fit_failure="NO_VIEW_MODEL_FIT_FAILED_KEEP_ORIGINAL_EXITS_NO_FALLBACK_SOLVER", rules="docs/510300_OVERNIGHT_DOWNSIDE_EXIT_V1.md",
        samples=str((P31 / "all_reference_samples.parquet").relative_to(ROOT)), new_reference_accounts=0, source_budget_cny=0,
        previous_goal_turn_classification="PROGRESS_ROUND106_COMPLETED_AND_DELIVERED", position_impact=0, goal_achieved=False,
    ...
    paths = [Path(__file__), ROOT / "research/overnight_downside_exit_inputs_v1.py", ROOT / "research/rearmed_cycle_exit_account_v1.py",
        ...
        ROOT / "tests/test_overnight_downside_exit_v1.py", ROOT / "tests/test_median_continuation_v1.py", OUT / "tests_receipt.json"]
    ...
    print("第107轮一个条件九项隔夜下行风险占比继续价值设置已冻结，尚未拟合或读取新账户收益。", flush=True)
...
def train_models(data, samples, originals, cfg):
    ...
    for original in originals:
        ...
        if eligible and not missing_rows:
            try:
                stored = fit_downside_exit(rows, cfg)
                status = "FIT_COMPLETE"
    ...
    print(f"原成熟时点不变：九项隔夜下行风险占比拟合完成{sum(r['status']=='FIT_COMPLETE' for r in receipts)}次，未成熟或求解失败{sum(r['status']!='FIT_COMPLETE' for r in receipts)}次。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "九项隔夜下行风险占比冻结来源改变")
    ...
    factors = downside_frame(data, cfg["downside_window"])
    factors.to_parquet(OUT / "overnight_downside_factors.parquet", index=False)
    samples = attach_downside(samples, factors)
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], OvernightDownsideExitController(frame, models, cfg["confirmation_days"], factors=factors.iloc[:len(frame)]))
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "九项隔夜下行风险占比账户结算不符")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "九项隔夜下行风险占比退出违反次日可卖")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "九项隔夜下行风险占比继续收益退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "九项隔夜下行风险占比及对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新九项隔夜下行风险占比账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "OVERNIGHT_DOWNSIDE_EXIT_EXIT_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
