"""按事件时钟调整的完整账户，沿用既有费用、整手、分红和T+1记账。"""
...
def simulate_request_account(data: pd.DataFrame, dividends: pd.DataFrame, config: dict, cost: dict, start: str,
             model_id: str, targets: np.ndarray | None = None, prediction: np.ndarray | None = None,
             horizon: int = 1, event_mask: np.ndarray | None = None, *, request_policy) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def decision_at(t: int) -> dict:
        ...
        if model_id == "BUY_HOLD":
            ...
            if np.isfinite(value):
                ...
                result = choose_order(account, price, value, horizon * variance[t], cost, config) if prediction is not None else request_policy(account, price, value, config, model_id)
            else:
