...
def run_referenced_target_batch(root, out, config_path, candidates, parents, controls, build_frames, reference_receipt):
    ...
    require(reference_receipt["new_reference_accounts"]==2, "本批次只新增两条较早参考")
    for item in reference_receipt["files"]:
        require(digest(root/item["path"])==item["sha256"], "参考账户生成后来源改变")
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "FINITE_SAVED_TARGET_BATCH_ACCOUNTS_COMPLETE",
        ...
        "reused_earlier_accounts": len(controls)*len(cfg["costs"])-2, "new_model_fits": 0, "new_reference_accounts": 2,
        "new_earlier_reference_accounts": 2, "reference_run_seconds": reference_receipt["run_seconds"],
        ...
        "historical_point_target_met": any(m["meets_point_target"] for m in candidate_main), "run_seconds": time.perf_counter()-began+reference_receipt["run_seconds"],
        "run_time_scope": "两条新较早参考、保存来源复制与八条新组合账户；不含开发、测试和核对",
