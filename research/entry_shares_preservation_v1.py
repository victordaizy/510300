...
from research.entry_shares_preservation_inputs_v1 import CANDIDATES,MODELS,PARENT,PRIMARY,preservation_frames
...
OUT=ROOT/'reports/research/510300_entry_shares_preservation_v1'
CONFIG=ROOT/'config/510300_entry_shares_preservation_v1.json'
...
def prepare():
    require(not CONFIG.exists() and not (OUT/'RUN_STARTED.json').exists(),'第193轮已有冻结或运行')
    OUT.mkdir(parents=True,exist_ok=True)
    receipt=OUT/'tests_receipt.json'
    require(not receipt.exists(),'必要测试已有回执')
    began=time.perf_counter()
    t=subprocess.run([sys.executable,'-m','pytest','tests/test_entry_shares_preservation_v1.py','-q','-p','no:cacheprovider'],
                     cwd=ROOT,capture_output=True,text=True,encoding='utf-8')
    (OUT/'tests_output.txt').write_text(t.stdout+t.stderr,encoding='utf-8')
    print(t.stdout,flush=True)
    require(t.returncode==0 and '6 passed' in t.stdout,'第193轮测试未通过')
    write_json(receipt,{'recorded_at':now(),'exit_code':t.returncode,'passed':6,'seconds':time.perf_counter()-began,
    ...
    cfg.update(study_id='510300_ENTRY_SHARES_PRESERVATION_V1',round=193,primary=PRIMARY,candidate_models=list(CANDIDATES),
        parent_models=MODELS,candidate_configurations=1,registered_at=now(),decision_clock='15:05:00',
        rules='docs/510300_ENTRY_SHARES_PRESERVATION_V1.md',holding_positive_policy='KEEP_ACTUAL_SHARES',
        ...
        previous_goal_turn_classification='PROGRESS_ROUND192_TRAINED_VERIFIED_AND_DELIVERED',
    ...
    paths=[Path(__file__),ROOT/'research/entry_shares_preservation_inputs_v1.py',ROOT/'research/joint_account_acceptance_v1.py',
        ROOT/'research/saved_target_custom_execution_v1.py',ROOT/'research/saved_parent_target_alignment_v1.py',
        ROOT/'research/saved_entry_shares_checks_v1.py',ROOT/'research/entry_shares_preservation_account_v1.py',
        ROOT/'research/adaptive_allocation_v1.py',ROOT/'research/intraday_overnight_increment_v1.py',
        ROOT/'tests/test_entry_shares_preservation_v1.py',ROOT/'tests/test_median_continuation_v1.py',receipt,OUT/'tests_output.txt',rules,
    ...
    require(i['latest_completed_round']['round']==192,'第193轮前序不同')
    i['running_studies']=[{'round':193,'study':cfg['study_id'],'status':'FROZEN_NOT_STARTED','config':str(CONFIG.relative_to(ROOT))}]
    i['next_work'].update(registered=True,status='ENTRY_SHARES_PRESERVATION_FROZEN',source=cfg['rules'])
    write_json(p,i)
    print('第193轮已冻结。',flush=True)


def run():
    r=run_custom_execution_batch(ROOT,OUT,CONFIG,CANDIDATES,PARENTS,CONTROLS,preservation_frames,simulate_preserved_shares)
    cfg=json.loads(CONFIG.read_text(encoding='utf-8'))
    print(json.dumps(save_joint_assessment(OUT,cfg,r),ensure_ascii=False),flush=True)


def verify():
    require(not (OUT/'saved_verification_receipt.json').exists(),'第193轮已核对')
    cfg=json.loads(CONFIG.read_text(encoding='utf-8'))
    ...
    def expected(period,cost_id,frame,start):
        ...
        for origin,value in zip(origins,parent.reference_weight):
            target[origin]=value
        require(pd.DatetimeIndex(parent.decision_time).equals(pd.DatetimeIndex(parent.origin)+pd.Timedelta(hours=15,minutes=5)),'来源真实判断时钟不同')
        ...
        checks.append({'period':period,'cost':cost_id,'fills':int(filled.sum()),'maximum_target_error':0.,'intermediate_position_trades':int((filled & ledger.shares_before.gt(0) & ledger.shares.gt(0)).sum())})
        return target

    accounts,cycles,diffs,count=verify_preserved_shares_accounts(OUT,cfg,r,data,dividends,expected,comparison_models=list(CONTROLS))
    require(len(accounts)==4 and count==5646,'第193轮核对范围不同')
    pd.DataFrame(checks).to_csv(OUT/'saved_preserved_shares_checks.csv',index=False,encoding='utf-8-sig')
    receipt={'verified_at':now(),'status':'PASS_FOUR_SIMULATED_ACCOUNTS_ENTRY_SHARES_PRESERVATION_AND_COSTS',
