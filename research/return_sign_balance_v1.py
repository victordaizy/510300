"""冻结单一上涨日优势与累计方向规则，共读来源后运行四个完整账户。"""
import json
from pathlib import Path
from research.return_sign_balance_inputs_v1 import PRIMARY, CANDIDATES, return_sign_balance_frames
from research.saved_target_batch_runner_v1 import run_saved_target_batch
from research.intraday_overnight_increment_v1 import now, digest, require, write_json

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_return_sign_balance_v1"
CONFIG = ROOT / "config/510300_return_sign_balance_v1.json"
...
def freeze():
    require(not CONFIG.exists(), "第172轮上涨日优势与累计方向规则已经冻结")
    prior_path = ROOT / "config/510300_return_lag_state_v1.json"
    prior = json.loads(prior_path.read_text(encoding="utf-8"))
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "上涨日优势与累计方向六项必要测试尚未通过")
    require((ROOT / "reports/research/510300_return_lag_state_v1/acceptance_outcome.json").is_file(), "第171轮尚未完成关闭")
    ...
    cfg.update(study_id="510300_RETURN_SIGN_BALANCE_V1", round=172, primary=PRIMARY, candidate_configurations=1,
        candidate_models=list(CANDIDATES), registered_at=now(), decision_clock="15:05:00", parent_models=[],
        sign_window=60, momentum_window=60, volatility_window=20, risk_target=.1,
        entry_threshold=0., exit_threshold=0., confirmation_closes=2,
        sign_method="UP_MINUS_DOWN_DAYS_DIVIDED_BY_SIXTY_ACTUAL_ZERO_RETURN_BOUNDARY",
        flat_day_method="INCLUDED_IN_SIXTY_DENOMINATOR_ALL_FLAT_IS_KNOWN_ZERO",
        all_flat_window="KNOWN_NONPOSITIVE_DIRECTION",
        missing_window="WHOLE_WINDOW_UNKNOWN_RESET_DIRECTION_AND_ALL_THREE_COUNTERS",
        direction="TWO_POSITIVE_SIGN_BALANCE_AND_MOMENTUM_OR_SEPARATE_NONPOSITIVE_SIGN_BALANCE_MOMENTUM_EXIT",
        ...
        rules="docs/510300_RETURN_SIGN_BALANCE_V1.md", independent_validation="NOT_ESTABLISHED",
        previous_goal_turn_classification="PROGRESS_ROUND171_COMPLETED_CLOSED_AND_DELIVERED_FULL_GOAL_NOT_MET")
    paths = [Path(__file__), prior_path, ROOT / "research/return_sign_balance_inputs_v1.py",
        ...
        ROOT / "research/intraday_overnight_increment_v1.py", ROOT / "tests/test_return_sign_balance_v1.py",
        ...
        ROOT / "docs/510300_RETURN_SIGN_BALANCE_NEXT_20260910.md", ROOT / "config/510300_research_authority_v6.json",
        P143 / "saved_verification_receipt.json", P165 / "saved_verification_receipt.json",
        ROOT / "reports/research/510300_return_lag_state_v1/acceptance_outcome.json"]
    ...
    print("第172轮一个固定上涨日优势与累计方向规则已冻结，尚未生成新历史目标或账户。", flush=True)


def run():
    return run_saved_target_batch(ROOT, OUT, CONFIG, CANDIDATES, {}, CONTROLS, return_sign_balance_frames)
