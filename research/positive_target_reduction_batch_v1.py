...
from research.positive_target_reduction_batch_inputs_v1 import CANDIDATES, MODELS, PRIMARY, SETTINGS, allowance_column, gate_frames, simulate_gate_account
...
OUT = ROOT/'reports/research/510300_positive_target_reduction_batch_v1'
CONFIG = ROOT/'config/510300_positive_target_reduction_batch_v1.json'
SOURCE = ROOT/'reports/research/510300_finite_rebalance_band_batch_v1'
PARENTS = {m: SOURCE for m in MODELS}
CONTROLS = {'ADD_GATE_EXPOSURE_105': (ROOT/'reports/research/510300_addition_gate_exposure_batch_v1', '原206目标1.05倍'), 'ADD_GATE_BAND20_LOWER01': (ROOT/'reports/research/510300_addition_only_return_gate_batch_v1', '原205仅强势加仓'), MODELS[0]: (SOURCE, '原201零门槛合并'), MODELS[1]: (SOURCE, '原201二十个百分点合并'),
...
def prepare():
    ...
    test = subprocess.run([sys.executable, '-m', 'pytest', 'tests/test_positive_target_reduction_batch_v1.py', '-q', '-p', 'no:cacheprovider'],
        cwd=ROOT, capture_output=True, text=True, encoding='utf-8')
    (OUT/'tests_output.txt').write_text(test.stdout+test.stderr, encoding='utf-8')
    print(test.stdout, flush=True)
    require(test.returncode == 0 and '7 passed' in test.stdout, '七项买入过滤测试未通过')
    ...
    cfg.update(study_id='510300_POSITIVE_TARGET_REDUCTION_BATCH_V1', round=207, registered_at=now(), primary=PRIMARY,
        candidate_models=list(CANDIDATES), candidate_configurations=8, candidate_settings=SETTINGS, parent_models=MODELS,
        ...
        evidence_class='RETROSPECTIVE_POSITIVE_TARGET_REDUCTION_WITH_ZERO_EXIT_PRESERVED',
        prior_goal_turn_classification='PROGRESS_ROUND205_206_COMPLETED_48_ACCOUNTS_AND_REDUCTION_REQUEST_INVENTORY',
        rules='docs/510300_POSITIVE_TARGET_REDUCTION_BATCH_V1.md')
    text = (ROOT/'docs/510300_POSITIVE_TARGET_REDUCTION_BATCH_NEXT_20260913.md').read_text(encoding='utf-8')
    text = text.replace('本批目前只准备规则，尚未实现、测试、冻结或计算账户。', '本批在七项必要测试通过后、首次新账户计算前冻结规则。')
    text += '\n\n## 八套实际冻结设置\n\n|方案|已有持仓调仓门槛|已有持仓加仓的当日收益条件|目标倍率|普通减仓方式|\n|---|---:|---:|---:|---|\n'
    for model, s in SETTINGS.items():
        text += f"|{CANDIDATES[model]}|{s['band']:.0%}|{'不超过' if s['direction']=='UPPER' else '至少为'}{s['return_threshold']:.0%}|{s['exposure_multiplier']:.2f}|{CANDIDATES[model].split('、')[-1]}|\n"
    text += '\n\n## 来源全部中文因素和原进出场\n\n下文保留第201轮及内部参考规则，说明原目标如何产生。本批固定二十个百分点门槛及至少1%的加仓准入，目标倍率1.00或1.05，上限100%；普通正目标减仓使用上述方式，明确零及终点全部退出优先。旧轮次状态属于来源记录。\n\n'
    text += (ROOT/old['rules']).read_text(encoding='utf-8')
    with (ROOT/cfg['rules']).open('x', encoding='utf-8') as stream:
        stream.write(text)
    paths = [Path(__file__), ROOT/'research/positive_target_reduction_batch_inputs_v1.py',
        ROOT/'tests/test_positive_target_reduction_batch_v1.py', ROOT/'tests/test_two_close_zero_exit_v1.py',
    ...
    require(index['latest_completed_round']['round'] == 206 and not index['running_studies'], '前序完成状态不同')
    write_json(CONFIG, cfg, exclusive=True)
    index['running_studies'] = [{'round': 207, 'study': cfg['study_id'], 'status': 'FROZEN_NOT_STARTED', 'config': str(CONFIG.relative_to(ROOT))}]
    index['next_work'].update(registered=True, status='POSITIVE_TARGET_REDUCTION_BATCH_FROZEN', source=cfg['rules'])
    write_json(path, index)
    print('第207轮八套正目标减仓已冻结，计划三十二条账户。', flush=True)
...
def run():
    ...
    ranked.to_csv(OUT/'thirteen_setting_joint_comparison.csv', index=False, encoding='utf-8-sig')
...
def independent_reduction(requests, targets, lot, mode, upper, lower):
...
def verify():
    ...
    def expected(period, cost, frame, start, model):
        ...
        raw = np.minimum(1., originals[setting['parent']]*setting['exposure_multiplier'])
        ...
        targets[indices] = raw
        return targets, allowed, upper, lower
    ...
    for model in CANDIDATES:
        ...
        def target_callback(period, cost, frame, start):
            targets, allowed, upper, lower = expected(period, cost, frame, start, model)
            active.update(allowed=allowed, upper=upper, lower=lower)
            return targets
    ...
    receipt = {'verified_at': now(), 'status': 'PASS_THIRTY_TWO_POSITIVE_TARGET_REDUCTION_ACCOUNTS_AND_ZERO_EXIT_PRIORITY',
