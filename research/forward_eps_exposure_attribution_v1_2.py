...
MANIFEST=ROOT/'config/510300_forward_eps_exposure_attribution_v1_2_manifest.json'
...
def components(w_on,w_day,r_on,r_day,dividend_residual,cost):
    ...
    scale=np.maximum(1.,np.abs(w_on*r_on)+np.abs(w_day*r_day)+np.abs(w_on.mean()*r_on)+np.abs(w_day.mean()*r_day)
                     +np.abs((w_on-w_on.mean())*r_on)+np.abs((w_day-w_day.mean())*r_day)
                     +np.abs(dividend_residual)+np.abs(cost))
...
def freeze():
    paths=[Path(__file__),ROOT/'docs/510300_FORWARD_EPS_EXPOSURE_ATTRIBUTION_V1.md',
           ...
           ROOT/'docs/510300_FORWARD_EPS_EXPOSURE_ATTRIBUTION_V1_1.md',ROOT/'docs/510300_FORWARD_EPS_EXPOSURE_ATTRIBUTION_V1_2.md',
           ROOT/'research/forward_eps_exposure_attribution_v1_1.py',ROOT/'config/510300_forward_eps_exposure_attribution_v1_1_manifest.json',
           ROOT/'tests/test_forward_eps_exposure_attribution_v1_2.py',
...
def run(scope):
    ...
    out=ROOT/('reports/research/510300_forward_eps_exposure_attribution_v1_2_'+scope)
