...
OUT = ROOT / "reports/research/510300_panic_learned_signal_union_v1"
P46 = ROOT / "reports/research/510300_panic_learned_equal_blend_v1"
CONFIG = ROOT / "config/510300_panic_learned_signal_union_v1.json"
PRIMARY = "PANIC_LEARNED_UNION"
NAME = "急跌或学习任一持有，合并使用完整预算"
...
def freeze():
    ...
    cfg.update(study_id="510300_PANIC_LEARNED_SIGNAL_UNION_V1", round=64, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        target="ONE_IF_ANY_EXPERT_ON_AND_BOTH_KNOWN_ZERO_IF_BOTH_OFF_OTHERWISE_NO_VIEW", source_folder=str(P46.relative_to(ROOT)),
        signal_update="EVERY_CLOSE_NEXT_OPEN_RECOMPUTE_FROM_CURRENT_ACTUAL_ACCOUNT", new_model_fits=0, new_reference_accounts=0,
        rules="docs/510300_PANIC_LEARNED_SIGNAL_UNION_V1.md", chinese_saved_models="deliverables/510300广度筛选学习进入_第63轮_20260907/原学习模型逐月中文规则.md",
        position_impact=0, goal_achieved=False, independent_validation="NOT_ESTABLISHED")
    paths = [Path(__file__), ROOT / "research/signal_union_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ...
        ROOT / cfg["rules"], ROOT / cfg["chinese_saved_models"], ROOT / "tests/test_signal_union_v1.py", OUT / "tests_receipt.json",
    ...
    require(receipt["exit_code"] == 0, "信号并集和原组合账户验证尚未通过")
    cfg["previously_saved_half_allocation_verification_replays"] = 4
    cfg["frozen_files"] = [{"path": str(p.relative_to(ROOT)), "sha256": digest(p)} for p in paths]
    write_json(CONFIG, cfg, exclusive=True)
    print("第64轮一个完整预算并集设置已冻结，尚未计算新策略收益。", flush=True)


def run():
    cfg = json.loads(CONFIG.read_text(encoding="utf-8"))
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "并集研究冻结文件变化")
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        target = union_targets(states.panic_state, states.learned_state)
        ...
        states = states.rename(columns={"target": "original_half_target"})
        states["union_target"] = target
        states.to_parquet(OUT / f"{period}_states.parquet", index=False)
        origins = states.iloc[anchor:-1]
        for a in [0., 1.]:
            for b in [0., 1.]:
                counts.append({"period": period, "panic_state": a, "learned_state": b, "old_target": .5 * (a + b),
                    "union_target": max(a, b), "decision_origins": int((origins.panic_state.eq(a) & origins.learned_state.eq(b)).sum())})
        for cost_id, cost in cfg["costs"].items():
            ...
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "完整预算并集结算不符")
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "并集比较账户未保留同一完整日历")
            ...
            print(f"{period}／{cost_id}：完整预算并集和四个原样对照完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "PANIC_LEARNED_SIGNAL_UNION_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
        ...
        "new_model_fits": 0, "new_reference_accounts": 0, "previously_saved_half_allocation_verification_replays": 4,
