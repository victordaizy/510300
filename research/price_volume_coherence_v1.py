"""冻结单一价量相关与上涨动量规则，共读来源后运行四个完整账户。"""
import json
from pathlib import Path
from research.price_volume_coherence_inputs_v1 import PRIMARY, CANDIDATES, price_volume_coherence_frames
from research.saved_target_batch_runner_v1 import run_saved_target_batch
from research.intraday_overnight_increment_v1 import now, digest, require, write_json

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_price_volume_coherence_v1"
CONFIG = ROOT / "config/510300_price_volume_coherence_v1.json"
...
def freeze():
    require(not CONFIG.exists(), "第163轮价量相关与上涨动量规则已经冻结")
    prior_path = ROOT / "config/510300_session_signed_rank_v1.json"
    prior = json.loads(prior_path.read_text(encoding="utf-8"))
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "价量相关与上涨动量六项必要测试尚未通过")
    ...
    cfg.update(study_id="510300_PRICE_VOLUME_COHERENCE_V1", round=163, primary=PRIMARY, candidate_configurations=1,
        candidate_models=list(CANDIDATES), registered_at=now(), decision_clock="15:05:00", parent_models=[],
        correlation_window=20, momentum_window=20, volatility_window=20, risk_target=.1,
        ...
        rules="docs/510300_PRICE_VOLUME_COHERENCE_V1.md", independent_validation="NOT_ESTABLISHED",
        previous_goal_turn_classification="PROGRESS_ROUND162_COMPLETED_CLOSED_AND_DELIVERED_FULL_GOAL_NOT_MET")
    paths = [Path(__file__), prior_path, ROOT / "research/price_volume_coherence_inputs_v1.py",
        ...
        ROOT / "research/intraday_overnight_increment_v1.py", ROOT / "tests/test_price_volume_coherence_v1.py",
        ...
        ROOT / "docs/510300_PRICE_VOLUME_COHERENCE_NEXT_20260910.md", ROOT / "config/510300_research_authority_v6.json",
    ...
    print("第163轮一个固定价量相关与上涨动量规则已冻结，尚未生成新历史目标或账户。", flush=True)


def run():
    return run_saved_target_batch(ROOT, OUT, CONFIG, CANDIDATES, {}, CONTROLS, price_volume_coherence_frames)
