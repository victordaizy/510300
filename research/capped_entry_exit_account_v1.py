...
def simulate_capped_entry(data, dividends, config, cost, start, rule, spec, controller=None, gap_sigma=1.):
    ...
    def decide(t):
        ...
        daily_sigma = float(data.vol20.iloc[t]) / np.sqrt(config["annual_days"])
        ceiling = np.floor(cl[t] * (1 + gap_sigma * daily_sigma) / config["tick"] + 1e-9) * config["tick"] if np.isfinite(daily_sigma) else np.nan
        row["entry_volatility_daily_known"] = daily_sigma
        row["entry_price_ceiling_before_ex_adjustment"] = ceiling
    ...
    for day in range(first, last + 1):
        ...
        cap, cap_rejected, rejected_status = np.nan, False, None
        if quantity > 0:
            cap = np.floor((pending["entry_price_ceiling_before_ex_adjustment"] - distribution[day]) / config["tick"] + 1e-9) * config["tick"]
            if not np.isfinite(cap) or cap <= 0:
                cap_rejected, rejected_status = True, "UNFILLED_NO_VIEW_ENTRY_PRICE_CAP"
            elif fill_price(op[day], 1, cost, config["tick"]) > cap + 1e-9:
                cap_rejected, rejected_status = True, "UNFILLED_ENTRY_PRICE_ABOVE_PRESET_CAP"
        execution = execute_order(account, 0 if cap_rejected else quantity, op[day], previous[day], distribution[day], day, cost, config)
        if cap_rejected:
            execution.update(requested_quantity=quantity, status=rejected_status)
        ...
        records.append({"date": dates[day], "open": op[day], "mark": mark, "mark_clock": "OPEN_TERMINAL" if terminal else "CLOSE",
                        ...
                        "entry_price_ceiling_after_ex_adjustment": cap,
                        "entry_price_cap_rejected": cap_rejected,
                        **execution})
