"""原周期内模型加入当前日内相对隔夜强弱，九项系数从完整成熟周期内部变化学习。"""
...
FEATURES = BASE_FEATURES+["d60_factor"]
CN = BASE_CN+["当前六十日日内相对隔夜强弱"]
from research.intraday_overnight_increment_v1 import require


def fit_session_strength_within(rows, cfg):
    ...
    return {"kind": "SESSION_STRENGTH_WITHIN_CYCLE_RIDGE", "features": FEATURES.copy(), "mean": mean.tolist(), "scale": scale.tolist(),
...
def session_strength_prediction(model, values):
    require(model["kind"] == "SESSION_STRENGTH_WITHIN_CYCLE_RIDGE" and model["features"] == FEATURES, "周期内预测模型身份不同")
...
class SessionStrengthWithinController:
    def __init__(self, data, models, factors, confirmation_days=2):
        self.data, self.models, self.factors = data, models, factors
        require(pd.DatetimeIndex(data.date).equals(pd.DatetimeIndex(factors.date)), "日内隔夜强弱因子与实际账户日历不同")
    ...
    def __call__(self, t, cycle, current_value, peak_value):
        ...
        x = np.r_[state_values(self.data, t, cycle, current_value, peak_value), float(self.factors.d60_factor.iloc[t])]
        ...
        if record and record["status"] == "FIT_COMPLETE":
            ...
            if np.isfinite(x).all():
                value, status = session_strength_prediction(record["model"], x), "PREDICTION_AVAILABLE"
            else:
