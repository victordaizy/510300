...
OUT = ROOT / "reports/research/510300_active_risk_budget_v1"
CONFIG = ROOT / "config/510300_active_risk_budget_v1.json"
P46 = ROOT / "reports/research/510300_panic_learned_equal_blend_v1"
PRIMARY = "ACTIVE_RISK_BUDGET"
P76 = ROOT / "reports/research/510300_two_policy_risk_budget_v1"
NAME = "按实际承担股票风险日估计两策略风险"
CONTROLS = {"TWO_POLICY_RISK_BUDGET": "第76轮完整日风险预算", "PANIC_LEARNED_HALF": "原急跌与学习各半", "REARM_RIDGE": "原学习退出及等待新机会", "PANIC_ONLY": "原急跌回升单独策略", "BUY_HOLD": "买入持有"}
...
def freeze():
    ...
    cfg.update(study_id="510300_ACTIVE_RISK_BUDGET_V1", round=77, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        risk_window=242, risk_clock="MONTH_FIRST_COMPLETE_CLOSE_NEXT_OPEN", state_cost="BASE", initial_budgets=[.5, .5],
        missing_or_zero_risk="NO_VIEW_KEEP_PREVIOUS_BUDGET", rules="docs/510300_ACTIVE_RISK_BUDGET_V1.md",
        new_model_fits=0, new_reference_accounts=0, risk_condition="OPENING_OR_CLOSING_ACTUAL_SHARES_POSITIVE", minimum_active_days=2,
    ...
    paths = [Path(__file__), ROOT / "research/active_risk_budget_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ...
        ROOT / "research/simple_signal_blend_v1.py", ROOT / cfg["features"], ROOT / cfg["dividends"], ROOT / cfg["rules"],
        ROOT / "tests/test_active_risk_budget_v1.py", OUT / "tests_receipt.json", ROOT / "config/510300_research_authority_v6.json",
        ROOT / "docs/510300_TWO_POLICY_RISK_BUDGET_V1.md", ROOT / "research/two_policy_risk_budget_inputs_v1.py"]
    for period in ["evaluation", "earlier_diagnostic"]:
        ...
        for cost in cfg["costs"]:
            paths.extend((P76 if model == "TWO_POLICY_RISK_BUDGET" else P46) / period / cost / f"{model}_ledger.parquet" for model in CONTROLS)
    cfg["frozen_files"] = [{"path": str(p.relative_to(ROOT)), "sha256": digest(p)} for p in paths]
    write_json(CONFIG, cfg, exclusive=True)
    print("第77轮一个242日窗口内直接股票风险日条件风险设置已冻结，尚无新策略收益。", flush=True)
...
def run():
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        factors = active_budget_frame(frame.date, reference_returns, mapped, first, active_exposure, cfg["risk_window"])
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            for model, name in CONTROLS.items():
                saved = pd.read_parquet((P76 if model == "TWO_POLICY_RISK_BUDGET" else P46) / period / cost_id / f"{model}_ledger.parquet")
            ...
            print(f"{period}／{cost_id}：风险预算新账户和五个保存对照完成。", flush=True)
    ...
    write_json(OUT / "result.json", {"study_id": cfg["study_id"], "completed_at": now(), "status": "ACTIVE_RISK_BUDGET_ACCOUNTS_COMPLETE",
        "candidate_configurations": 1, "evaluation_accounts": 12, "new_accounts_generated": 2, "reused_control_accounts": 10,
        "earlier_diagnostic_accounts": 12, "new_earlier_diagnostic_accounts": 2, "reused_earlier_accounts": 10,
