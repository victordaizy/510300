"""冻结月度正均值预算，读取保存父收益及目标后计算四个独立账户。"""
import json
import pandas as pd
from pathlib import Path
from research.runs_net_profit_budget_inputs_v1 import PRIMARY, MODELS, CANDIDATES, runs_net_profit_budget_frames
from research.saved_target_batch_runner_v1 import run_saved_target_batch
from research.intraday_overnight_increment_v1 import now, digest, require, write_json

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT/'reports/research/510300_runs_net_profit_budget_v1'
CONFIG = ROOT/'config/510300_runs_net_profit_budget_v1.json'
...
CONTROLS = {'RUNS_OPPORTUNITY_CAPPED_SUM': (P168, '第168轮相加封顶'),
...
def freeze():
    require(not CONFIG.exists(), '第169轮月度正均值预算已经冻结')
    prior_path = ROOT/'config/510300_runs_opportunity_union_v1.json'
    prior = json.loads(prior_path.read_text(encoding='utf-8'))
    tests = json.loads((OUT/'tests_receipt.json').read_text(encoding='utf-8'))
    require(tests['exit_code'] == 0 and tests['passed'] == 6, '第169轮六项必要测试尚未通过')
    require((P168/'acceptance_outcome.json').is_file() and (P168/'saved_verification_receipt.json').is_file(), '第168轮进场合并尚未完成核对及关闭')
    ...
    cfg.update(study_id='510300_RUNS_NET_PROFIT_BUDGET_V1', round=169, primary=PRIMARY, candidate_configurations=1,
        candidate_models=list(CANDIDATES), registered_at=now(), decision_clock='15:05:00', parent_models=MODELS,
        profit_window=242, initial_budgets=[.5, .5], budget_update='FIRST_TRADING_DAY_MONTH_CLOSE',
        budget_source_cost='BASE', return_clock='CLOSE_ONLY_TERMINAL_OPEN_EXCLUDED',
        nonpositive_means='BOTH_BUDGETS_ZERO',
        new_model_fits=0, new_reference_accounts=0,
        combination='MONTHLY_POSITIVE_MEANS_OF_SAVED_BASE_PARENT_NET_RETURNS',
        unknown_parent='ANY_UNKNOWN_PARENT_MAKES_COMBINED_TARGET_UNKNOWN', parent_feedback='NONE',
        source_budget_cny=0, goal_achieved=False, position_impact=0, rules='docs/510300_RUNS_NET_PROFIT_BUDGET_V1.md',
        ...
        previous_goal_turn_classification='PROGRESS_ROUND168_COMPLETED_CLOSED_AND_DELIVERED_FULL_GOAL_NOT_MET')
    paths = [Path(__file__), prior_path, ROOT/'research/runs_net_profit_budget_inputs_v1.py',
        ...
        ROOT/'research/intraday_overnight_increment_v1.py', ROOT/'tests/test_runs_net_profit_budget_v1.py',
        ...
        ROOT/'docs/510300_RUNS_NET_PROFIT_BUDGET_NEXT_20260910.md', ROOT/'config/510300_research_authority_v6.json',
        P168/'acceptance_outcome.json', P168/'saved_verification_receipt.json',
    ...
    for period in ['evaluation', 'earlier_diagnostic']:
        for cost in cfg['costs']:
            for model, (folder, _) in CONTROLS.items():
                ...
                if model != 'RUNS_OPPORTUNITY_CAPPED_SUM':
    ...
    print('第169轮二百四十二日月度正均值预算已冻结，尚未生成新账户。', flush=True)
...
def run():
    ...
    def build_frames(frame, sources, cfg, start):
        ...
        factors, summaries, events = runs_net_profit_budget_frames(frame, sources, base_ledgers, cfg, start)
