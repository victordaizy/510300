...
def simulate_entry_sized_exit(data, dividends, config, cost, start, rule, spec, controller=None, fraction_provider=None):
    ...
    def decide(t):
        ...
        if account.shares:
            ...
            fraction = 1. if fraction_provider is None else fraction_provider(t)
            if fraction is None:
                qty, selected_mode, action = 0, mode, "初始仓位输入缺失，不新买入且保留进入许可"
                sizing = {"entry_fraction": None, "entry_budget_cny": None, "entry_sizing_status": "NO_VIEW_ENTRY_SIZING_INPUT"}
            else:
                require(np.isfinite(fraction) and 0 < fraction <= 1, "初始股票预算比例超出允许范围")
                budget = min(account.cash, account.value(cl[t]) * fraction)
                qty = affordable_quantity(budget, fill_price(cl[t], 1, cost, config["tick"]), cost, config["lot"])
                selected_mode, action = mode, "入场条件成立，下一开盘请求买入"
                sizing = {"entry_fraction": float(fraction), "entry_budget_cny": float(budget), "entry_sizing_status": "ENTRY_BUDGET_AVAILABLE"}
    ...
    for day in range(first, last + 1):
        ...
        if old_shares == 0 and account.shares > 0:
            ...
            cycle = {"cycle_id": cycle_number, "mode": int(pending["entry_mode"]), "entry_date": dates[day], "entry_index": day,
                     ...
                     "entry_fraction": pending["entry_fraction"], "entry_budget_cny": pending["entry_budget_cny"]}
