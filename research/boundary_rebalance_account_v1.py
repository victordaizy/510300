"""使用独立边界请求的完整账户，保留原费用、整手、分红和T+1记账。"""
from __future__ import annotations
import numpy as np
import pandas as pd
from research.adaptive_allocation_v1 import Account, affordable_quantity, fill_price, choose_order, execute_order, require
from research.boundary_rebalance_inputs_v1 import boundary_request as target_request

def simulate_boundary_account(data: pd.DataFrame, dividends: pd.DataFrame, config: dict, cost: dict, start: str,
             model_id: str, targets: np.ndarray | None = None, prediction: np.ndarray | None = None,
             horizon: int = 1, event_mask: np.ndarray | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    require(model_id == "BOUNDARY_REBALANCE" and prediction is None and targets is not None, "本入口仅支持边界目标账户")
    ...
    def decision_at(t: int) -> dict:
        ...
        if model_id == "BUY_HOLD":
            ...
            if np.isfinite(value):
                ...
                result = choose_order(account, price, value, horizon * variance[t], cost, config) if prediction is not None else target_request(account, price, value, config)
            else:
