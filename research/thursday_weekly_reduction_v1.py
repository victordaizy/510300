...
from research.thursday_weekly_reduction_inputs_v1 import CANDIDATES,MODELS,PARENT,PRIMARY,weekly_frames
...
OUT=ROOT/'reports/research/510300_thursday_weekly_reduction_v1'
CONFIG=ROOT/'config/510300_thursday_weekly_reduction_v1.json'
...
def prepare():
    require(not CONFIG.exists() and not (OUT/'RUN_STARTED.json').exists(),'第189轮已有冻结或运行')
    OUT.mkdir(parents=True,exist_ok=True)
    receipt=OUT/'tests_receipt.json'
    require(not receipt.exists(),'必要测试已有回执')
    began=time.perf_counter()
    t=subprocess.run([sys.executable,'-m','pytest','tests/test_thursday_weekly_reduction_v1.py','-q','-p','no:cacheprovider'],
                     cwd=ROOT,capture_output=True,text=True,encoding='utf-8')
    (OUT/'tests_output.txt').write_text(t.stdout+t.stderr,encoding='utf-8')
    print(t.stdout,flush=True)
    require(t.returncode==0 and '4 passed' in t.stdout,'第189轮测试未通过')
    ...
    cfg.update(study_id='510300_THURSDAY_WEEKLY_REDUCTION_V1',round=189,primary=PRIMARY,candidate_models=list(CANDIDATES),
        parent_models=MODELS,candidate_configurations=1,registered_at=now(),decision_clock='15:05:00',
        rules='docs/510300_THURSDAY_WEEKLY_REDUCTION_V1.md',zero_origin_weekday=3,
        ...
        previous_goal_turn_classification='PROGRESS_ROUND188_EIGHT_ACCOUNTS_AND_SESSION_ATTRIBUTION',
    ...
    with rules.open('x',encoding='utf-8') as stream:
        stream.write((ROOT/'docs/510300_THURSDAY_WEEKLY_REDUCTION_NEXT_20260913.md').read_text(encoding='utf-8'))
        stream.write('\n\n## 来源全部中文因素及进入退出规则\n\n')
        stream.write((ROOT/old['rules']).read_text(encoding='utf-8'))
    paths=[Path(__file__),ROOT/'research/thursday_weekly_reduction_inputs_v1.py',ROOT/'research/joint_account_acceptance_v1.py',
        ...
        ROOT/'tests/test_thursday_weekly_reduction_v1.py',receipt,OUT/'tests_output.txt',rules,
    ...
    require(i['latest_completed_round']['round']==188,'第189轮前序不同')
    i['running_studies']=[{'round':189,'study':cfg['study_id'],'status':'FROZEN_NOT_STARTED','config':str(CONFIG.relative_to(ROOT))}]
    i['next_work'].update(registered=True,status='THURSDAY_WEEKLY_REDUCTION_FROZEN',source=cfg['rules'])
    write_json(p,i)
    print('第189轮已冻结。',flush=True)


def run():
    r=run_saved_target_batch(ROOT,OUT,CONFIG,CANDIDATES,PARENTS,CONTROLS,weekly_frames)
    cfg=json.loads(CONFIG.read_text(encoding='utf-8'))
    print(json.dumps(save_joint_assessment(OUT,cfg,r),ensure_ascii=False),flush=True)


def verify():
    require(not (OUT/'saved_verification_receipt.json').exists(),'第189轮已核对')
    cfg=json.loads(CONFIG.read_text(encoding='utf-8'))
    ...
    def expected(period,cost_id,frame,start):
        ...
        for origin,value in zip(origins,parent.reference_weight):
            target[origin]=0. if frame.date.iloc[origin].weekday()==3 else value
        ...
        np.testing.assert_array_equal(factors.origin_weekday,[d.weekday() for d in frame.date])
    ...
    require(len(accounts)==4 and count==5646,'第189轮核对范围不同')
    pd.DataFrame(checks).to_csv(OUT/'saved_weekly_reduction_checks.csv',index=False,encoding='utf-8-sig')
    receipt={'verified_at':now(),'status':'PASS_FOUR_SIMULATED_ACCOUNTS_THURSDAY_WEEKLY_REDUCTION_AND_COSTS',
