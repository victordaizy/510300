...
from research.close_return_buy_strength_gate_batch_inputs_v1 import CANDIDATES, MODELS, PRIMARY, SETTINGS, gate_frames, simulate_gate_account
...
OUT = ROOT/'reports/research/510300_close_return_buy_strength_gate_batch_v1'
CONFIG = ROOT/'config/510300_close_return_buy_strength_gate_batch_v1.json'
...
def prepare():
    ...
    test = subprocess.run([sys.executable, '-m', 'pytest', 'tests/test_close_return_buy_strength_gate_batch_v1.py', '-q', '-p', 'no:cacheprovider'],
    ...
    cfg.update(study_id='510300_CLOSE_RETURN_BUY_STRENGTH_GATE_BATCH_V1', round=204, registered_at=now(), primary=PRIMARY,
        ...
        evidence_class='RETROSPECTIVE_CLOSE_STRENGTH_DIRECTION_AFTER_UPPER_GATE_FAILURE',
        prior_goal_turn_classification='PROGRESS_ROUND203_COMPLETED_16_ACCOUNTS_AND_REJECTED_UPPER_GATE',
        rules='docs/510300_CLOSE_RETURN_BUY_STRENGTH_GATE_BATCH_V1.md')
    text = (ROOT/'docs/510300_CLOSE_RETURN_BUY_STRENGTH_GATE_BATCH_NEXT_20260913.md').read_text(encoding='utf-8')
    text = text.replace('本批目前只准备规则，尚未实现、测试、冻结或计算账户。', '本批在五项必要测试通过后、首次新账户计算前冻结规则。')
    text += '\n\n## 四套实际冻结设置\n\n|方案|已有持仓调仓门槛|允许新增买入的当日涨幅下限|\n|---|---:|---:|\n'
    for model, s in SETTINGS.items():
        text += f"|{CANDIDATES[model]}|{s['band']:.0%}|{s['return_floor']:.0%}|\n"
    ...
    with (ROOT/cfg['rules']).open('x', encoding='utf-8') as stream:
        stream.write(text)
    paths = [Path(__file__), ROOT/'research/close_return_buy_strength_gate_batch_inputs_v1.py',
        ROOT/'tests/test_close_return_buy_strength_gate_batch_v1.py', ROOT/'tests/test_two_close_zero_exit_v1.py',
    ...
    require(index['latest_completed_round']['round'] == 203 and not index['running_studies'], '前序完成状态不同')
    write_json(CONFIG, cfg, exclusive=True)
    index['running_studies'] = [{'round': 204, 'study': cfg['study_id'], 'status': 'FROZEN_NOT_STARTED', 'config': str(CONFIG.relative_to(ROOT))}]
    index['next_work'].update(registered=True, status='CLOSE_RETURN_BUY_STRENGTH_GATE_BATCH_FROZEN', source=cfg['rules'])
    write_json(path, index)
    print('第204轮四套买入过滤已冻结，计划十六条账户。', flush=True)
...
def verify():
    ...
    def expected(period, cost, frame, start, model):
        ...
        allowed = market[f"buy_allowed_r{int(setting['return_floor']*100):02d}"].iloc[indices].to_numpy(bool)
        ...
        require(decisions.loc[known, 'used_band'].eq(setting['band']).all() and decisions.loc[known, 'return_floor'].eq(setting['return_floor']).all(), '过滤设置不同')
        ...
        for j in np.flatnonzero(blocked):
            suppressions.append({'model': model, 'period': period, 'cost': cost, 'origin': decisions.origin.iloc[j],
                ...
                'observed_daily_return': market.daily_total_simple.iloc[indices[j]], 'return_floor': setting['return_floor']})
