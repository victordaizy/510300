"""一个价格路径曲折度状态进入信号，复用真实账户引擎完成两段两费用。"""
import json
import time
from pathlib import Path
import numpy as np
import pandas as pd
from research.choppiness_state_entry_inputs_v1 import factor_frame, rules, attach_factor_context
from research.simple_price_entry_exit_v1 import simulate_policy
from research.adaptive_allocation_v1 import normalize_dividends, save_account, summarize
from research.intraday_overnight_increment_v1 import digest, now, require, write_json

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_choppiness_state_entry_v1"
CONFIG = ROOT / "config/510300_choppiness_state_entry_v1.json"
P91 = ROOT / "reports/research/510300_continuous_reference_min_variance_v1"
P32 = ROOT / "reports/research/510300_rearmed_session_exit_v1"
PRIMARY = "CHOPPINESS_STATE_ENTRY"
NAME = "价格路径曲折度的趋势震荡进出场"
CONTROLS = {"REARM_NONE": (P32, "原日内强弱进入及自然退出"), "CONTINUOUS_REFERENCE_MIN_VARIANCE": (P91, "原第91轮局部候选"), "BUY_HOLD": (P32, "买入持有")}


def freeze():
    require(not CONFIG.exists(), "价格路径曲折度状态进入策略已经登记")
    ...
    price = json.loads((ROOT / "config/510300_simple_price_entry_exit_v1.json").read_text(encoding="utf-8"))
    cfg.update(study_id="510300_CHOPPINESS_STATE_ENTRY_V1", round=105, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        choppiness_period=14, lower_threshold=38.2, upper_threshold=61.8, state_clock="CAUSAL_RETAIN_MIDDLE_RESET_MISSING",
        specification=price["candidate_specs"]["S1_TREND_REBOUND"], entry_clock="COMPLETE_CLOSE_NEXT_OPEN", price_scale="DIVIDEND_ADJUSTED_CONTINUOUS_HIGH_LOW_CLOSE",
        rules="docs/510300_CHOPPINESS_STATE_ENTRY_V1.md", new_model_fits=0, new_reference_accounts=0, goal_achieved=False, independent_validation="NOT_ESTABLISHED", position_impact=0)
    tests = json.loads((OUT / "tests_receipt.json").read_text(encoding="utf-8"))
    require(tests["exit_code"] == 0 and tests["passed"] == 5, "曲折度状态必要测试未通过")
    paths = [Path(__file__), ROOT / "research/choppiness_state_entry_inputs_v1.py", ROOT / "research/simple_price_entry_exit_v1.py", ROOT / "research/adaptive_allocation_v1.py", ROOT / "research/intraday_overnight_increment_v1.py",
        ROOT / "tests/test_choppiness_state_entry_v1.py", OUT / "tests_receipt.json", ROOT / cfg["features"], ROOT / cfg["dividends"], ROOT / cfg["rules"], ROOT / "config/510300_aroon_time_entry_v1.json", ROOT / "config/510300_simple_price_entry_exit_v1.json"]
    ...
    print("第105轮单一价格路径曲折度状态进入设置已冻结，尚无新策略账户收益。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "曲折度状态冻结输入改变")
    write_json(OUT / "RUN_STARTED.json", {"started_at": now(), "config_sha256": digest(CONFIG)}, exclusive=True)
    data = pd.read_parquet(ROOT / cfg["features"])
    dividends = normalize_dividends(pd.read_csv(ROOT / cfg["dividends"]))
    factors = factor_frame(data, cfg["choppiness_period"], cfg["lower_threshold"], cfg["upper_threshold"])
    factors.to_parquet(OUT / "choppiness_factors.parquet", index=False)
    factors.to_csv(OUT / "全部曲折度状态因子.csv", index=False, encoding="utf-8-sig")
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main), ("earlier_diagnostic", data[data.date.le(cfg["earlier_terminal"])], cfg["earlier_start"], earlier)]:
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "曲折度状态账户未完整结算")
            coverage.append({"period": period, "cost": cost_id, "holding_closes": int(ledger.shares.gt(0).sum()), "buy_trades": int(ledger.filled_quantity.gt(0).sum()), "sell_trades": int(ledger.filled_quantity.lt(0).sum()),
                "unfilled_requests": int((ledger.requested_quantity.ne(0) & ledger.filled_quantity.eq(0)).sum()), "no_view_factor_origins": int(decisions.factor_status.ne("CHOPPINESS_STATE_AVAILABLE").sum()),
                "mean_exposure": float(ledger.exposure.mean()), "completed_cycles": len(cycles), "raw_entry_origins": int(decisions.raw_entry.ne(0).sum())})
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "CHOPPINESS_STATE_ENTRY_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
        ...
        "run_seconds": time.perf_counter()-started, "post_selected_best_base": next(m for m in primary if m["cost"] == "BASE"), "historical_point_target_met": any(m["meets_point_target"] for m in primary), "goal_achieved": False, "independent_validation": "NOT_ESTABLISHED", "position_impact": 0}
