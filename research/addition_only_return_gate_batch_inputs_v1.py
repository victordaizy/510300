"""原初次入场与退出保持，只对实际持股后的正加仓申请应用收益条件。"""
import numpy as np
...
SETTINGS, CANDIDATES = {}, {}
for band in [0, 20]:
    for direction, label in [('UPPER', '不超过'), ('LOWER', '至少为')]:
        for percent in [0, 1]:
            model = f'ADD_GATE_BAND{band:02d}_{direction}{percent:02d}'
            SETTINGS[model] = {'parent': f'INTENT_MIX_BAND_{band:02d}', 'band': band/100,
                'direction': direction, 'return_threshold': percent/100}
            CANDIDATES[model] = f'{band}个百分点调仓、已有持仓仅当日收益{label}{percent}%才加仓'
PRIMARY = 'ADD_GATE_BAND00_LOWER00'
...
def gate_request(account, price, raw, cfg, model, allowed, daily_return):
    ...
    result.update(output_candidate=model, budget_source=setting['parent'], used_band=float(setting['band']),
        filter_direction=setting['direction'], return_threshold=float(setting['return_threshold']),
