...
from research.peak_age_exit_inputs_v1 import FEATURES, CN, fit_peak_age_exit, PeakAgeExitController, ADDED, attach_peak_age
from research.rearmed_cycle_exit_account_v1 import simulate_rearmed_exit
from research.simple_intraday_protection_v1 import make_rules

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_peak_age_exit_v1"
CONFIG = ROOT / "config/510300_peak_age_exit_v1.json"
...
def freeze():
    ...
    require(tests["exit_code"] == 0 and tests["passed"] == 6, "九项持仓高点时间与成熟成交测试尚未通过")
    cfg.update(study_id="510300_PEAK_AGE_EXIT_V1", round=113, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        fit_intercept=True, solver="svd", added_factor_definition="LOG_ONE_PLUS_TRADING_DAYS_SINCE_LAST_OWN_CYCLE_PEAK_ATTAINMENT", reference_decisions="reports/research/510300_learned_cycle_exit_v1/reference/D60_INTRA_decisions.parquet", reference_cycles="reports/research/510300_learned_cycle_exit_v1/reference/D60_INTRA_cycles.csv", missing_training_features="WHOLE_MONTH_NO_VIEW_PRESERVE_ALL_ORIGINAL_ROWS", model_loss="CYCLE_EQUAL_WEIGHTED_SQUARE_ERROR_WITH_ORIGINAL_RIDGE_PENALTY",
        feature_columns=FEATURES.copy(), feature_names=CN.copy(), added_feature_columns=[ADDED], dropped_feature_columns=[], planned_model_fits=114,
        fit_failure="NO_VIEW_MODEL_FIT_FAILED_KEEP_ORIGINAL_EXITS_NO_FALLBACK_SOLVER", rules="docs/510300_PEAK_AGE_EXIT_V1.md",
        samples=str((P31 / "all_reference_samples.parquet").relative_to(ROOT)), new_reference_accounts=0, source_budget_cny=0,
        previous_goal_turn_classification="PROGRESS_ROUNDS111_112_COMPLETED_AND_DELIVERED", position_impact=0, goal_achieved=False,
    ...
    paths = [Path(__file__), ROOT / "research/peak_age_exit_inputs_v1.py", ROOT / "research/rearmed_cycle_exit_account_v1.py",
        ...
        ROOT / cfg["rules"], ROOT / cfg["features"], ROOT / cfg["dividends"], ROOT / cfg["saved_models"], ROOT / cfg["samples"], ROOT / cfg["reference_decisions"], ROOT / cfg["reference_cycles"],
        ...
        ROOT / "tests/test_peak_age_exit_v1.py", ROOT / "tests/test_median_continuation_v1.py", OUT / "tests_receipt.json"]
    ...
    print("第113轮一个条件九项持仓高点时间继续价值设置已冻结，尚未拟合或读取新账户收益。", flush=True)
...
def train_models(data, samples, originals, cfg):
    ...
    for original in originals:
        ...
        if eligible and not missing_rows:
            try:
                stored = fit_peak_age_exit(rows, cfg)
                status = "FIT_COMPLETE"
    ...
    print(f"原成熟时点不变：九项持仓高点时间拟合完成{sum(r['status']=='FIT_COMPLETE' for r in receipts)}次，未成熟或求解失败{sum(r['status']!='FIT_COMPLETE' for r in receipts)}次。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "九项持仓高点时间冻结来源改变")
    ...
    reference_decisions = pd.read_parquet(ROOT / cfg["reference_decisions"])
    reference_cycles = pd.read_csv(ROOT / cfg["reference_cycles"])
    samples, states = attach_peak_age(samples, reference_decisions, reference_cycles)
    states.to_parquet(OUT / "reference_peak_states.parquet", index=False)
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], PeakAgeExitController(frame, models, cfg["confirmation_days"]))
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "九项持仓高点时间账户结算不符")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "九项持仓高点时间退出违反次日可卖")
            ...
            coverage.append({"period": period, "cost": cost_id, "holding_decisions": len(holding),
                "unknown_peak_age_rows": int(holding.peak_age_status.ne("PEAK_AGE_AVAILABLE").sum()), "model_available_rows": int(holding.learning_status.eq("PREDICTION_AVAILABLE").sum()), "no_model_rows": int(holding.learning_status.ne("PREDICTION_AVAILABLE").sum()),
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "九项持仓高点时间继续收益退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "九项持仓高点时间及对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新九项持仓高点时间账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "PEAK_AGE_EXIT_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
