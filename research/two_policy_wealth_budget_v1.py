...
OUT = ROOT / "reports/research/510300_two_policy_wealth_budget_v1"
CONFIG = ROOT / "config/510300_two_policy_wealth_budget_v1.json"
...
PRIMARY = "TWO_POLICY_WEALTH_BUDGET"
NAME = "两条原策略累计净值自然分配预算"
...
def freeze():
    ...
    cfg.update(study_id="510300_TWO_POLICY_WEALTH_BUDGET_V1", round=84, objective="RELATIVE_CUMULATIVE_REFERENCE_EQUITY", registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        budget_clock="EACH_COMPLETE_CLOSE_NEXT_OPEN", state_cost="BASE", initial_budgets=[.5, .5],
        missing_or_invalid_equity="NO_VIEW_KEEP_PREVIOUS_BUDGET", rules="docs/510300_TWO_POLICY_WEALTH_BUDGET_V1.md",
        new_model_fits=0, new_reference_accounts=0, previous_goal_turn_classification="PROGRESS_ROUND83_COMPLETE",
    ...
    paths = [Path(__file__), ROOT / "research/two_policy_wealth_budget_inputs_v1.py", ROOT / "research/event_clock_account_v1.py",
        ...
        ROOT / "tests/test_two_policy_wealth_budget_v1.py", OUT / "tests_receipt.json", ROOT / "config/510300_research_authority_v6.json"]
    ...
    print("第84轮单项累计净值预算已冻结，尚无新策略收益。", flush=True)
...
def run():
    ...
    for period, frame, start, destination in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        reference_equities = np.full((len(frame), 2), np.nan)
        for column, model in enumerate(["PANIC_ONLY", "REARM_RIDGE"]):
            ...
            reference_equities[first:, column] = ref.equity.to_numpy(float)
        factors = budget_frame(frame.date, reference_equities, mapped, first)
        factors["panic_reference_equity"], factors["learned_reference_equity"] = reference_equities[:, 0], reference_equities[:, 1]
        ...
        for row in factors[factors.budget_update_scheduled].to_dict("records"):
    ...
    for name, rows in [("metrics.csv", main), ("earlier_diagnostics.csv", earlier), ("yearly_metrics.csv", yearly), ("era_metrics.csv", eras),
        ("budget_update_records.csv", updates), ("account_coverage.csv", coverage)]:
        pd.DataFrame(rows).to_csv(OUT / name, index=False, encoding="utf-8-sig")
    primary = [m for m in main if m["model"] == PRIMARY]
    write_json(OUT / "result.json", {"study_id": cfg["study_id"], "completed_at": now(), "status": "TWO_POLICY_WEALTH_BUDGET_ACCOUNTS_COMPLETE",
        ...
        "post_selected_best_base": next(m for m in primary if m["cost"] == "BASE"), "budget_update_count": len(updates), "account_coverage": coverage,
