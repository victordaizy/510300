"""拟合有限的八曲线退出模型，保留月度时钟并计算四个真实资金账户。"""
import json
import time
...
from research.marginal_monotone_exit_inputs_v1 import build_monthly_models, MarginalMonotoneExitController
from research.rearmed_cycle_exit_account_v1 import simulate_rearmed_exit
from research.simple_intraday_protection_v1 import make_rules

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_marginal_monotone_exit_v1"
CONFIG = ROOT / "config/510300_marginal_monotone_exit_v1.json"
P114 = ROOT / "reports/research/510300_within_cycle_exit_v1"
PRIMARY = "MARGINAL_MONOTONE_EXIT"
CONTROLS = {
...
def freeze():
    require(not CONFIG.exists(), "八曲线退出策略已经登记")
    ...
    require(tests["exit_code"] == 0 and tests["passed"] == 9, "九项八曲线退出必要测试尚未通过")
    cfg.update(study_id="510300_MARGINAL_MONOTONE_EXIT_V1", round=159, registered_at=now(), primary=PRIMARY,
        ...
        curve_weight=.125, direction_rule="SIGN_OF_CYCLE_WEIGHTED_COVARIANCE",
        ...
        rules="docs/510300_MARGINAL_MONOTONE_EXIT_V1.md", new_reference_accounts=0, source_budget_cny=0,
        independent_validation="NOT_ESTABLISHED", goal_achieved=False, position_impact=0,
        previous_goal_turn_classification="PROGRESS_ROUND158_COMPLETED_CLOSED_AND_DELIVERED")
    names = ["research/marginal_monotone_exit_inputs_v1.py", "research/learned_cycle_exit_v1.py", "research/within_cycle_exit_inputs_v1.py",
        ...
        "tests/test_marginal_monotone_exit_v1.py", "tests/test_median_continuation_v1.py", "config/510300_research_authority_v6.json",
        "docs/510300_MARGINAL_MONOTONE_EXIT_NEXT_20260910.md"]
    ...
    known = {}
    for name in ["510300_probability_payoff_exit_v1"]:
    ...
    print("第159轮一套八曲线退出规则已冻结，尚未拟合新模型或计算新账户。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "八曲线退出冻结来源改变")
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], MarginalMonotoneExitController(frame, models, cfg["confirmation_days"]))
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "八曲线退出账户未完整结算")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "八曲线退出违反次日可卖")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "原八因素保序曲线等权平均后固定版本退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "八曲线退出与对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新八曲线退出账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "MARGINAL_MONOTONE_EXIT_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
