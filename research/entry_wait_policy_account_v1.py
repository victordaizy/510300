...
def simulate_entry_wait_policy(data, dividends, config, cost, start, rule, spec, entry_views):
    ...
    require(pd.DatetimeIndex(entry_views.date).equals(dates), "进入模型与原价格日历不符")
    ...
    def decide(t):
        ...
        view = entry_views.iloc[t]
        admission = {"entry_model_status": view.entry_model_status, "entry_value_margin": view.entry_value_margin,
                     "buy_value": view.buy_value, "wait_value": view.wait_value, "entry_decision_rule": view.entry_decision_rule,
                     "entry_fit_origin": view.entry_fit_origin}
        if account.shares:
            ...
            selected_mode = mode
            if np.isfinite(view.entry_value_margin) and view.entry_value_margin > 0:
                qty = affordable_quantity(account.cash, fill_price(cl[t], 1, cost, config["tick"]), cost, config["lot"])
                action = "原进入条件和本轮价值比较均满足，下一开盘请求买入"
            elif np.isfinite(view.entry_value_margin):
                action = "本轮价值比较不支持立即进入，继续等待下一收盘"
            else:
                action = "进入模型无可用判断，本收盘不申请新的买入"
        ...
        if not account.shares and mode and not np.isfinite(view.entry_value_margin):
