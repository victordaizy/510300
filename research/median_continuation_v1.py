...
from research.median_continuation_inputs_v1 import fit_median, median_chinese_formula, MedianExitController
from research.rearmed_cycle_exit_account_v1 import simulate_rearmed_exit
from research.simple_intraday_protection_v1 import make_rules

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports/research/510300_median_continuation_v1"
CONFIG = ROOT / "config/510300_median_continuation_v1.json"
...
def freeze():
    ...
    require(tests["exit_code"] == 0, "中位数与成熟成交测试尚未通过")
    for item in old["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "原入场退出及模型来源身份改变")
    cfg.update(study_id="510300_MEDIAN_CONTINUATION_V1", round=71, registered_at=now(), primary=PRIMARY, candidate_configurations=1,
        quantile=.5, alpha=0., fit_intercept=True, solver="highs", model_loss="CYCLE_EQUAL_WEIGHTED_ABSOLUTE_ERROR",
        fit_failure="NO_VIEW_MODEL_FIT_FAILED_KEEP_ORIGINAL_EXITS_NO_FALLBACK_SOLVER", rules="docs/510300_MEDIAN_CONTINUATION_V1.md",
        samples=str((P31 / "all_reference_samples.parquet").relative_to(ROOT)), new_reference_accounts=0, source_budget_cny=0,
        previous_goal_turn_classification="PROGRESS_ROUND70_COMPLETED_AND_DELIVERED", position_impact=0, goal_achieved=False,
    ...
    paths = [Path(__file__), ROOT / "research/median_continuation_inputs_v1.py", ROOT / "research/rearmed_cycle_exit_account_v1.py",
        ...
        ROOT / "tests/test_median_continuation_v1.py", OUT / "tests_receipt.json"]
    ...
    print("第71轮一个条件中位数继续价值设置已冻结，尚未拟合或读取新账户收益。", flush=True)


def train_models(data, samples, originals, cfg):
    models, receipts, memberships = [], [], []
    chinese = ["# 每月中位数继续价值模型：全部系数与中文计算规则", "", "只使用当时已完全自然结束的参考周期；没有模型不填预测数值。", ""]
    for original in originals:
        ...
        if eligible:
            try:
                stored = fit_median(rows, cfg)
                status = "FIT_COMPLETE"
            except (ConvergenceWarning, RuntimeError, FloatingPointError) as error:
        ...
        chinese += median_chinese_formula(stored) + [""] if stored is not None else ["本月没有有效中位数模型，继续执行原价格及时间退出。", ""]
    ...
    (OUT / "每月中位数退出模型中文规则.md").write_text("\n".join(chinese) + "\n", encoding="utf-8")
    print(f"原成熟时点不变：中位数拟合完成{sum(r['status']=='FIT_COMPLETE' for r in receipts)}次，未成熟或求解失败{sum(r['status']!='FIT_COMPLETE' for r in receipts)}次。", flush=True)
...
def run():
    ...
    for item in cfg["frozen_files"]:
        require(digest(ROOT / item["path"]) == item["sha256"], "中位数冻结来源改变")
    ...
    for period, frame, start, dest in [("evaluation", data, cfg["evaluation_start"], main),
        ...
        for cost_id, cost in cfg["costs"].items():
            ...
            ledger, decisions, cycles = simulate_rearmed_exit(frame, dividends, cfg, cost, start, rule, cfg["specification"], MedianExitController(frame, models, cfg["confirmation_days"]))
            save_account(folder, PRIMARY, ledger, decisions)
            cycles.to_csv(folder / f"{PRIMARY}_cycles.csv", index=False, encoding="utf-8-sig")
            require(ledger.accounting_error.abs().max() < 1e-6 and not ledger.terminal_unliquidated.iloc[-1], "中位数账户结算不符")
            require((cycles.dropna(subset=["exit_date"]).holding_intervals >= 1).all(), "中位数退出违反次日可卖")
            ...
            accounts, names = {PRIMARY: ledger}, {PRIMARY: "条件中位数继续价值退出"}
            ...
            for model, saved in accounts.items():
                require(pd.DatetimeIndex(saved.date).equals(pd.DatetimeIndex(accounts["BUY_HOLD"].date)), "中位数及对照完整日历不同")
            ...
            print(f"{period}／{cost_id}：一个新中位数账户和四个保存对照已完成。", flush=True)
    ...
    result = {"study_id": cfg["study_id"], "completed_at": now(), "status": "MEDIAN_CONTINUATION_ACCOUNTS_COMPLETE", "candidate_configurations": 1,
