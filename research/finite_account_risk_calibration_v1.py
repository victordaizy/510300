...
from research.finite_account_risk_calibration_inputs_v1 import CANDIDATES, MODELS, PARENT, PRIMARY, SOURCE, RISK_TARGETS, calibration_frames
...
OUT = ROOT / 'reports/research/510300_finite_account_risk_calibration_v1'
CONFIG = ROOT / 'config/510300_finite_account_risk_calibration_v1.json'
PARENTS = {PARENT: SOURCE}
CONTROLS = {PARENT:(SOURCE,'第174轮原组合'),
...
def prepare():
    require(not CONFIG.exists() and not (OUT/'RUN_STARTED.json').exists(), '第194轮已存在，请接续')
    OUT.mkdir(parents=True, exist_ok=True)
    receipt = OUT/'tests_receipt.json'
    require(not receipt.exists(), '必要测试已有回执')
    began = time.perf_counter()
    test = subprocess.run([sys.executable, '-m', 'pytest', 'tests/test_finite_account_risk_calibration_v1.py',
    ...
    require(test.returncode == 0 and '5 passed' in test.stdout, '第194轮必要测试未通过')
    write_json(receipt, {'recorded_at': now(), 'exit_code': test.returncode, 'passed': 5,
    ...
    cfg.update(study_id='510300_FINITE_ACCOUNT_RISK_CALIBRATION_V1', round=194, primary=PRIMARY,
        candidate_models=list(CANDIDATES), candidate_configurations=2, parent_models=MODELS,
        account_risk_window=60, risk_targets=RISK_TARGETS.copy(), annual_return_target=.10,
        ...
        rules='docs/510300_FINITE_ACCOUNT_RISK_CALIBRATION_V1.md',
        evidence_class='EXPLICIT_RETROSPECTIVE_FINITE_PARAMETER_CALIBRATION',
        legacy_meets_point_target_field='SHARPE_ONLY_USE_JOINT_TARGET_ASSESSMENT',
        previous_goal_turn_classification='PROGRESS_ROUND193_FOUR_ACCOUNTS_VERIFIED_DELIVERED')
    ...
    paths = [Path(__file__), ROOT/'research/finite_account_risk_calibration_inputs_v1.py', ROOT/'research/account_volatility_exposure_inputs_v1.py', ROOT/'research/joint_account_acceptance_v1.py',
             ...
             ROOT/'tests/test_finite_account_risk_calibration_v1.py', receipt, OUT/'tests_output.txt',
    ...
    require(i['latest_completed_round']['round'] == 193, '前序轮次不同')
    i['running_studies'] = [{'round': 194, 'study': cfg['study_id'], 'status': 'FROZEN_NOT_STARTED', 'config': str(CONFIG.relative_to(ROOT))}]
    i['next_work'].update(registered=True, status='FINITE_ACCOUNT_RISK_CALIBRATION_FROZEN', source=cfg['rules'])
    write_json(p, i)
    print('第194轮两档有限风险预算已冻结。', flush=True)
...
def verify():
    require(not (OUT/'saved_verification_receipt.json').exists(), '第194轮已经核对')
    cfg = json.loads(CONFIG.read_text(encoding='utf-8'))
    ...
    def expected(period, cost_id, frame, start, model):
        ...
        for t in range(len(frame)):
            if t < first+59:
                ...
                if np.isfinite(sample).all():
                    ...
                    multiplier[t] = cfg['risk_targets'][model]/risk[t] if risk[t] > 0 else 1.
        ...
        np.testing.assert_allclose(factors[model+'_multiplier'], multiplier, atol=1e-10, rtol=1e-12, equal_nan=True)
        np.testing.assert_allclose(factors[model+'_target'], target, atol=1e-12, rtol=1e-12, equal_nan=True)
        ...
        checks.append({'model':model, 'period': period, 'cost': cost_id, 'decisions': len(indices), 'fills': int(filled.sum()),
                       'maximum_target_error': float(np.nanmax(abs(factors[model+'_target'].to_numpy()-target)))})
        return target
    ...
    receipt = {'verified_at': now(), 'status': 'PASS_EIGHT_SIMULATED_ACCOUNTS_FINITE_RISK_CALIBRATION_AND_COSTS',
