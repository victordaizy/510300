...
- 生成时间：`2026-08-19T20:21:29.850062+08:00`
...
- 状态：`RUNNING_SESSION_SCOPED_DEGRADED_AUTOMATION`
- 已核验安装：`False`
- 手动运行器已验证：`True`
...
- `BLOCK` `WINDOWS_TASK_SCHEDULER` `BLOCKED_PERSISTENT_AUTOMATION_PERMISSION`：当前登录会话的隐藏守护进程正在运行，但任务调度器和登录自启动均未获权限；重启或退出登录后不能自动恢复。
