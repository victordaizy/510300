...
from research.addition_gate_exposure_batch_inputs_v1 import CANDIDATES, MODELS, SETTINGS, allowance_column, gate_frames, gate_request, market_factors, simulate_gate_account
...
def run(targets, frame=None, model='ADD_GATE_EXPOSURE_110'):
...
def test_four_mappings_cost_identity_and_market_factor_selection():
    ...
    assert len(SETTINGS) == 4 and len(summaries) == 8
    for model, setting in SETTINGS.items():
        assert allowance_column(setting) in frames['BASE']
        np.testing.assert_allclose(frames['BASE'][model+'_target'].iloc[:-1], .5*setting['exposure_multiplier'])
    ...
    cfg['candidate_settings']['ADD_GATE_EXPOSURE_105']['direction'] = 'UPPER'
...
def test_future_prices_targets_and_direction_flags_do_not_change_past_accounts():
    ...
    for model in ['ADD_GATE_EXPOSURE_110', 'ADD_GATE_EXPOSURE_120']:
