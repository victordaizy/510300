"""验证除息成交量、九因子周期内方程及实际退出。"""
import numpy as np
import pandas as pd
import pytest
from research.signed_volume_balance_inputs_v1 import signed_volume_balance
from research.signed_volume_within_inputs_v1 import FEATURES, fit_signed_volume_within, signed_volume_prediction, SignedVolumeWithinController
...
def constant(value=-.01, t=0, latest=None, volume_coefficient=0.):
    return {"fit_index": t, "latest_exit_index": t if latest is None else latest, "status": "FIT_COMPLETE",
            "model": {"kind": "SIGNED_VOLUME_WITHIN_CYCLE_RIDGE", "features": FEATURES.copy(), "mean": [0.]*9, "scale": [1.]*9,
                      "coefficients": [0.]*8+[volume_coefficient], "intercept": value, "feature_clip": 5.,
                      "cycle_intercepts": [{"cycle_id": 999, "cycle_intercept": 100.}]}}


def factors(data, value=0.):
    return pd.DataFrame({"date": data.date, "signed_volume_balance20": value})


def test_dividend_adjusted_direction_and_volume_unit_invariance():
    data = pd.DataFrame({"date": pd.bdate_range("2020-01-01", periods=4), "close": [10., 9.5, 9.4, 9.3],
                         "previous_close": [10., 10., 9.5, 9.4], "dividend": [0., .7, .1, 0.], "volume": [100., 200., 300., 400.]})
    f = signed_volume_balance(data, window=2)
    np.testing.assert_array_equal(f.adjusted_return_direction, [0., 1., 0., -1.])
    np.testing.assert_allclose(f.signed_volume_balance20.iloc[1:], [2/3, 2/5, -4/7], atol=1e-12, rtol=0)
    larger = data.copy(); larger.volume *= 100.
    np.testing.assert_allclose(f.signed_volume_balance20, signed_volume_balance(larger, 2).signed_volume_balance20, equal_nan=True)
    raw = data.copy(); raw.dividend = 0.
    assert signed_volume_balance(raw, 2).adjusted_return_direction.iloc[1] == -1.


def test_missing_window_zero_volume_and_future_changes_preserve_known_prefix():
    data = fixture()[0]; data["volume"] = 100.
    data.loc[1, "close"] = 10.1
    base = signed_volume_balance(data, 2)
    changed = data.copy(); changed.loc[5:, ["volume", "close"]] = 999.
    pd.testing.assert_frame_equal(base.iloc[:5], signed_volume_balance(changed, 2).iloc[:5])
    missing = data.copy(); missing.loc[2, "volume"] = np.nan
    f = signed_volume_balance(missing, 2)
    assert f.signed_volume_balance20.iloc[2:4].isna().all() and np.isfinite(f.signed_volume_balance20.iloc[4])
    missing.loc[2, "volume"] = -1.
    assert signed_volume_balance(missing, 2).signed_volume_balance20.iloc[2:4].isna().all()
    zero = data.copy(); zero.volume = 0.
    z = signed_volume_balance(zero, 2)
    assert z.signed_volume_balance20.isna().all() and z.factor_status.iloc[-1] == "NO_VIEW_ZERO_WINDOW_VOLUME"


def test_nine_factor_fit_equals_joint_unpenalized_cycle_intercept_equation():
    rows = eight_samples(); rows[FEATURES[-1]] = np.random.default_rng(116).uniform(-1, 1, len(rows))
    rows.target += .04*rows[FEATURES[-1]]
    m = fit_signed_volume_within(rows, CFG)
    ...
    assert signed_volume_prediction(m, x) == pytest.approx(b[9:].mean()+z[0]@b[:9])


def test_within_constant_volume_adds_no_slope_and_future_cycles_or_length_do_not_reweight():
    ...
    for key in ["coefficients", "mean", "scale", "intercept"]:
        np.testing.assert_allclose(m[key], fit_signed_volume_within(repeated, CFG)[key], atol=1e-12, rtol=0)
    rows["origin_index"] = np.arange(len(rows)); rows["exit_index"] = rows.cycle_id.map({1: 25, 2: 55, 3: 95})
    mature, ids = training_rows(rows, 55, {"recent_cycles": 20}); before = fit_signed_volume_within(mature, CFG)
    assert ids == [1, 2]
    rows.loc[rows.cycle_id.eq(3), [FEATURES[-1], "target"]] = 1000.
    assert fit_signed_volume_within(training_rows(rows, 55, {"recent_cycles": 20})[0], CFG) == before
    mature.loc[mature.index[0], FEATURES[-1]] = np.nan
    with pytest.raises(ValueError, match="禁止删行"):
        fit_signed_volume_within(mature, CFG)
...
def test_actual_current_close_factor_missing_confirmation_and_future_model_clock():
    ...
    c = SignedVolumeWithinController(data, [constant(value=0., volume_coefficient=1.)], f)
    ...
    assert SignedVolumeWithinController(data, [constant(t=5)], f)(1, cycle, 100000., 100000.)["continuation_prediction"] is None
    with pytest.raises(ValueError, match="未来周期"):
        SignedVolumeWithinController(data, [constant(latest=5)], f)(1, cycle, 100000., 100000.)
    c = SignedVolumeWithinController(data, [constant()], factors(data))
...
def test_actual_next_open_exit_lock_and_original_exit_without_model():
    ...
    ledger, decisions, cycles = simulate_rearmed_exit(*args, SignedVolumeWithinController(args[0], [constant(), constant(.03, 3)], factors(args[0])))
    ...
    ledger, decisions, cycles = simulate_rearmed_exit(*args, SignedVolumeWithinController(args[0], [], factors(args[0])))
