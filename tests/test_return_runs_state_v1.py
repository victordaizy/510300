"""核对收益强弱连续段、平局、未知与真实资金进出场。"""
import numpy as np
import pandas as pd
import pytest
from research.return_runs_state_inputs_v1 import run_count_summary, confirmed_runs_directions, return_runs_state_factors, return_runs_state_frames, PRIMARY, CANDIDATES
...
CFG = {**OLD_CFG, 'candidate_models': list(CANDIDATES), 'runs_window': 60, 'momentum_window': 60,
    'volatility_window': 20, 'entry_threshold': -1., 'exit_threshold': 0., 'confirmation_closes': 2, 'risk_target': .1}
...
def test_hand_count_same_marginals_different_order_ties_and_constant():
    clustered = run_count_summary([2., 2., 2., 0., 0., 0.])
    assert clustered['median_return60'] == 1. and clustered['actual_runs60'] == 2
    assert clustered['strong_count60'] == clustered['weak_count60'] == 3
    assert clustered['expected_runs60'] == 4. and clustered['run_variance60'] == pytest.approx(1.2)
    assert clustered['run_score60'] == pytest.approx(-2/np.sqrt(1.2))
    alternating = run_count_summary([2., 0., 2., 0., 2., 0.])
    assert alternating['actual_runs60'] == 6 and alternating['run_score60'] == pytest.approx(2/np.sqrt(1.2))
    tied = run_count_summary([-2., 0., 2., 0., -1., 1., 0.])
    assert tied['tie_count60'] == 3 and tied['non_tie_count60'] == 4 and tied['actual_runs60'] == 4
    assert tied['run_score60'] == pytest.approx(1/np.sqrt(2/3))
    positive = run_count_summary([.1, .2, .3, .4])
    assert positive['strong_count60'] == positive['weak_count60'] == 2
    assert np.isnan(run_count_summary(np.zeros(60))['run_score60'])
    assert np.isnan(run_count_summary([0., 0., 1.])['run_score60'])
    with pytest.raises(ValueError):
        run_count_summary([1., np.nan])


def test_strict_entry_inclusive_exit_separate_counts_and_unknown_restart():
    scores = [-1., -2., -2., -.5, 0., -.5, 0., 0., -2., np.nan, -2., -2., -2., -2.]
    momentum = [.1, .1, .1, .1, .1, 0., .1, .1, .1, .1, .1, .1, 0., 0.]
    f = confirmed_runs_directions(scores, momentum)
    np.testing.assert_allclose(f.positive_direction, [0, 0, 1, 1, 1, 1, 1, 0, 0, np.nan, 0, 1, 1, 0], equal_nan=True)
    assert f.runs_exit_confirmation_count.iloc[4] == 1 and f.runs_exit_confirmation_count.iloc[7] == 2
...
def test_complete_window_missing_reset_and_ex_dividend_constant_unknown():
    ...
    f = return_runs_state_factors(data)
    assert f.run_score60.iloc[:60].isna().all() and pd.notna(f.run_score60.iloc[60])
    data.loc[150, 'total_log'] = np.nan
    changed = return_runs_state_factors(data)
    assert changed.run_score60.iloc[150:210].isna().all() and pd.notna(changed.run_score60.iloc[210])
    frames, _ = return_runs_state_frames(data, parents, CFG, start)
    ...
    ex_factors = return_runs_state_factors(ex)
    assert ex_factors.total_daily_log.iloc[125] == 0. and ex_factors.momentum60.iloc[60:].eq(0.).all()
    assert ex_factors.run_score60.isna().all() and ex_factors.non_tie_count60.iloc[60:].eq(0.).all()
...
def test_future_prefix_and_both_costs_preserve_target_clock():
    data, parents, start = fixture()
    original, _ = return_runs_state_frames(data, parents, CFG, start)
    changed = data.copy(); changed.loc[300:, 'total_log'] *= -1.
    future, _ = return_runs_state_frames(changed, parents, CFG, start)
    pd.testing.assert_frame_equal(original['BASE'].iloc[:300], future['BASE'].iloc[:300])
    short, _ = return_runs_state_frames(data.iloc[:300], parents, CFG, start)
