"""验证稀疏最优解、周期权重、因果缓存、固定版本和真实进出场。"""
import copy
import numpy as np
import pandas as pd
import pytest
from scipy.linalg import hadamard
from research import sparse_vintage_exit_inputs_v1 as module
...
CFG = {"feature_clip": 5., "lasso_alpha": .0014, "lasso_max_iter": 10000, "lasso_tolerance": 1e-10,
       "kkt_tolerance": 1e-8, "recent_cycles": 20, "minimum_cycles": 4, "minimum_rows": 40}
...
def test_nonzero_solution_matches_orthogonal_soft_threshold_and_constant_factor_is_zero():
    rows = sample()
    fitted = module.fit_sparse_cycle(rows, CFG)
    expected = np.zeros(8); expected[:2] = [.0386, -.0186]
    ...
    assert fitted["kkt_max_violation"] < 1e-12
...
def test_whole_cycle_weight_is_invariant_when_one_cycles_observations_are_duplicated():
    ...
    a, b = module.fit_sparse_cycle(rows, CFG), module.fit_sparse_cycle(duplicated, CFG)
    ...
    with pytest.raises(ValueError, match="总权重"):
        module.fit_sparse_cycle(duplicated, CFG)


def test_exact_zero_solution_is_valid_and_missing_data_cannot_be_deleted():
    rows = sample()
    rows["target"] = rows.cycle_id * .1
    fitted = module.fit_sparse_cycle(rows, CFG)
    assert fitted["nonzero_factor_count"] == 0 and fitted["solver_iterations"] == 0
    assert module.sparse_prediction(fitted, rows[FEATURES].iloc[0].to_numpy()) == pytest.approx(.25)
    rows.loc[0, FEATURES[2]] = np.nan
    with pytest.raises(ValueError, match="禁止删行"):
        module.fit_sparse_cycle(rows, CFG)
...
def test_same_actual_input_is_fit_once_and_future_month_does_not_change_earlier_models(monkeypatch):
    ...
    original, calls = module.fit_sparse_cycle, []
    def counted(*args):
        calls.append(1)
        return original(*args)
    monkeypatch.setattr(module, "fit_sparse_cycle", counted)
...
def test_failed_input_is_cached_without_retry_or_original_model_fallback(monkeypatch):
    ...
    monkeypatch.setattr(module, "fit_sparse_cycle", failed)
    full, _, _, counts = module.build_monthly_models(rows, records, CFG)
...
def test_entry_version_is_fixed_missing_state_resets_count_and_future_model_is_rejected():
    ...
    controller = module.SparseVintageExitController(data, [constant(-.01), constant(.05, 2)])
    ...
    with pytest.raises(ValueError, match="未来周期"):
        module.SparseVintageExitController(data, [constant(-.1, 0, 8)])(5, cycle, 100000., 100000.)
    with pytest.raises(ValueError, match="首次持仓收盘"):
        module.SparseVintageExitController(data, [constant(-.1)])(6, cycle, 100000., 100000.)
...
def test_real_blocked_exit_reentry_dividend_and_price_exit_without_initial_model():
    ...
    args[5]["entry"][5] = 0
    ledger, decisions, cycles = simulate_rearmed_exit(*args, module.SparseVintageExitController(data, [constant(-.01), constant(.03, 3)]))
    ...
    ledger, decisions, cycles = simulate_rearmed_exit(*args, module.SparseVintageExitController(args[0], [constant(-.5, 2)]))
