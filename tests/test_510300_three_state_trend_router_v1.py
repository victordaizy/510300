...
from three_state_trend_router_v1 import (  # noqa: E402
    ALL_STATES,
    CONFIG_PATH,
    MANIFEST_PATH,
    STATE_BEAR,
...
def test_frozen_config_core_contract() -> None:
    config = load_config()
    assert config["protocol"]["project_id"] == "510300_THREE_STATE_TREND_ROUTER_V1"
...
def test_result_replay_contract_when_frozen() -> None:
    if not MANIFEST_PATH.exists():
        pytest.skip("首次冻结前尚无清单")
    from three_state_trend_router_v1 import run_study
