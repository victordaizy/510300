...
import pytest
from research.forward_eps_guosen_layout_v2 import parse,exact_row,internal_date
...
def test_caption_year_is_not_a_prediction_column():
    p,m,r=fixture();p[0]=p[0].replace('盈利预测和财务指标 ', '盈利预测和财务指标（2021年起采用新口径）\n')
    assert [x['eps_value_exact'] for x in parse(p,m,r)['facts']]==['2.24','2.40','2.67']


def test_eps_alias_requires_same_latest_share_basis_note():
    p,m,r=fixture();p[0]=p[0].replace('摊薄每股收益(元)','每股收益(元)')
    f=parse(p,m,r)['facts'][0]
    assert f['eps_value_exact']=='2.24' and f['source_eps_label']=='每股收益' and f['eps_unit_as_reported']=='元/股'


def test_diluted_only_label_does_not_invent_currency_unit():
    p,m,r=fixture();p[0]=p[0].replace('摊薄每股收益(元)','每股收益(摊薄)')
    f=parse(p,m,r)['facts'][0]
    assert f['eps_value_exact']=='2.24' and f['eps_unit_as_reported']=='原行仅标注摊薄，金额单位未明确'
