...
FEATURE_FILE = ROOT / "data" / "features" / "510300_round3_breadth_dataset.parquet"
RESEARCH_FILE = ROOT / "reports" / "research" / "round3_breadth_research.json"
ETF_FILE = ROOT / "data" / "raw" / "market" / "510300_daily_raw.parquet"
DIVIDEND_FILE = ROOT / "data" / "reference" / "510300_dividends.csv"
JSON_FILE = ROOT / "reports" / "backtest" / "round3_breadth_backtests.json"
MARKDOWN_FILE = ROOT / "reports" / "backtest" / "round3_breadth_backtests.md"
LEDGER_ROOT = ROOT / "data" / "processed" / "round3_breadth_backtests"
...
def _render_markdown(report: dict) -> str:
    ...
    if eligible:
        for item in eligible:
            lines.append(f"- `{item['factor_id']}`进入真正前向观察，但不得冻结或宣称已验证。")
    else:
...
def main() -> int:
    for path in (SETTINGS_FILE, REGISTRY_FILE, FEATURE_FILE, RESEARCH_FILE, ETF_FILE, DIVIDEND_FILE):
        if not path.exists():
            raise FileNotFoundError(f"第三轮回测输入缺失：{path}")
        ...
        factor_results.append(
            {
                ...
                "candidate_status": "ROUND3_PROVISIONAL_FOR_TRUE_FORWARD" if eligible else "NOT_ELIGIBLE",
            }
        )
    ...
    print(f"第三轮宽度回测报告：{MARKDOWN_FILE}")
    return 0
